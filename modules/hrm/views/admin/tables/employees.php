...
$has_permission_delete = has_permission('hr', '', 'delete');

$custom_fields = get_custom_fields('hr', [
    'show_on_table' => 1,
    ]);
$aColumns = [
    'firstname',
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'last_login') {
            ...
            $_data = '<a href="' . admin_url('hr/employees/profile/' . $aRow['staffid']) . '">' . staff_profile_image($aRow['staffid'], [
                'staff-profile-image-small',
                ]) . '</a>';
            $_data .= ' <a href="' . base_url('hrm/employees/member/' . $aRow['staffid'] . '?group=basic') . '">' . $aRow['firstname'] . ' ' . $aRow['lastname'] . '</a>';

            $_data .= '<div class="row-options">';
            $_data .= '<a href="' . base_url('hrm/employees/member/' . $aRow['staffid'] . '?group=basic') . '">' . _l('view') . '</a>';
