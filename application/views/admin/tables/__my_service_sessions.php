...
//$_data .= _l('task_start_timer');
//$_data .= ' | <a href="#" onclick="edit_session(' . $aRow['id'] . '); return false">' . _l('edit') . '</a>';
//$_data .= ' | <a href="' . admin_url('LegalServices/Cases_controller/move_to_recycle_bin/' .$service_id.'/'. $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
//$_data .= '</div>';
//$row[] = $_data;
...
foreach ($rResult as $aRow) {
  ...
  $row[] = $aRow['time'];
  if($aRow['send_to_customer'] == 0):
      $stc = '<a href="#" data-toggle="modal" data-target="#customer_report" class="btn btn-info pull-left display-block">';
      $stc .=  _l('add_new') . '<i class="fa fa-plus"></i>  ';
      $stc .= '</a>';
  else:
      $stc = '<a href="#" class="btn btn-info pull-left display-block">';
