...
if(!is_admin() && !has_permission('additional_timesheets_management','','view')){
    array_push($where,'AND ('. get_staff_user_id() .' in (select staffid from '.db_prefix().'timesheets_approval_details where rel_type = "additional_timesheets" and rel_id = '.db_prefix().'timesheets_additional_timesheet.id))');
}
...
foreach ($rResult as $aRow) {
    ...
    if(has_permission('leave_management', '', 'view') || $aRow['creator'] == get_staff_user_id() || is_admin()){
