...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if($aColumns[$i] == '1'){
            ...
            $link = admin_url('hr/knowledge_base_q_a/view/' . $aRow['slug']);
            ...
            if (has_permission('hr_manage_q_a', '', 'edit')) {
                $_data = '<a href="' . admin_url('hr/knowledge_base_q_a/article/' . $aRow['articleid']) . '" class="font-size-14">' . $_data . '</a>';
            } else {
            ...
            if (has_permission('hr_manage_q_a', '', 'edit')) {
                $_data .= ' | <a href="' . admin_url('hr/knowledge_base_q_a/article/' . $aRow['articleid']) . '">' . _l('hr_edit') . '</a>';
            }

            if (has_permission('hr_manage_q_a', '', 'delete')) {
                $_data .= ' | <a href="' . admin_url('hr/knowledge_base_q_a/delete_article/' . $aRow['articleid']) . '" class="_delete text-danger">' . _l('delete') . '</a>';
            }
