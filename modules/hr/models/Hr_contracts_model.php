...
class Hr_contracts_model extends App_Model
{
    ...
    {
        $where['type_id'] = 0;
        $this->db->select('*,' . db_prefix() . 'contracts_types.name as type_name,' . db_prefix() . 'hr_contracts.id as id, ' . db_prefix() . 'hr_contracts.addedfrom');
        $this->db->where($where);
        $this->db->join(db_prefix() . 'contracts_types', '' . db_prefix() . 'contracts_types.id = ' . db_prefix() . 'hr_contracts.contract_type', 'left');
        $this->db->join(db_prefix() . 'staff', '' . db_prefix() . 'staff.staffid = ' . db_prefix() . 'hr_contracts.client');
        if (is_numeric($id)) {
            $this->db->where(db_prefix() . 'hr_contracts.id', $id);
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(datestart)) as year FROM ' . db_prefix() . 'hr_contracts')->result_array();
    }
    ...
    public function get_contract_attachments($attachment_id = '', $id = '')
    {
        ...
        $this->db->where('rel_type', 'hr_contract');
    ...
    public function add($data)
    {
        ...
        $this->db->insert(db_prefix() . 'hr_contracts', $data);
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hr_contracts', $data);
    ...
    public function clear_signature($id)
    {
        ...
        if ($contract) {
            $this->db->where('id', $id);
            $this->db->update(db_prefix() . 'hr_contracts', array_merge(get_acceptance_info_array(true), ['signed' => 0]));
    ...
    public function add_comment($data, $client = false)
    {
        ...
        $this->db->insert(db_prefix() . 'hr_contract_comments', $data);
        $insert_id = $this->db->insert_id();
    ...
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hr_contract_comments', [
    ...
    public function get_comments($id)
    {
        ...
        return $this->db->get(db_prefix() . 'hr_contract_comments')->result_array();
    }
    ...
    public function get_comment($id)
    {
        ...
        return $this->db->get(db_prefix() . 'hr_contract_comments')->row();
    }
    ...
    public function remove_comment($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'hr_contract_comments');
        if ($this->db->affected_rows() > 0) {
    ...
    public function copy($id)
    {
        ...
        $fields         = $this->db->list_fields(db_prefix() . 'hr_contracts');
        $newContactData = [];
    ...
    public function delete($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'hr_contracts');
        if ($this->db->affected_rows() > 0) {
            $this->db->where('contract_id', $id);
            $this->db->delete(db_prefix() . 'hr_contract_comments');
            ...
            $this->db->delete(db_prefix() . 'hr_contract_renewals');
            // Get related tasks
            $this->db->where('rel_type', 'hr_contract');
            $this->db->where('rel_id', $id);
    ...
    public function delete_contract_attachment($attachment_id)
    {
        ...
        if ($attachment) {
            if (empty($attachment->external)) {
                unlink(get_upload_path_by_type('hr_contract') . $attachment->rel_id . '/' . $attachment->file_name);
            }
            $this->db->where('id', $attachment->id);
            $this->db->delete(db_prefix() . 'files');
            if ($this->db->affected_rows() > 0) {
                $deleted = true;
                log_activity('HR Contract Attachment Deleted [ContractID: ' . $attachment->rel_id . ']');
            }

            if (is_dir(get_upload_path_by_type('hr_contract') . $attachment->rel_id)) {
                // Check if no attachments left, so we can delete the folder also
                $other_attachments = list_files(get_upload_path_by_type('hr_contract') . $attachment->rel_id);
                if (count($other_attachments) == 0) {
                    // okey only index.html so we can delete the folder also
                    delete_dir(get_upload_path_by_type('hr_contract') . $attachment->rel_id);
                }
            }
        }
    ...
    public function renew($data)
    {
        ...
        $this->db->insert(db_prefix() . 'hr_contract_renewals', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            ...
            $this->db->update(db_prefix() . 'hr_contracts', $_data);
            if ($this->db->affected_rows() > 0) {
    ...
    public function delete_renewal($id, $contractid)
    {
        ...
        $this->db->select('id')->from(db_prefix() . 'hr_contract_renewals')->where('contractid', $contractid)->order_by('id', 'desc')->limit(1);
        $query                 = $this->db->get();
        ...
        if ($last_contract_renewal == $id) {
            $is_last = true;
            $this->db->where('id', $id);
            $original_renewal = $this->db->get(db_prefix() . 'hr_contract_renewals')->row();
        }
    ...
    public function get_contracts_about_to_expire($staffId = null, $days = 7)
    {
        ...
        return $this->db->get(db_prefix() . 'hr_contracts')->result_array();
    }
    ...
    public function get_contract_renewal_history($id)
    {
        ...
        return $this->db->get(db_prefix() . 'hr_contract_renewals')->result_array();
    }
