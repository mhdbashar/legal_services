...
$sIndexColumn = 'id';
$sTable       = db_prefix().'hr_profile_immigration';

$now = date('Y/m/d');

$join = [
	'LEFT JOIN '.db_prefix().'staff ON '.db_prefix().'hr_profile_immigration.staff_id='.db_prefix().'staff.staffid'
];
...
if (has_permission('expired_documents', '', 'delete')){
    $row[]   = icon_btn('hr_profile/delete_immigration/' . $aRow['id'], 'remove', 'btn-danger _delete');
    }
