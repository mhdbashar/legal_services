...
class Payments_model extends App_Model
{
    ...
    {
        $this->db->select('*,' . db_prefix() . 'my_project_invoicepaymentrecords.id as paymentid');
        $this->db->join(db_prefix() . 'payment_modes', db_prefix() . 'payment_modes.id = ' . db_prefix() . 'my_project_invoicepaymentrecords.paymentmode', 'left');
        $this->db->order_by(db_prefix() . 'my_project_invoicepaymentrecords.id', 'asc');
        $this->db->where(db_prefix() . 'my_project_invoicepaymentrecords.id', $id);
        $payment = $this->db->get(db_prefix() . 'my_project_invoicepaymentrecords')->row();
        if (!$payment) {
            return false;
        }
        // Since version 1.0.1
    ...
    public function get_invoice_payments($invoiceid)
    {
        ...
        $payments = $this->db->get(db_prefix() . 'my_project_invoicepaymentrecords')->result_array();
        // Since version 1.0.1
        $this->load->model('payment_modes_model');
        $payment_gateways = $this->payment_modes_model->get_payment_gateways(true);
        $i                = 0;
    ...
    public function process_payment($data, $invoiceid = '')
    {
        ...
        // Is online payment mode request by client or staff
            ...
            if (get_option('allow_payment_amount_to_be_modified') == 0) {
                $data['amount'] = disputes_get_invoice_total_left_to_pay($invoiceid, $invoice->total);
            }
    ...
    public function add($data, $subscription = false)
    {
        ...
        $this->db->insert(db_prefix() . 'my_project_invoicepaymentrecords', $data);
        ...
        if ($insert_id) {
            ...
            disputes_update_invoice_status($data['invoiceid'], $force_update);
            ...
            if (!isset($do_not_send_email_template)
                ...
                if ($subscription != false) {
                    ...
                    if ($after_success == 'send_invoice_and_receipt' || $after_success == 'send_invoice') {
                        $invoice_number = disputes_format_invoice_number($payment->invoiceid);
                        set_mailing_constant();
                ...
                if (get_option('notification_when_customer_pay_invoice') == 1) {
                    ...
                    $notified = add_notification([
                        ...
                        'additional_data' => serialize([
                            disputes_format_invoice_number($invoice->id),
                        ]),
                    ]);
                    if ($notified) {
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'my_project_invoicepaymentrecords', $data);
    ...
    public function delete($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            log_activity('Payment Deleted [ID:' . $id . ', Invoice Number: ' . disputes_format_invoice_number($current->id) . ']');
