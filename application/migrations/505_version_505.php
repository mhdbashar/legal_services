...
class Migration_Version_505 extends CI_Migration
{
    ...
    public function up()
    {
        ...
        if (!$this->db->field_exists('individual', db_prefix() . 'clients')) {
            $this->db->query("ALTER TABLE `tblclients` ADD `individual` tinyint(4) NOT NULL DEFAULT '1';");
        }

        if (!$this->db->field_exists('branch_id', db_prefix() . 'clients')) {
            $this->db->query("ALTER TABLE `tblclients` ADD `branch_id` int(11) NOT NULL DEFAULT '0';");
        }

        if (!$this->db->field_exists('client_type', db_prefix() . 'clients')) {
            $this->db->query("ALTER TABLE `tblclients` ADD `client_type` int(11) NOT NULL DEFAULT '0';");
        }

        if (!$this->db->field_exists('short_name_ar', db_prefix() . 'countries')) {
            $this->db->query("ALTER TABLE `tblcountries` ADD `short_name_ar` varchar(80) NOT NULL DEFAULT '';");
        ...
        if (!$this->db->table_exists(db_prefix() . 'my_customer_company_groups')) {
            $this->db->query('CREATE TABLE `tblmy_customer_company_groups` (
              `id` int(11) NOT NULL,
              `groupid` int(11) NOT NULL,
              `customer_id` int(11) NOT NULL            
            ) ENGINE=InnoDB DEFAULT CHARSET=' . $this->db->char_set . '');

            $this->db->query("ALTER TABLE `tblmy_customer_company_groups` ADD PRIMARY KEY (`id`);");
            $this->db->query("ALTER TABLE `tblmy_customer_company_groups` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;");
        }

        if (!$this->db->table_exists(db_prefix() . 'my_customers_company_groups')) {
            $this->db->query('CREATE TABLE `tblmy_customers_company_groups` (
              `id` int(11) NOT NULL,
              `name` varchar(191) NOT NULL           
            ) ENGINE=InnoDB DEFAULT CHARSET=' . $this->db->char_set . '');

            $this->db->query("ALTER TABLE `tblmy_customers_company_groups` ADD PRIMARY KEY (`id`);");
            $this->db->query("ALTER TABLE `tblmy_customers_company_groups` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT");
        }

        if (!$this->db->table_exists(db_prefix() . 'my_transactions')) {
            $this->db->query('CREATE TABLE `tblmy_transactions` (
              `id` int(11) NOT NULL,
            ...
            $this->db->query("ALTER TABLE `tblmy_transactions` ADD PRIMARY KEY (`id`);");
            $this->db->query("ALTER TABLE `tblmy_transactions` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT");
        }

        //Alter table tblcreditnotes
        if (!$this->db->field_exists('rel_sid', db_prefix().'creditnotes')) {
            $this->db->query("ALTER TABLE `tblcreditnotes` ADD `rel_sid` int(11) DEFAULT NULL;");
            $this->db->query("ALTER TABLE `tblcreditnotes` ADD KEY `rel_sid` (`rel_sid`)");
        }
        if (!$this->db->field_exists('rel_stype', db_prefix().'creditnotes')) {
            $this->db->query("ALTER TABLE `tblcreditnotes` ADD `rel_stype` varchar(20) DEFAULT NULL;");
            $this->db->query("ALTER TABLE `tblcreditnotes` ADD KEY `rel_stype` (`rel_stype`)");
        }
