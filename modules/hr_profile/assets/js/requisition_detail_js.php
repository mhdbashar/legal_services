<script>  
  (function(){
    ...
    $(window).on('load', function() {
      if($('input[name="has_send_mail"]').val() == 1){
        ...
        if(rel_type == "Leave"){
          $.post(admin_url+'hr_profile/send_notifi_handover_recipients', data_send_mail).done(function(response){
          });

          $.post(admin_url+'hr_profile/send_notification_recipient', data_send_mail).done(function(response){
          });
        }
      }
    });
  ...
  function view_isn_file(id, rel_id) {
    ...
    $("#asset_file_data").load(admin_url + 'hr_profile/file_view_requisition/' + rel_id +'/' + id, function(response, status, xhr) {
      if (status == "error") {
  ...
  function cancel_request(id){
    ...
    data.rel_id = id;
    data.rel_type = '<?php echo html_entity_decode($rel_type); ?>';
    $.post(admin_url + 'hr_profile/cancel_request', data).done(function(response){
      response = JSON.parse(response); 
      if (response.success === true || response.success == 'true') {
        alert_float('success', response.message);
        window.location.reload();
  ...
  function delete_requisition_attachment(wrapper, id) {
    "use strict";
    if (confirm_delete()) {
     $.get(admin_url + 'hr_profile/delete_timesheets_attachment_file/' + id, function (response) {
