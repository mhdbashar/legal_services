...
$invoice_info = '<br />' . _l('appointment_date_and_time') . ' ' . _d($irac->datecreated) . '<br />';
...
$pdf->writeHTML($tbltotal, true, false, false, false, '');
    ...
    $pdf->Cell(0, 0, _l('case_info'), 0, 1, 'L', 0, '', 0);
...
if (!empty($irac->facts)) {
    $pdf->Ln(4);
    $pdf->SetFont($font_name, 'B', $font_size);
    $pdf->Cell(0, 0, _l('facts'), 0, 1, 'L', 0, '', 0);
    $pdf->SetFont($font_name, '', $font_size);
    $pdf->Ln(2);
    $pdf->writeHTMLCell('', '', '', '', $irac->facts, 0, 1, false, true, 'L', true);
}

if (!empty($irac->legal_authority)) {
    $pdf->Ln(4);
    $pdf->SetFont($font_name, 'B', $font_size);
    $pdf->Cell(0, 0, _l('legal_authority'), 0, 1, 'L', 0, '', 0);
    $pdf->SetFont($font_name, '', $font_size);
    $pdf->Ln(2);
    $pdf->writeHTMLCell('', '', '', '', $irac->legal_authority, 0, 1, false, true, 'L', true);
}

if (!empty($irac->analysis)) {
    $pdf->Ln(4);
    $pdf->SetFont($font_name, 'B', $font_size);
    $pdf->Cell(0, 0, _l('analysis'), 0, 1, 'L', 0, '', 0);
    $pdf->SetFont($font_name, '', $font_size);
    $pdf->Ln(2);
    $pdf->writeHTMLCell('', '', '', '', $irac->analysis, 0, 1, false, true, 'L', true);
}

if (!empty($irac->result)) {
    $pdf->Ln(4);
    $pdf->SetFont($font_name, 'B', $font_size);
    $pdf->Cell(0, 0, _l('IRAC_result'), 0, 1, 'L', 0, '', 0);
    $pdf->SetFont($font_name, '', $font_size);
    $pdf->Ln(2);
    $pdf->writeHTMLCell('', '', '', '', $irac->result, 0, 1, false, true, 'L', true);
}
