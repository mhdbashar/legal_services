<script>
	$(function(){
		'use strict';
			...
			$('.article_useful_buttons button').on('click', function(e) {
				...
				$.post(admin_url+'hr/knowledge_base_q_a/add_kb_answer', data).done(function(response) {
	...
	function send_mail_support(obj){
	 ...
	 function view_hr_profile_q_a(id, rel_id) {  
	 	...
	 	$("#contract_file_data").load(admin_url + 'hr/preview_q_a_file/' + id + '/' + rel_id, function(response, status, xhr) {
	 		if (status == "error") {
