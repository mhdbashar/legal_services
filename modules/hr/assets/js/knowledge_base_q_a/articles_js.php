...
$(function () {
  ...
  $(".groups").sortable({
    ...
    update: function (event, ui) {
      if (this === ui.item.parent()[0]) {
        ...
        setTimeout(function () {
          $.post(admin_url + 'hr/knowledge_base_q_a/update_kan_ban', {
            order: order,
  ...
  $(".container-fluid").sortable({
    ...
    update: function (event, ui) {
      ...
      var data = {}
      data.order = order;
      $.post(admin_url + 'hr/knowledge_base_q_a/update_groups_order', data);
    }
  });
  // Status color change
  $('body').on('click', '.kb-kan-ban .cpicker', function () {
    ...
    $.post(admin_url + 'hr/knowledge_base_q_a/change_group_color', {
      color: color,
      group_id: group_id
    });
  });
   ...
   function staff_delete_bulk_action(event) {
    ...
    if (confirm_delete()) {
      ...
      if(mass_delete == true){
        ...
        setTimeout(function() {
          $.post(admin_url + 'hr/hrm_delete_bulk_action', data).done(function() {
