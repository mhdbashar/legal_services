...
foreach ($estimate_statuses as $status) {
    ...
    ?>
    <ul class="kan-ban-col" data-col-status-id="<?php echo $status; ?>" data-total-pages="<?php echo $total_pages; ?>" data-total="<?php echo $total_estimates; ?>">
        <li class="kan-ban-col-wrapper">
            <div class="border-right panel_s no-mbot">
                <div class="panel-heading-bg <?php echo estimate_status_color_class($status); ?>-bg estimate-status-pipeline-<?php echo estimate_status_color_class($status); ?>">
                    ...
                    <?php echo estimate_status_by_id($status); ?> - <?php echo $kanBan->countAll() . ' ' . _l('estimates') ?>
                </div>
                        ...
                        <ul class="sortable<?php if($has_permission_edit){echo ' status pipeline-status';} ?>" data-status-id="<?php echo $status; ?>">
                            ...
                            <?php if($total_estimates > 0 ){ ?>
                                <li class="text-center not-sortable kanban-load-more" data-load-status="<?php echo $status; ?>">
                                    <a href="#" class="btn btn-default btn-block<?php if($total_pages <= 1 || $kanBan->getPage() === $total_pages){echo ' disabled';} ?>" data-page="<?php echo $kanBan->getPage(); ?>" onclick="kanban_load_more(<?php echo $status; ?>,this,'estimates/pipeline_load_more',310,360); return false;";><?php echo _l('load_more'); ?></a>
                                </li>
                            <?php } ?>
