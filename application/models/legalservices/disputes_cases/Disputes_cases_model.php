...
class Disputes_cases_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->where(array('my_disputes_cases.id' => $id, 'my_disputes_cases.deleted' => 0));
            $this->db->select('my_disputes_cases.*,countries.short_name_ar as country_name, cat.name as cat, subcat.name as subcat,my_courts.court_name,my_judicialdept.Jud_number,my_customer_representative.representative as Representative,my_disputes_cases_statuses.status_name as StatusCase');
            //$this->db->select('*');
            ...
            if ($project) {
                ...
                // SYNC NEW TABS
                $tabs = get_disputes_case_tabs_admin();
                $tabs_flatten = [];
                    ...
                    foreach ($tabs_flatten as $tab) {
                        if (!in_array($tab, $settings_available_features)) {
                            if ($available_features_index) {
                                ...
                                $this->db->update(db_prefix() . 'my_disputes_case_settings', ['value' => serialize($tmp)]);
                            }
                        }
                    }
                }
                ...
                $this->db->where('case_id', $project->id);
                $this->db->select('my_disputes_cases_opponents.*,clients.userid , clients.company ');
                $this->db->join(db_prefix() . 'clients', db_prefix() . 'clients.userid=' . db_prefix() . 'my_disputes_cases_opponents.opponent_id');
                $this->db->from('my_disputes_cases_opponents');
                $project->opponent_data = $this->db->get()->result();

                $project->members_data = new StdClass();
                $this->db->where('project_id', $project->id);
                $this->db->select('*');
                $this->db->from('my_disputes_cases_members');
                $project->members_data = $this->db->get()->result();

//                $project->opponent_lawyer = new StdClass();
//                $project->opponent_lawyer = $this->clients_model->get($project->opponent_id);
        ...
        $this->db->where('my_disputes_cases.deleted', 0);
        $this->db->select('*,' . get_sql_select_client_company());
        $this->db->join(db_prefix() . 'clients', db_prefix() . 'clients.userid=' . db_prefix() . 'my_disputes_cases.clientid');
        $this->db->order_by('my_disputes_cases.id', 'desc');
        return $this->db->get(db_prefix() . 'my_disputes_cases')->result_array();
    }
    ...
    public function GetMembersCases($id)
    {
        ...
        $this->db->from('my_disputes_cases_members');
        $this->db->join('my_disputes_cases', 'my_disputes_cases.id = my_disputes_cases_members.project_id');
        $this->db->join('staff', 'staff.staffid = my_disputes_cases_members.staff_id');
        $this->db->where('my_disputes_cases.id', $id);
    ...
    public function GetJudgesCases($id)
    {
        ...
        $this->db->from('my_disputes_cases_judges');
        $this->db->join('my_judges', 'my_judges.id = my_disputes_cases_judges.judge_id');
        $this->db->where('my_disputes_cases_judges.case_id', $id);
        return $this->db->get()->result();
    }
    ...
    public function add($ServID,$data)
    {
        ...
        $this->db->insert(db_prefix() . 'my_disputes_cases', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {

            if (isset($opponents)) {
                $cases_opponents['opponents'] = $opponents;
                $this->add_edit_opponents($cases_opponents, $insert_id);
                $this->Dmovement->add_edit_opponents_movement($cases_opponents, $insert_id);
            ...
            if (isset($project_members)) {
                ...
                $this->Dmovement->add_edit_members_movement($_pm, $insert_id);
            }

            if (isset($judges)) {
                $cases_judges['judges'] = $judges;
                $this->add_edit_judges($cases_judges, $insert_id);
                $this->Dmovement->add_edit_judges_movement($cases_judges, $insert_id);
            }
            ...
            $this->Dmovement->add($ServID, $insert_id, $movement_data);
            ...
            if (isset($case_settings)) {
                ...
                foreach ($original_settings as $setting) {
                    ...
                    $this->db->insert(db_prefix() . 'my_disputes_case_settings', [
                        'case_id'    => $insert_id,
                        'name'       => $setting,
                        'value'      => $value_setting,
                    ]);
    ...
    public function update($ServID,$id,$data)
    {
        ...
        if (!isset($data['settings'])) {
            ...
            $this->db->update(db_prefix() . 'my_disputes_case_settings', [
                'value' => 0,
            ]);
        ...
        if ($this->add_edit_opponents($cases_opponents, $id)) {
            $affectedRows++;
        }
    ...
    public function delete($ServID,$id)
    {
        ...
        $this->db->delete(db_prefix() . 'my_disputes_cases');
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('project_id', $id);
            $this->db->delete(db_prefix() . 'my_disputes_cases_members');
...
//
            ...
            $this->db->where('case_id', $id);
            $this->db->delete(db_prefix() . 'my_disputes_cases_judges');
            ...
            $this->db->where('project_id', $id);
            $discussions = $this->db->get(db_prefix() . 'my_disputes_casediscussions')->result_array();
            foreach ($discussions as $discussion) {
                ...
                $this->db->delete(db_prefix() . 'my_disputes_casediscussioncomments');
            }
            $this->db->where('project_id', $id);
            $this->db->delete(db_prefix() . 'my_disputes_casediscussions');
...
//            $this->db->delete(db_prefix() . 'invoices');
    ...
    public function move_to_recycle_bin($ServID,$id)
    {
        ...
        $this->db->update(db_prefix() . 'my_disputes_cases');
...
//            ]);
        ...
        if (sizeof($project_members_in) > 0) {
            ...
            if (isset($project_members)) {
                ...
                foreach ($project_members as $staff_id) {
                    $this->db->where('project_id', $id);
                    $this->db->where('staff_id', $staff_id);
                    $_exists = $this->db->get(db_prefix() . 'my_disputes_cases_members')->row();
                    if (!$_exists) {
                        if (empty($staff_id)) {
                            continue;
                        }
        ...
        if (sizeof($cases_judges_in) > 0) {
            ...
            if (isset($cases_judges)) {
                foreach ($cases_judges as $judge_id) {
                    ...
                    $_exists = $this->db->get(db_prefix() . 'my_disputes_cases_judges')->row();
                    if (!$_exists) {
                        if (empty($judge_id)) {
                            continue;
                        }
    ...
    public function add_edit_opponents($data, $id)
    {
        $affectedRows = 0;
        if (isset($data['opponents'])) {
            $cases_opponents = $data['opponents'];
        }
        $cases_opponents_in = $this->get_case_opponents($id);
        ...
        if (sizeof($cases_opponents_in) > 0) {
            ...
            if (isset($cases_opponents)) {
                foreach ($cases_opponents as $opponent_id) {
                    ...
                    $_exists = $this->db->get(db_prefix() . 'my_disputes_cases_opponents')->row();
                    if (!$_exists) {
    ...
    {
        $this->db->select('my_disputes_cases_judges.*,my_judges.*');
        $this->db->join(db_prefix() . 'my_judges', db_prefix() . 'my_judges.id=' . db_prefix() . 'my_disputes_cases_judges.judge_id');
        $this->db->where('my_disputes_cases_judges.case_id', $id);
        return $this->db->get(db_prefix() . 'my_disputes_cases_judges')->result_array();
    }

    public function get_case_opponents($id)
    {
        $this->db->select('my_disputes_cases_opponents.*,clients.*');
        $this->db->join(db_prefix() . 'clients', db_prefix() . 'clients.userid=' . db_prefix() . 'my_disputes_cases_opponents.opponent_id');
        $this->db->where('my_disputes_cases_opponents.case_id', $id);
        return $this->db->get(db_prefix() . 'my_disputes_cases_opponents')->result_array();
    }
    ...
    {
        return $this->db->query('SELECT staff_id, firstname, lastname FROM ' . db_prefix() . 'my_disputes_cases_members JOIN ' . db_prefix() . 'staff ON ' . db_prefix() . 'staff.staffid=' . db_prefix() . 'my_disputes_cases_members.staff_id GROUP by staff_id order by firstname ASC')->result_array();
    }

    public function get_most_used_billing_type()
    {
        return $this->db->query('SELECT billing_type, COUNT(*) AS total_usage
                FROM ' . db_prefix() . 'my_disputes_cases
    ...
    public function get_last_case_settings()
    {
        ...
        $this->db->order_by('id', 'DESC');
        $this->db->limit(1);
        $last_project = $this->db->get(db_prefix() . 'my_disputes_cases')->row();
        if ($last_project) {
    ...
    public function get_CaseSession($id, $where = [], $apply_restrictions = false, $count = false, $ServID = 22)
    {
        $slug = $this->legal->get_service_by_id($ServID)->row()->slug;
    ...
    public function do_milestones_kanban_query($milestone_id, $project_id, $page = 1, $where = [], $count = false)
    {
        ...
        $tasks              = $this->get_tasks($project_id, $where, true, $count,$ServID = 22, function () use ($count, $page, $limit) {
            if ($count == false) {
                if ($page > 1) {
    ...
    public function get_files($project_id)
    {
        ...
        return $this->db->get(db_prefix() . 'my_disputes_case_files')->result_array();
    }
    ...
    public function update_file_data($data)
    {
        ...
        unset($data['id']);
        $this->db->update(db_prefix() . 'my_disputes_case_files', $data);
    }

    public function change_file_visibility($id, $visible)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'my_disputes_case_files', [
    ...
    public function remove_file($id, $logActivity = true)
    {
        ...
        if ($file) {
            ...
            $this->db->where('id', $id);
            $this->db->delete(db_prefix() . 'my_disputes_case_files');
            if ($logActivity) {
    ...
    public function get_all_projects_gantt_data($filters = [])
    {
        ...
        foreach ($statuses as $status) {
            ...
            if (!has_permission('projects', '', 'view')) {
                $this->db->where(db_prefix() . 'my_disputes_cases.id IN (SELECT project_id FROM ' . db_prefix() . 'my_disputes_cases_members WHERE staff_id=' . get_staff_user_id() . ')');
            }

            if ($filters['member']) {
                $this->db->where(db_prefix() . 'my_disputes_cases.id IN (SELECT project_id FROM ' . db_prefix() . 'my_disputes_cases_members WHERE staff_id=' . $filters['member'] . ')');
            }

            $this->db->where('status', $status['id']);
            $this->db->order_by('deadline IS NULL ASC, deadline', '', false);
            $projects = $this->db->get(db_prefix() . 'my_disputes_cases')->result_array();
    ...
    public function mark_as($ServID, $data, $slug)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($data['status_id'] == 4) {
                ...
                if ($old_status == 4) {
                    $this->db->update(db_prefix() . 'my_disputes_cases', ['date_finished' => null]);
                }
            }
    ...
    public function is_member($project_id, $staff_id = '')
    {
        ...
        $member = total_rows(db_prefix() . 'my_disputes_cases_members', [
            'staff_id'   => $staff_id,
    ...
    public function get_case_settings($project_id)
    {
        ...
        return $this->db->get(db_prefix() . 'my_disputes_case_settings')->result_array();
    }
    ...
    public function get_discussion($id, $project_id = '')
    {
        ...
        if (is_client_logged_in()) {
            ...
            $this->db->where('project_id IN (SELECT id FROM ' . db_prefix() . 'my_disputes_cases WHERE clientid=' . get_client_user_id() . ')');
        }
        $discussion = $this->db->get(db_prefix() . 'my_disputes_casediscussions')->row();
        if ($discussion) {
            return $discussion;
        }
    ...
    {
        $this->db->where('id', $id);
        $comment = $this->db->get(db_prefix() . 'my_disputes_casediscussioncomments')->row();
        ...
        if (!is_null($comment->file_name)) {
            $comment->file_url = site_url('uploads/discussions/disputes_cases/' . $comment->discussion_id . '/' . $comment->file_name);
        }
    ...
    public function get_discussion_comments($id, $type)
    {
        ...
        $comments             = $this->db->get(db_prefix() . 'my_disputes_casediscussioncomments')->result_array();
        $i                    = 0;
        $allCommentsIDS       = [];
        ...
        foreach ($comments as $comment) {
            ...
            if (!is_null($comment['file_name'])) {
                $comments[$i]['file_url'] = site_url('uploads/discussions/disputes_cases/' . $id . '/' . $comment['file_name']);
            }
    ...
    public function get_discussions($project_id)
    {
        ...
        $i           = 0;
        foreach ($discussions as $discussion) {
            $discussions[$i]['total_comments'] = total_rows(db_prefix() . 'my_disputes_casediscussioncomments', [
    ...
    public function add_discussion_comment($ServID = '', $data, $discussion_id, $type)
    {
        ...
        $this->db->insert(db_prefix() . 'my_disputes_casediscussioncomments', $_data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            if ($type == 'regular') {
                $discussion = $this->get_discussion($discussion_id);
                $not_link   = 'Disputes_cases/view/' .$ServID.'/'. $discussion->project_id . '?group=project_discussions&discussion_id=' . $discussion_id;
            } else {
                $discussion                   = $this->get_file($discussion_id);
                $not_link                     = 'Disputes_cases/view/' .$ServID.'/'. $discussion->project_id . '?group=project_files&file_id=' . $discussion_id;
    ...
    public function update_discussion_comment($data)
    {
        ...
        $this->db->where('id', $data['id']);
        $this->db->update(db_prefix() . 'my_disputes_casediscussioncomments', [
    ...
    public function add_discussion($data, $ServID)
    {
        ...
        $data['description'] = nl2br($data['description']);
        $this->db->insert(db_prefix() . 'my_disputes_casediscussions', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            $members           = $this->get_project_members($data['project_id']);
            $notification_data = [
                'description' => 'not_created_new_project_discussion',
                'link'        => 'Disputes_cases/view/'. $ServID.'/'. $data['project_id'] . '?group=project_discussions&discussion_id=' . $insert_id,
            ];
    ...
    public function edit_discussion($data, $id)
    {
        ...
        $data['description'] = nl2br($data['description']);
        $this->db->update(db_prefix() . 'my_disputes_casediscussions', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function copy($ServID,$project_id, $data)
    {
        ...
        $_new_data = [];
        $fields    = $this->db->list_fields(db_prefix() . 'my_disputes_cases');
        ...
        $this->db->insert(db_prefix() . 'my_disputes_cases', $_new_data);
        ...
        if ($id) {
            if(isset($data['files'])){
                ...
                foreach ($files as $key => $value) {
                    $file_url = base_url().'uploads/disputes_cases/'.$project_id.'/'.$value['file_name'];
            ...
            if (isset($data['milestones'])) {
                ...
                foreach ($milestones as $milestone) {
                    ...
                    $milestone_data = $milestone_data = $ServID2 != 22 ? [
                        'name' => $milestone['name'],
                        'rel_sid' => $id,
                        'rel_stype' => $slug,
                        'milestone_order' => $milestone['milestone_order'],
    ...
    public function get_activity($id = '', $limit = '', $only_project_members_activity = false)
    {
        if (!is_client_logged_in()) {
            ...
            if (!$has_permission) {
                $this->db->where('project_id IN (SELECT project_id FROM ' . db_prefix() . 'my_disputes_cases_members WHERE staff_id=' . get_staff_user_id() . ')');
            }
        }
        if (is_client_logged_in()) {
            $this->db->where('visible_to_customer', 1);
        ...
        foreach ($activities as $activity) {
            ...
            $activities[$i]['project_name']    = get_disputes_case_name_by_id($activity['project_id']);
    ...
    public function new_project_file_notification($ServID = '', $file_id, $project_id)
    {
        ...
        $notification_data = [
            ...
            'link'        => 'Disputes_cases/view/' .$ServID. '/' . $project_id . '?group=project_files&file_id=' . $file_id,
        ];
    ...
    public function add_external_file($data)
    {
        ...
        $this->db->insert(db_prefix() . 'my_disputes_case_files', $insert);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    private function _get_project_billing_data($id)
    {
        ...
        return $this->db->get(db_prefix() . 'my_disputes_cases')->row();
    }
    ...
    {
        if ($type == 'file') {
            $table = db_prefix() . 'my_disputes_case_files';
        } elseif ($type == 'regular') {
            $table = db_prefix() . 'my_disputes_casediscussions';
        }
        $this->db->where('id', $id);
    ...
    public function get_all_statuses()
    {
        ...
        return $this->db->get(db_prefix() .'my_disputes_cases_statuses')->result();
    }

    public function get_status_by_id($id)
    {
        return $this->db->get_where(db_prefix() .'my_disputes_cases_statuses', array('id' => $id))->row();
    }

    public function add_new_case_status($data)
    {
        $this->db->insert(db_prefix() .'my_disputes_cases_statuses', $data);
        $insert_id = $this->db->insert_id();
        return $insert_id;
    }

    public function update_case_status($id,$data)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() .'my_disputes_cases_statuses', $data);
        //if ($this->db->affected_rows() > 0) {
        return true;
        //}
        //return false;
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() .'my_disputes_cases_statuses');
        if ($this->db->affected_rows() > 0) {
            return true;
        }
        return false;
