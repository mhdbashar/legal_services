...
if(!is_admin() && !has_permission('leave_management','','view')){
    array_push($where, ' AND ('.get_staff_user_id() .' in (select staffid from '.db_prefix().'timesheets_approval_details where rel_type = "Leave" and rel_id = '.db_prefix().'timesheets_requisition_leave.id) or '.db_prefix().'timesheets_requisition_leave.staff_id = '.get_staff_user_id().')');
}
...
foreach ($rResult as $aRow) {
    ...
    $row[] = '<div class="row">'
    .'<a class="col-md-10" href="'.admin_url('hr_profile/requisition_detail'. '/' . ($aRow[db_prefix().'timesheets_requisition_leave.id']) ).'">' . $aRow['subject'] . '</a>'
    .'</div>';
...
if(is_admin() || $aRow['status'] == 0){                     
    $action_option .='<a id="delete-insurance" data-placement="top" data-toggle="tooltip" data-title="'._l('delete').'" href="'.admin_url('hr_profile/delete_requisition'. '/' . ($aRow[db_prefix().'timesheets_requisition_leave.id']) ).'" class="btn btn-danger btn-icon _delete">' . '<i class="fa fa-remove"></i>' . '</a>';
}
