            ...
            if (!empty($username)) {
               ...
               $login_data = [];
                ...
                if (!empty($password)) {
                        ...
                        $login = $this->Auth_model->login($username, $password, 0, true);
                        if ($login['staff_logged_in'] == 1 ) {
                        $token_number=$this->Auth_model->setToken($login['client_user_id']->staffid,$tokenid,'staff',$tokenkey);  
                            $login_data[] = $login['client_user_id'];
                            $this->db->select('*')->from('tbloptions')->where('name','company_logo');
                            $company_logo= $this->db->get()->result();
                            $data =  [
