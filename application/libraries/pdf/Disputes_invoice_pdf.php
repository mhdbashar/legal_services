...
class Disputes_invoice_pdf extends App_pdf
{
    protected $invoice;
    ...
    public function __construct($invoice, $tag = '')
    {
        ...
        if (!class_exists('Disputes_invoices_model', false)) {
            $this->ci->load->model('legalservices/disputes_cases/Disputes_invoices_model','invoices_model');
        }

        $this->tag            = $tag;
        $this->invoice        = $invoice;
        $this->invoice_number = disputes_format_invoice_number($this->invoice->id);
    ...
    {
        $customPath = APPPATH . 'views/themes/' . active_clients_theme() . '/views/disputes_invoicepdf.php';
