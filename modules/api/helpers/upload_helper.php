 ...
 * @return boolean
 */
function handle_project_file_uploads($project_id,$staff_id,$contact_id)
{
        ...
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    ...
                    $CI->db->insert('tblproject_files', $data);
 ...
 * @return array  - Result values
 */
function handle_client_attachments_upload($id,$staff_id, $customer_upload = false)
{
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    ...
                    if ($customer_upload == true) {
                        ...
                        $attachment[0]['contact_id']       = $staff_id;
                        $attachment['visible_to_customer'] = 1;
                    }else{
                        $attachment[0]['staffid']          = $staff_id;
            ...
            if ($i <= get_option('maximum_allowed_ticket_attachments')) {
                ...
                if (!empty($tmpFilePath) && $tmpFilePath != '') {
                    ...
                    $extension = strtolower(pathinfo($_FILES[$index_name]['name'][$i], PATHINFO_EXTENSION));
                      // echo "<pre>";print_r($tmpFilePath);echo "</pre>"; die();
 ...
 * @return string
 */
function get_leads_summary_from_helper($staff_id)
{
    $CI = &get_instance();
    ...
    $has_permission_view   = has_permission('leads', $staff_id, 'view');
    $sql                   = '';
    $whereNoViewPermission = '(addedfrom = ' . $staff_id . ' OR assigned=' . $staff_id . ' OR is_public = 1)';
