...
if(!empty(get_dialog_boxes())):
foreach (get_dialog_boxes() as $row):
    $url = '/'.$row['page_url'];
    //if($_SERVER['REDIRECT_QUERY_STRING'] === $url):
    if (strpos($_SERVER['REDIRECT_QUERY_STRING'], $url) !== false): ?>
        <a href="#" class="tooltip_pulse" date-html="true" data-toggle="popover" title="<?php echo $row['title']; ?>" data-content="<?php echo $staff_language == 'arabic' ? $row['desc_ar'] : $row['desc_en']; ?>"><div class=" pulse"><i class="fa fa-question-circle" aria-hidden="true"></i></div></a>
