...
$where = ['AND name="technical_competencies_type"'];
...
foreach (json_decode($values) as $value) {
	...
	foreach ($rResult as $aRow) {
        ...
        if (has_permission('hr_settings', '', 'delete')) $options .= icon_btn('hr/setting/delete_type/' . $a .'/technical_competencies_type', 'remove', 'btn-danger _delete');
        $row[]   = $options;
