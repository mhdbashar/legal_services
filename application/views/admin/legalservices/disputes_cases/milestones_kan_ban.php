  ...
  <div class="kan-ban-content">
   <ul class="status case-milestone milestone-tasks-wrapper sortable relative" data-task-status-id="<?php echo $milestone['id']; ?>">
    ...
    foreach ($tasks as $task) {
     $this->load->view('admin/legalservices/disputes_cases/_milestone_kanban_card', array('task'=>$task, 'milestone'=>$milestone['id']));
   } ?>
   <?php if ($total_tasks > 0) { ?>
     <li class="text-center not-sortable kanban-load-more" data-load-status="<?php echo $milestone['id']; ?>">
       <a href="#" class="btn btn-default btn-block<?php if ($total_pages <= 1) { echo ' disabled'; } ?>" data-page="1" onclick="kanban_load_more(<?php echo $milestone['id']; ?>,this,'legalservices/disputes_cases/milestones_kanban_load_more',320,360); return false;";>
