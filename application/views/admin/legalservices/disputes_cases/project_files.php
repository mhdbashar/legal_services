...
<?php echo form_open_multipart(admin_url('legalservices/disputes_cases/upload_file/'.$ServID.'/'.$project->id),array('class'=>'dropzone','id'=>'project-files-upload')); ?>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a href="#" class="btn btn-info" onclick="case_files_bulk_action(this); return false;"><?php echo _l('confirm'); ?></a>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<a href="#" onclick="window.location.href = '<?php echo admin_url('legalservices/disputes_cases/download_all_files/'.$ServID.'/'.$project->id); ?>'; return false;" class="table-btn hide" data-table=".table-case-files"><?php echo _l('download_all'); ?></a>
<div class="clearfix"></div>
<table class="table dt-table table-case-files" data-order-col="7" data-order-type="desc">
  ...
  <tbody>
    <?php foreach($files as $file){
      $path = get_upload_path_by_type_case('disputes_case') . $project->id . '/'. $file['file_name'];
      ?>
      <tr>
        <td>
        ...
        <td data-order="<?php echo $file['file_name']; ?>">
          <a href="#" onclick="view_case_file(<?php echo $file['id']; ?>,<?php echo $file['project_id']; ?>); return false;">
            <?php if(is_image('uploads/disputes_cases/' .$project->id.'/'.$file['file_name']) || (!empty($file['external']) && !empty($file['thumbnail_link']))){
              echo '<div class="text-left"><i class="fa fa-spinner fa-spin mtop30"></i></div>';
              echo '<img class="project-file-image img-table-loading" src="#" data-orig="'.case_file_url($file,true).'" width="100">';
              echo '</div>';
            }
            echo $file['subject']; ?></a>
          </td>
          ...
          <?php $total_file_comments = total_rows(db_prefix().'my_disputes_casediscussioncomments',array('discussion_id'=>$file['id'],'discussion_type'=>'file')); ?>
          ...
          <td data-order="<?php echo $file['visible_to_customer']; ?>">
            ...
            ?>
            <div class="onoffswitch">
              <input type="checkbox" data-switch-url="<?php echo admin_url(); ?>legalservices/disputes_cases/change_file_visibility" id="<?php echo $file['id']; ?>" data-id="<?php echo $file['id']; ?>" class="onoffswitch-checkbox" value="<?php echo $file['id']; ?>" <?php echo $checked; ?>>
         ...
         <td>
           ...
           <button type="button" data-toggle="modal" data-original-file-name="<?php echo $file['file_name']; ?>" data-filetype="<?php echo $file['filetype']; ?>" data-path="<?php echo 'uploads/disputes_cases/' .$project->id.'/'.$file['file_name']; ?>" data-target="#send_file" class="btn btn-info btn-icon"><i class="fa fa-envelope"></i></button>
           <?php } ?>
           <?php if($file['staffid'] == get_staff_user_id() || has_permission('projects','','delete')){ ?>
           <a href="<?php echo admin_url('legalservices/disputes_cases/remove_file/'.$ServID.'/'.$project->id.'/'.$file['id']); ?>" class="btn btn-danger btn-icon _delete"><i class="fa fa-remove"></i></a>
           <?php } ?>
         </td>
       </tr>
       <?php } ?>
