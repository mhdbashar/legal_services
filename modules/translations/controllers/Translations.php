...
class Translations extends AdminController
{
    ...
    public function pull_from_files(){
        ...
        $this->scan_and_insert_files($selected_lang, APPPATH . $selected_lang_folder);
        //SCAN AND ADD FROM MODULE FOLDERS
        foreach ($modules as $module){
            $folder = APP_MODULES_PATH . $module['module_name'] . DIRECTORY_SEPARATOR . $selected_lang_folder;
            $this->scan_and_insert_files($selected_lang, $folder, $module['id']);
        }
    ...
    private function scan_and_insert_files($selected_lang, $folder, $module_id = null){
        if(is_dir($folder)){
            $files = scandir($folder);
            foreach ($files as $file) {
                if(in_array($file, [".", ".."]))
