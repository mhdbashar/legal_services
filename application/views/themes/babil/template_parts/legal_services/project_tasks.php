...
<?php if ($project->settings->view_tasks == 1) { ?>
   ...
   <?php if ($project->settings->view_milestones == 1 && !isset($view_task)) { ?>
      ...
      <div class="tasks-phases">
         ...
         $this->load->model('legalservices/Tasks_model', 'Tasks');
         if ($ServID == 1) {
                        ...
                        <?php if(!is_session($task['id'])) : ?>
                           <div class="media _task_wrapper<?php if ((!empty($task['duedate']) && $task['duedate'] < date('Y-m-d')) && $task['status'] != Tasks_model::STATUS_COMPLETE) {
                              ...
                              <div class="media-body">
                                 <a href="<?php echo site_url('clients/legal_services/' . $project->id . '/' . $ServID . '?group=project_tasks&taskid=' . $task['id']); ?>" class="task_milestone pull-left<?php if ($task['status'] == Tasks_model::STATUS_COMPLETE) {
