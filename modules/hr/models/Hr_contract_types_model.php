...
class Hr_contract_types_model extends App_Model
{
    ...
    {
        $this->db->insert(db_prefix().'hr_contracts_types', $data);
    ...
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix().'hr_contracts_types', $data);
    ...
    public function get($id = '')
    {
        if (is_numeric($id)) {
            ...
            return $this->db->get(db_prefix().'hr_contracts_types')->row();
        }

        $types = $this->app_object_cache->get('contract-types');

        if (!$types && !is_array($types)) {
            $types = $this->db->get(db_prefix().'hr_contracts_types')->result_array();
    ...
    public function delete($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix().'hr_contracts_types');
        if ($this->db->affected_rows() > 0) {
