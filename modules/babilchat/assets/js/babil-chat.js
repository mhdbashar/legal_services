...
function changeColor(obj)
{
    ...
    $.post(url, {
        ...
        if (r.success === "unknownColor") {
            alert_float("warning", babilchatSettings.invalidColor);
            return false;
        }
...
{
    if (confirm(babilchatSettings.areYouSure)) {
        $.post(babilchatSettings.resetChatColors).done(function (r) {
            if (r == "true") {
                location.reload();
            }
        });
...
function chatMemberUpdate(subscribed_event)
{
    ...
    $.get(babilchatSettings.usersList, function (data) {
        var offlineUser = "";
        var onlineUser = "";
        ...
        $.each(data, function (user_id, value) {
            if (value.staffid != presenceChannel.members.me.id) {
                ...
                if (user != null) {
                    ...
                    offlineUser += " data-toggle=\"tooltip\" title=\"" + babilchatSettings.chatLastSeenText + ": " + lastLoginText + "\">";
        ...
        if (subscribed_event === true) {
            if (babilchatSettings.getUnread != null) {
                var parsedUnreadMessages = JSON.parse(babilchatSettings.getUnread);
...
function babilchat_setNoMoreMessages(to)
{
    if ($("#no_messages_" + to).length == 0) {
        $(".logMsg#id_" + to).prepend("<div class=\"text-center\" style=\"margin-top:5px;\" id=\"no_messages_" + to + "\">" + babilchatSettings.noMoreMessagesText + "</div>");
    }
}

/*---------------* Sound functions *---------------*/
var getSound = new Audio(site_url + "modules/babilchat/assets/chat_implements/sounds/push.mp3");
...
const deleteUrl = admin_url + "babilchat/Babilchat_Controller/purgeConversations";
...
{
    let confirmation = prompt(babilchatSettings.typeInToConfirm, "");

    confirmation = confirmation === "DELETE";

    if (!confirmation) alert(babilchatSettings.actionCancelled);
...
{
    return confirm(babilchatSettings.areYouSure);
}
...
function deleteChatConversation(type)
{
    $.post(deleteUrl, {type: type}).done(function (r) {
        ...
        if (true === r.success) alert_float("success", babilchatSettings.conversationDeleted);
        if (false === r.success) alert_float("info", babilchatSettings.conversationAlreadyDeleted);
