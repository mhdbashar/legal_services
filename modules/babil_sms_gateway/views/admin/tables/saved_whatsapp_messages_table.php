...
$join = [
    'LEFT JOIN '.db_prefix().'staff ON '.db_prefix().'staff.staffid='.db_prefix().'saved_wa.staff_id',
    'LEFT JOIN '.db_prefix().'my_basic_services ON '.db_prefix().'my_basic_services.slug='.db_prefix().'saved_wa.rel_type'
];


$result  = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix().'saved_wa.id', db_prefix() . 'my_basic_services.id as ServID', 'rel_id', 'msg_id']);
...
foreach ($rResult as $aRow) {
    ...
    //if (has_permission('hr', '', 'delete'))
        $options .= icon_btn('babil_sms_gateway/whatsapp/delete/' . $aRow['id'], 'remove', 'btn-danger _delete');
    $row[]   = $options;
