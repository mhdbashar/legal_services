...
class Disputes_invoices extends AdminController
{
    private $ci;
...
//        $data['bodyclass']            = 'invoices-total-manual';
    ...
    public function recurring($id = '')
    {
        ...
        $data['invoices_years']       = $this->invoices->get_invoices_years();
        $data['invoices_sale_agents'] = $this->invoices->get_sale_agents();
        $this->load->view('admin/legalservices/disputes_cases/invoices/recurring/list', $data);
    }
    ...
    public function disputes_get_table_data($table, $params = [])
    {
        ...
        $path = APPPATH.'../application/views/admin/legalservices/disputes_cases/invoices/' . $table . EXT;
    ...
    public function client_change_data($customer_id, $current_invoice = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($current_invoice != '') {
                ...
                $current_invoice_status = $this->db->get(db_prefix() . 'my_disputes_cases_invoices')->row()->status;
            }

            $_data['invoices_to_merge'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != 5) ? $this->invoices->check_for_merge_invoice($customer_id, $current_invoice) : [];

            $data['merge_info'] = $this->load->view('admin/legalservices/disputes_cases/invoices/merge_invoice', $_data, true);

            $this->load->model('currencies_model');

            $__data['expenses_to_bill'] = !isset($current_invoice_status) || (isset($current_invoice_status) && $current_invoice_status != 5) ? $this->invoices->get_expenses_to_bill($customer_id) : [];

            $data['expenses_bill_info'] = $this->load->view('admin/legalservices/disputes_cases/invoices/bill_expenses', $__data, true);
    ...
    public function pause_overdue_reminders($id)
    {
        if (has_permission('invoices', '', 'edit')) {
            $this->db->where('id', $id);
            $this->db->update(db_prefix() . 'my_disputes_cases_invoices', ['cancel_overdue_reminders' => 1]);
        }
    ...
    public function resume_overdue_reminders($id)
    {
        if (has_permission('invoices', '', 'edit')) {
            $this->db->where('id', $id);
            $this->db->update(db_prefix() . 'my_disputes_cases_invoices', ['cancel_overdue_reminders' => 0]);
        }
        redirect(admin_url('legalservices/disputes_invoices/list_invoices/' . $id));
    }
    ...
    public function mark_as_cancelled($id)
    {
        ...
        $success = $this->invoices->mark_as_cancelled($id);
    ...
    public function unmark_as_cancelled($id)
    {
        ...
        $success = $this->invoices->unmark_as_cancelled($id);
        if ($success) {
    ...
    public function copy($id)
    {
        ...
        $new_id = $this->invoices->copy($id);
        if ($new_id) {
            set_alert('success', _l('invoice_copy_success'));
            redirect(admin_url('legalservices/disputes_invoices/invoice/' . $new_id));
        } else {
            set_alert('success', _l('invoice_copy_fail'));
        }
        redirect(admin_url('legalservices/disputes_invoices/invoice/' . $id));
    }
...
//        }
        $opponents = get_disputes_cases_opponents_by_case_id($invoice->project_id);
        $data['opponents'] = [];
        foreach ($opponents as $opponent) {
...
//        }
        $this->db->where('case_id', $invoice->project_id);
        $data['selected_opponents'] = $this->db->get('tblmy_disputes_cases_opponents')->result_array();
        $this->load->view('admin/legalservices/disputes_cases/invoices/invoice', $data);
    }
...
//        $data['payments']                   = $this->payments->get_invoice_payments($id);
        ...
        $this->load->view('admin/legalservices/disputes_cases/invoices/invoice_preview_template', $data);
    }
    ...
    public function apply_credits($invoice_id)
    {
        ...
        redirect(admin_url('legalservices/disputes_invoices/list_invoices/' . $invoice_id));
    }
    ...
    public function record_payment()
    {
        ...
        if ($this->input->post()) {
            ...
            redirect(admin_url('legalservices/disputes_invoices/list_invoices/' . $this->input->post('invoiceid')));
        }
    }
    ...
    public function send_to_email($id)
    {
        ...
        try {
            $success = $this->invoices->send_invoice_to_client($id, '', $this->input->post('attach_pdf'), $this->input->post('cc'));
        } catch (Exception $e) {
    ...
    public function delete_payment($id, $invoiceid)
    {
        ...
        redirect(admin_url('legalservices/disputes_invoices/list_invoices/' . $invoiceid));
    }
    ...
    public function delete($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('legalservices/disputes_invoices/list_invoices'));
        }
        $success = $this->invoices->delete($id);
    ...
    public function delete_attachment($id)
    {
        ...
        if ($file->staffid == get_staff_user_id() || is_admin()) {
            echo $this->invoices->delete_attachment($id);
        } else {
    ...
    public function send_overdue_notice($id)
    {
        ...
        $send = $this->invoices->send_invoice_overdue_notice($id);
        if ($send) {
...
//        echo '<pre>';echo print_r($invoice);exit();
    ...
    public function mark_as_sent($id)
    {
        ...
        $success = $this->invoices->set_invoice_sent($id, true);
        if ($success) {
