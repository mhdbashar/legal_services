    ...
    <div class="col-md-12 text-right">
        <div class="form-group pull-right">
            <select class="selectpicker" name="gantt_type" onchange="gantt_filter();">
                <option value="milestones"<?php if(!$this->input->get('gantt_type') || $this->input->get('gantt_type') == 'milestones'){echo ' selected';} ?>><?php echo _l('oservice_milestones'); ?></option>
                <option value="members"<?php if($this->input->get('gantt_type') == 'members'){echo ' selected';} ?>>
                    <?php
                    if(has_permission('tasks','','view') || (!has_permission('tasks','','view') && get_option('show_all_tasks_for_oservice_member') == 1)){
                        echo _l('oservice_members');
                    } else {
