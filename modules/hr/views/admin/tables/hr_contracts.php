...
$aColumns = [
    db_prefix() . 'hr_contracts.id as id',
    'subject',
...
$sIndexColumn = 'id';
$sTable       = db_prefix() . 'hr_contracts';

$join = [
    'LEFT JOIN ' . db_prefix() . 'staff ON ' . db_prefix() . 'staff.staffid = ' . db_prefix() . 'hr_contracts.client',
    'LEFT JOIN ' . db_prefix() . 'projects ON ' . db_prefix() . 'projects.id = ' . db_prefix() . 'hr_contracts.project_id',
    'LEFT JOIN ' . db_prefix() . 'contracts_types ON ' . db_prefix() . 'contracts_types.id = ' . db_prefix() . 'hr_contracts.contract_type',
];

$custom_fields = get_table_custom_fields('hr_contracts');
...
foreach ($custom_fields as $key => $field) {
    ...
    array_push($join, 'LEFT JOIN ' . db_prefix() . 'customfieldsvalues as ctable_' . $key . ' ON ' . db_prefix() . 'hr_contracts.id = ctable_' . $key . '.relid AND ctable_' . $key . '.fieldto="' . $field['fieldto'] . '" AND ctable_' . $key . '.fieldid=' . $field['id']);
}
...
if ($projectId) {
    array_push($where, 'AND staffid=' . $this->ci->db->escape_str($projectId));
}
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'hr_contracts.id', 'trash', 'client', 'hash', 'marked_as_signed', 'project_id', 'staffid']);
...
foreach ($rResult as $aRow) {
    ...
    $subjectOutput = '<a href="' . admin_url('hr/contracts/contract/' . $aRow['id']) . '"' . ($projectId ? ' target="_blank"' : '') . '>' . $aRow['subject'] . '</a>';
//    if ($aRow['trash'] == 1) {
...
//    }
    ...
    $row[] = '<a href="' . admin_url('hr/general/general/' . $aRow['staffid']) . '?group=basic_information">' . $aRow['firstname'] . '</a>';
