    ...
    public function add_expense()
    {
        if ($this->input->post()) {
            ...
            if ($id) {
                ...
                echo json_encode([
                    'url'       => admin_url('disputes/view/' . $this->input->post('project_id') . '/?group=project_expenses'),
                    'expenseid' => $id,
                ]);
                die;
            }
...
/*
    public function printallcontacts($project_id=0)
    {
...
*/
    public function project($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                $id = $this->Disputes_model->add($data);
                $this->Disputes_model->add_update_meta($id,$meta);

                $this->load->model('Project_contacts');
                $this->Project_contacts->update_project_temp_contact($id);

                if ($id) {

                    set_alert('success', _l('added_successfully', _l('project')));
                    redirect(admin_url('disputes/view/' . $id));
                }
                ...
                $success = $this->Disputes_model->update($data, $id);
                $this->Disputes_model->add_update_meta($id,$meta);
        ...
        $data['projects_statuses'] = $this->projects_statuses->get_all();

        //$data['project_contacts'] = $this->printallcontacts($id?$id:0);
        ...
        $data['title'] = $title;
        $this->load->view('disputes/project', $data);
    }
    ...
    public function view($id)
    {
        if (has_permission('projects', '', 'view') || $this->projects_model->is_member($id)) {
            ...
            if ($group == 'project_overview') {
                ...
                $data['milestones_found'] = $data['total_milestones'] > 0 || (!$data['total_milestones'] && total_rows(db_prefix() . 'tasks', ['is_session' => 0, 'rel_id' => $id, 'rel_type' => 'project', 'milestone' => 0]) > 0);
            ...
            $this->load->view('disputes/view', $data);
        } else {
    ...
    public function download_all_files($id)
    {
        if ($this->projects_model->is_member($id) || has_permission('projects', '', 'view')) {
            ...
            if (count($files) == 0) {
                ...
                redirect(admin_url('disputes/view/' . $id . '?group=project_files'));
            }
    ...
    public function remove_file($project_id, $id)
    {
        ...
        redirect(admin_url('disputes/view/' . $project_id . '?group=project_files'));
    }
    ...
    public function milestone($id = '')
    {
        ...
        redirect(admin_url('disputes/view/' . $this->input->post('project_id') . '?group=project_milestones'));
    }
    ...
    public function delete_milestone($project_id, $id)
    {
        ...
        redirect(admin_url('disputes/view/' . $project_id . '?group=project_milestones'));
    }
    ...
    public function remove_team_member($project_id, $staff_id)
    {
        ...
        redirect(admin_url('disputes/view/' . $project_id));
    }
    ...
    public function save_note($project_id)
    {
        if ($this->input->post()) {
            ...
            redirect(admin_url('disputes/view/' . $project_id . '?group=project_notes'));
        }
    }
    ...
    public function get_pre_invoice_project_info($project_id)
    {
        if (has_permission('invoices', '', 'create')) {
            ...
            $this->load->view('disputes/project_pre_invoice_settings', $data);
        }
    }
    ...
    public function invoice_project($project_id)
    {
        if (has_permission('invoices', '', 'create')) {
            ...
            redirect(admin_url('disputes/view/' . $project_id . '?group=disputes_invoices'));
        }
    }
