<script>
    $(document).ready(function () {
        ...
        $("#addNewUserForm").submit(function (e) {
            var action = babilchatSettings.addChatGroup;
        ...
        pusher.bind("group-renamed", function (data) {
            let oldGroupName = $("body").find("li.group_selector#" + data.group_id + " a.group-list").text();
            ...
            $("body").find("li.group_selector#" + data.group_id + " a.group-list").text(data.newName);
        });
        ...
        pusher.bind("group-notify-event", function (data) {
            if (data.from !== userSessionId) {
                ...
                if (user_chat_status != "busy" && user_chat_status != "offline") {
                    ...
                    if (data.message.match("audio controls src=")) {
        ...
        var inChannels = [], members, data, result, channels, resp;
        var myGroups = $.post(babilchatSettings.getMyGroups).done(function (r) {
        ...
        $("body").on("click", "#frame .add_chat_member", function () {
            ...
            $(".modal_container").load(babilchatSettings.addNewChatGroupMembersModal, {
                group_id: group_id
            }, function (res) {
    ...
    function loadGroupMessages(el)
    {
        ...
        if ($(messagesScrollbar).children().length !== 0) {
            if (pos == 0 && groupOffsetPush >= 10) {
                ...
                $.get(babilchatSettings.getGroupMessagesHistory, {
                    group_id: to_group,
                    ...
                    .done(function (message) {
                        ...
                        if (Array.isArray(messages) == false) {
                            ...
                            if ($(".group_messages").hasScrollBar() && groupEndOfScroll == true) {
                                babilchat_setNoMoreGroupMessages();
                            }
                        } else {
                            groupOffsetPush += 10;
                        }
    ...
    function getGroupMessages(group_id)
    {
        ...
        createGrooupBoxRequest = $.get(babilchatSettings.getGroupMessages, {
            group_id: group_id,
            offset: 0,
            limit: 20
        })
    ...
    {
        // make event also and change name to all members and rejoin group
        var groupName = prompt(customerSettings.groupNewName);

        if (groupName !== null && groupName !== "" && groupName.length !== 0) {
            groupName = escapeHtml(groupName);
            $.post(babilchatSettings.chatRenameGroup, {
                groupId: groupId,
                groupName: groupName
            }).done(function (r) {
                r = JSON.parse(r);
                ...
                if (r.success) {
                    $("body").find("li.group_selector#" + groupId + " a.group-list").text(groupName);
    ...
    function appendNewChatGroup(data)
    {
        ...
        group += "<img data-toggle=\"tooltip\" title=\"" + strCapitalize(group_name) + '" class="groups_image" src="<?= module_dir_url('babilchat', 'assets/chat_implements/icons/groups.png'); ?>" alt="Group"/>';
    ...
    function getGroupUsers(group_id, group_name, users)
    {
        ...
        if ($("#frame ul li.groups").hasClass("active")) {
            ...
            $("#frame .contact-profile").append('<div class="group_members_inline"><img class="groups_image" src="<?= module_dir_url('babilchat', 'assets/chat_implements/icons/groups.png'); ?>" alt="Group"/><p class="active_group_members"></p></div>');
        }
        ...
        $.each(users, function (i, user) {
            ...
            if (user.created_by_id == userSessionId && $("#frame .chat_groups_list li.active .leave_chat_+group").length == 0) {
                $("#frame .chat_groups_list li.active").append("<button data-toggle=\"tooltip\" title=\"" + customerSettings.chatRenameLabel + "\" onClick=\"renameGroup(" + group_id + ")\" class=\"leave_chat_group btn btn-sm btn-info pull-right\"><i class='fa fa-pencil-square-o' aria-hidden='true'></i></button>");
            }
    ...
    function leaveGroup(group_id)
    {
        ...
        $.post(babilchatSettings.chatMemberLeaveGroup, {
            group_id: group_id,
            member_id: member_id
        }).done(function (r) {
            if (r) {
    ...
    function removeChatGroupUser(user)
    {
        ...
        if (member_id !== "") {
            $.post(babilchatSettings.removeChatGroupUser, {
                id: member_id,
                group_id: group_id,
    ...
    function deleteGroup(el)
    {
        ...
        if (confirm("<?php echo _l('chat_are_you_sure_delete_group'); ?>")) {
            $.post(babilchatSettings.deleteGroup, {
                "group_name": group_name,
                "group_id": group_id
            }).done(function (r) {
    ...
    function getGroupSharedFiles(group_id)
    {
        ...
        $.post(babilchatSettings.getGroupSharedFiles, {
            group_id: group_id
        }).done(function (data) {
            if (data) {
    ...
    $("#frame").on("keypress", "textarea.group_chatbox", function (e) {
        ...
        if (e.which == 13 && !e.shiftKey) {
            ...
            $.post(babilchatSettings.groupMessagePath, formData);
            $(this).val("");
            $(this).focus();
            scroll_event();
            ...
            $.post(babilchatSettings.groupMessagePath, formTyping);
        }
    });
    ...
    function uploadGroupFileForm(file)
    {
        ...
        $.ajax({
            type: "POST",
            url: babilchatSettings.groupUploadMethod,
            data: formData,
            dataType: "json",
            processData: false,
            contentType: false,
            ...
            success: function (r) {
                if (!r.error) {
                    ...
                    var basePath = "<?php echo base_url('modules/babilchat/uploads/groups/'); ?>";
    ...
    function sendMentionNotifications(from, channel, users)
    {
        ...
        $.post(admin_url + "babilchat/Babilchat_Controller/pusherMentionEvent", {
            from: from,
            channel: channel,
            users: users
        });
    ...
    $("textarea.mention").mentionsInput({
        onDataRequest: _.debounce(function (mode, query, callback) {
            ...
            $.getJSON("<?= admin_url('babilchat/Babilchat_Controller/getUsersInJsonFormat'); ?>", {
                group_id: group_id
