...
$aColumns = [
    ...
    '(SELECT date FROM ' . db_prefix() . 'my_disputes_cases_invoices t WHERE is_recurring_from=' . db_prefix() . 'my_disputes_cases_invoices.id ORDER BY id DESC LIMIT 1) as last_date', // Last Date
...
foreach ($rResult as $aRow) {
    ...
    $numberOutput = '<a href="' . admin_url('invoices/list_invoices/' . $aRow['id']) . '" onclick="init_invoice_disputes(' . $aRow['id'] . '); return false;">' . format_invoice_number($aRow['id']) . '</a>';
