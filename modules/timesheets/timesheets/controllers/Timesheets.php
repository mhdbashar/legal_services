...
public function add_requisition_ajax(){
	...
	if ($this->input->post()) {
		...
		if ($result != '') {
			...
			$check_proccess = $this->timesheets_model->get_approve_setting($rel_type, false);
			$check = '';
			if($check_proccess){
	...
	public function send_mail()
	{
		if ($this->input->is_ajax_request()) {
			$data = $this->input->post();
			if((isset($data)) && $data != ''){
				$this->timesheets_model->send_mail($data);
				$success = 'success';
				echo json_encode([
					'success' => $success,                
				]); 
...
{
	if(!class_exists('XLSXReader_fin')){
		require_once(module_dir_path(HIRA_MODULE_NAME).'/assets/plugins/XLSXReader/XLSXReader.php');
	}
	require_once(module_dir_path(HIRA_MODULE_NAME).'/assets/plugins/XLSXWriter/xlsxwriter.class.php');
	if($this->input->post()){
		$data = $this->input->post();
		$list = json_decode($data['attendance']);
		...
		$filename = 'attendance_'.$data['month'].'.xlsx';
