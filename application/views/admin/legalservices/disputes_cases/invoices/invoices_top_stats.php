  ...
  <div class="panel_s mtop20">
    <div class="panel-body">
      ...
      if(!$has_permission_view){
        $where_all .= disputes_get_invoices_where_sql_for_staff(get_staff_user_id());
      }
      $where_all = trim($where_all);
      if(endsWith($where_all,' AND')) {
        $where_all = substr_replace($where_all, '', -4);
      }
      $total_invoices = total_rows(db_prefix().'my_disputes_cases_invoices',$where_all);
      ?>
      <div class="row text-left quick-top-stats">
        ...
        $total_by_status = total_rows(db_prefix().'my_disputes_cases_invoices',$where);
          ...
          <div class="row">
            <div class="col-md-7">
              <a href="#" data-cview="invoices_<?php echo $status; ?>" onclick="dt_custom_view('invoices_<?php echo $status; ?>','.table-invoices','invoices_<?php echo $status; ?>',true); return false;">
                <h5><?php echo disputes_format_invoice_status($status,'',false); ?></h5>
              </a>
            </div>
            ...
            <div class="col-md-12">
              <div class="progress no-margin">
                <div class="progress-bar progress-bar-<?php echo disputes_get_invoice_status_label($status); ?>" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 0%" data-percent="<?php echo $percent; ?>">
                </div>
              </div>
            </div>
          </div>
