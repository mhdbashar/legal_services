 ...
 public function Ticket_List($filters)
        {
            ...
            $this->db->select('ticketid,userid,contactid,department,subject,date,lastreply,status,message,priority,project_id')->from('tbltickets');
...
public function getSupportlistCount()
{
...
$this->db->join('tbltickets', 'tbltickets.status = tbltickets_status.ticketstatusid', 'left');
...
return $query1;
 ...
 public function getSupportlist($filters)
    {
...
{
    $files=get_upload_path_by_type($type) . $id . '/';
   return $files;
}
...

    {
     ...
     public function get_lead_status($id)

    {
       // $this->db->select('id')
         $this->db->where('id', $id);
       $q = $this->db->get('tblleads_status');
       $data = $q->row();
        return $data->name;
    }
    ...
    public function get_priority_name($id)
 ...
 public function get_department_Name($id)
     ...
     public function get_lead_source($id)

    {

       // $this->db->select('id')
        $this->db->where('id', $id);
       $q = $this->db->get('tblleads_sources');
       $data = $q->row();
       return $data->name;
    ...
    public function get_task_statuscolor($id)
    {
       // $this->db->select('id')
        $this->db->where('ticketstatusid', $id);
       $q = $this->db->get('tbltickets_status');
 ...
 public function projectfiles($id='',$type)
 {
    if($type=='project'){
      $this->db->where('id', $id);
       $q = $this->db->get('tblproject_files');
       $data = $q->row();
       return $data->visible_to_customer;
}else{
      $this->db->where('id', $id);
       $q = $this->db->get('tblfiles');
       $data = $q->row();
       return $data->visible_to_customer;
}
  ...
  public function get_addedfrom_name($id)
    {
       // $this->db->select('id')
    ...
    public function check_permission($has_permission,$id,$capability)
        ...
        if ($staff->admin == 1) {
           return 1;
            ...
            if($has_permission=='support'){
                $this->db->select('count(*)')->from('tblstaff_departments');
            $this->db->join('tbldepartments ', 'tbldepartments.departmentid = tblstaff_departments.departmentid');
               
            $this->db->where('tbldepartments.name', $has_permission);
            $this->db->where('tblstaff_departments.staffid', $id);
            ...
            $this->db->where('feature', $has_permission);
            $this->db->where('staff_id', $id);
            $this->db->where('capability', $capability);
            $q = $this->db->get('tblstaff_permissions');
            $data = $q->row();
            }
       if (!empty($data)) {
