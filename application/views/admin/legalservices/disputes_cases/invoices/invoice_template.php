   ...
   <div class="panel-body">
      <div class="row">
         <div id="merge" class="col-md-6">
            <?php
              if(isset($invoice)){
                 $this->load->view('admin/legalservices/disputes_cases/invoices/merge_invoice', array('invoices_to_merge'=>$invoices_to_merge));
              }
            ?>
         </div>
...
<div class="panel_s invoice accounting-template">
   ...
   <div class="panel-body">
      ...
      <?php  echo disputes_format_invoice_status($invoice->status); ?>
      ...
      <div class="row">
         <div class="col-md-6">
            ...
            <?php
               $next_invoice_number = get_option('next_disputes_invoice_number');
               ...
               $prefix = 'DIS-';//get_option('invoice_prefix');
            ...
            <div class="row">
               ...
               <div class="col-md-6">
                ...
                <?php if(disputes_is_invoices_overdue_reminders_enabled()){ ?>
               <div class="form-group">
         ...
         <div class="col-md-6">
            <div class="panel_s no-shadow">
               ...
               <div class="row">
                  ...
                  <div class="col-md-6 <?php echo(isset($invoice)?'hide':''); ?>">
                     <div class="form-group select-placeholder"<?php if(isset($invoice) && !empty($invoice->is_recurring_from)){ ?> data-toggle="tooltip" data-title="<?php echo _l('create_recurring_from_child_error_message', [_l('invoice_lowercase'),_l('invoice_lowercase'), _l('invoice_lowercase')]); ?>"<?php } ?>>
                        ...
                        name="recurring"
                        id="recurring_select"
                        onchange="show_hide_installments()"
                  ...
                  <div id="cycles_wrapper" class="col-md-6 <?php if(!isset($invoice) || (isset($invoice) && $invoice->recurring == 0)){echo ' hide';}?>">
                  ...
                  <div class="col-md-6 hide clearfix">
                     <div class="form-group select-placeholder">
                  ...
                  <div class="clearfix recurring_custom <?php if((isset($invoice) && $invoice->custom_recurring != 1) || (!isset($invoice))){echo 'hide';} ?>">
                     <div class="col-md-6">
                  ...
                  <?php if(isset($project->disputes_total)){ ?>
                  <div>
                    ...
                    <div class="col-md-12 text-center clearfix" id="disputes_total">
                      <?php echo _l('disputes_total') . ': ' . $project->disputes_total; ?>
                    </div>
                  </div>
...
<?php for($ins=0; $ins<50; $ins++){ ?>
                     <div class="installments clearfix">
                      <div class="col-md-6 <?php echo($ins==0?'installmenttotal':'hide'); ?>">
                        <?php $value = (isset($invoice) && $invoice->total && $ins==0 ? $invoice->total : ''); ?>
                        <?php echo render_input('installment_total['.$ins.']','total',$value,'number',array('min'=>1,  'onblur'=>'disputes_calculate_total();', 'onchange'=>'disputes_calculate_total();')); ?>
                      </div>
                      <div class="col-md-6 hide">
                        <?php $value = '';/*(isset($invoice) && $invoice->date ? _d($invoice->duedate) : '');*/ ?>
                        <?php echo render_date_input('installment_date['.$ins.']','date',$value); ?>
                      </div>
                     </div>
<?php } ?>
                  </div>
   ...
   <div class="panel-body mtop10">
      ...
      <?php isset($invoice_from_project); if(isset($invoice_from_project)){ echo '<hr class="no-mtop" />'; } ?>
      <div class="table-responsive s_table">
        <style type="text/css">
          .invoice-items-table tr td:nth-child(4), .invoice-items-table tr th:nth-child(4){
            display: none;
          }
        </style>
            ...
            <tbody>
               ...
               <?php if (isset($invoice) || isset($add_items)) {
                  ...
                  foreach ($add_items as $item) {
                    ...
                    $invoice_item_taxes = disputes_get_invoice_item_taxes($item['id']);
                    // passed like string
                    if ($item['id'] == 0) {
                     ...
                     <div class="row">
                        ...
                        <div class="col-md-5">
                            <div class="input-group" id="discount-total">
                                ...
                                <input type="number" value="<?php echo (isset($invoice) ? $invoice->discount_percent : 0); ?>" class="form-control pull-left input-discount-percent<?php if(isset($invoice) && !is_sale_discount($invoice,'percent') && is_sale_discount_applied($invoice)){echo ' hide';} ?>" min="0" onchange="disputes_calculate_total()" onblure="disputes_calculate_total()" max="100" name="discount_percent">

                                <input type="number" data-toggle="tooltip" data-title="<?php echo _l('numbers_not_formatted_while_editing'); ?>" value="<?php echo (isset($invoice) ? $invoice->discount_total : 0); ?>" class="form-control pull-left input-discount-fixed<?php if(!isset($invoice) || (isset($invoice) && !is_sale_discount($invoice,'fixed'))){echo ' hide';} ?>" onchange="disputes_calculate_total()" onblure="disputes_calculate_total()" min="0" name="discount_total">
               ...
               <tr  class="hide">
                  <td>
                     <div class="row">
                        ...
                        <div class="col-md-5">
                           <input type="number" data-toggle="tooltip" data-title="<?php echo _l('numbers_not_formatted_while_editing'); ?>" value="<?php if(isset($invoice)){echo $invoice->adjustment; } else { echo 0; } ?>" class="form-control pull-left" onchange="disputes_calculate_total()" onblure="disputes_calculate_total()" name="adjustment">
                        </div>
                     </div>
                  </td>
...
<script type="text/javascript">
  ...
  function show_installments(){
    var cycl = $("#cycles").val();
    var cycl_0;
    var total_disputes = <?php echo $project->disputes_total; ?>;
    ...
    $( ".installments" ).each(function( index ) {
      if(cycl>index){
        ...
        if($('input[name=merge_current_invoice]').val()>0){
          // DONT CHANGE OLD DATES
        }else{
          var recurring_inst = $("#recurring_select").val();
          if(recurring_inst<=12){
            var dateString = $('.accounting-template input[name=duedate]').val(); // DUE date.
            var dueDate = new Date(dateString); // convert to actual date
            var nextDate = new Date(dueDate.getFullYear(), (dueDate.getMonth() + (recurring_inst*index)), dueDate.getDate());
            $( this ).find('input.datepicker').val(nextDate.getFullYear()+'-'+(nextDate.getMonth()==0?12:nextDate.getMonth())+'-'+nextDate.getDate());
          }
        }
      }else{
        $( this ).find('div.col-md-6').addClass('hide');
        $( this ).find('input[type=number]').val('');
      }
    });
  }

  function show_hide_installments(){
    if($("#recurring_select").val()==0){
      $('.installments div.col-md-6').addClass('hide');
      $('.installmenttotal').removeClass('hide');
    }else{
      $('.installments div.col-md-6').removeClass('hide');
      $("#cycles").attr('disabled',false);
      show_installments();
    }
  }


  function is_not_recurring(){
    $("#recurring_select").val(1);
    $("#cycles").val(1);
    //disputes_calculate_total();
  }
...
function disputes_calculate_total() {
    ...
    var calculated_tax,
        ...
        rows = $('.installments input[type=number]'),
    ...
    $.each(rows, function() {
        ...
        _amount = accounting.toFixed($(this).val(), app.options.decimal_places);
        _amount = parseFloat(_amount);

        $(this).find('td.amount').html(format_money(_amount, true));
        subtotal += _amount;
        row = $(this);
        item_taxes = null;//$(this).find('select.tax').selectpicker('val');
