...
class Invoices_model extends App_Model
{
    ...
    {
        return $this->db->query('SELECT DISTINCT(sale_agent) as sale_agent, CONCAT(firstname, \' \', lastname) as full_name FROM ' . db_prefix() . 'my_project_invoices JOIN ' . db_prefix() . 'staff ON ' . db_prefix() . 'staff.staffid=' . db_prefix() . 'my_project_invoices.sale_agent WHERE sale_agent != 0')->result_array();
    }
    ...
    {
        $this->db->select('*, ' . db_prefix() . 'currencies.id as currencyid, ' . db_prefix() . 'my_project_invoices.id as id, ' . db_prefix() . 'currencies.name as currency_name');
        $this->db->from(db_prefix() . 'my_project_invoices');
        $this->db->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'my_project_invoices.currency', 'left');
        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->where(db_prefix() . 'my_project_invoices' . '.id', $id);
    ...
    public function get_invoice_recurring_invoices($id)
    {
        ...
        $invoices           = $this->db->get(db_prefix() . 'my_project_invoices')->result_array();
        $recurring_invoices = [];
    ...
    public function get_invoices_total($data)
    {
        ...
        for ($i = 1; $i <= 3; $i++) {
            $select = 'id,total';
            if ($i == 1) {
                $select .= ', (SELECT total - (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'my_project_invoicepaymentrecords WHERE invoiceid = ' . db_prefix() . 'my_project_invoices.id) - (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'credits WHERE ' . db_prefix() . 'credits.invoice_id=' . db_prefix() . 'my_project_invoices.id)) as outstanding';
            } elseif ($i == 2) {
                $select .= ',(SELECT SUM(amount) FROM ' . db_prefix() . 'my_project_invoicepaymentrecords WHERE invoiceid=' . db_prefix() . 'my_project_invoices.id) as total_paid';
            }
    ...
    public function add($data, $expense = false)
    {
        ...
        if ($insert_id) {
            ...
            update_sales_total_tax_column($insert_id, 'invoice', db_prefix() . 'my_project_invoices');
    ...
    public function check_for_merge_invoice($client_id, $current_invoice = '')
    {
        if ($current_invoice != '') {
            ...
            $row = $this->db->get(db_prefix() . 'my_project_invoices')->row();
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'my_project_invoices', $data);
        ...
        if ($affectedRows > 0) {
            update_sales_total_tax_column($id, 'invoice', db_prefix() . 'my_project_invoices');
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'my_project_invoices');
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('invoiceid', $id);
            $this->db->delete(db_prefix() . 'my_project_invoicepaymentrecords');
...
foreach ($client_ids as $clientid) {
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' . db_prefix() . 'my_project_invoices ORDER BY year DESC')->result_array();
    }
