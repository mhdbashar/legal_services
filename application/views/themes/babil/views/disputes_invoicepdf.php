...
if (get_option('show_status_on_pdf_ei') == 1) {
    $info_right_column .= '<br /><span style="color:rgb(' . disputes_invoice_status_color_pdf($status) . ');text-transform:uppercase;">' . disputes_format_invoice_status($status, '', false) . '</span>';
}
...
if ($invoice->project_id != 0 && get_option('show_project_on_invoice') == 1) {
    $invoice_info .= _l('project') . ': ' . get_disputes_case_name_by_id($invoice->project_id) . '<br />';
}
...
if (count($invoice->payments) > 0 && get_option('show_total_paid_on_invoice') == 1) {
    ...
    <tr>
        ...
        <td align="'.$table_td.'" width="15%">-' . app_format_money(sum_from_table(db_prefix().'my_disputes_cases_invoicepaymentrecords', [
        'field' => 'amount',
        'where' => [
