   ...
   $pinned_projects = get_user_pinned_disputes_cases($slug);
   if(count($pinned_projects) > 0){ ?>
      <li class="pinned-separator"></li>
      <?php foreach($pinned_projects as $project_pin){ ?>
         <li class="pinned_project">
            <a href="<?php echo admin_url('Disputes_cases/view/22/'.$project_pin['id']); ?>" data-toggle="tooltip" data-title="<?php echo _l('pinned_project'); ?>"><?php echo $project_pin['name']; ?><br><small><?php echo $project_pin["company"]; ?></small></a>
            <div class="col-md-12">
