<script>
  ...
  (function(){
    ...
    initDataTable(table_registration_leave, admin_url+'hr_profile/table_registration_leave', [1], [1], requisitionServerParams, [1, 'desc']);
     ...
     $.each(requisitionServerParams, function() {
      ...
      initDataTable(table_type_of_leave,admin_url + 'hr_profile/table_type_of_leave');
    ...
    initDataTable(table_additional_timesheets,admin_url + 'hr_profile/table_additional_timesheets', [0], [0],addtimesheetServerParams, [3, 'desc']); 
    ...
    function get_day_from_date(){
      ...
      $.post(admin_url + 'hr_profile/calculate_number_days_off',data).done(function(response){
       response = JSON.parse(response);
    ...
    <?php if(isset($send_mail_approve)){ ?>
      ...
      $.post(admin_url+'hr_profile/send_mail', data_send_mail).done(function(response){
      });
    <?php } ?>
    ...
    $('select[name="staff_id"]').change(function(){
      ...
      $.post(admin_url+'hr_profile/get_remain_day_of/'+id).done(function(response){
        response = JSON.parse(response);
  ...
  function add_requisition(){
    ...
    if($approver_id != '' && $followers_id != ''){
      ...
      $.ajax({ 
        url: admin_url + 'hr_profile/add_requisition_ajax', 
        method: 'post', 
        data: formData, 
        contentType: false, 
        processData: false
  ...
  function staff_delete_bulk_action(event) {
    ...
    if (confirm_delete()) {
      ...
      if(mass_delete == true){
        ...
        setTimeout(function() {
          $.post(admin_url + 'hr_profile/timesheets_delete_bulk_action', data).done(function() {
  ...
  function view_additional_timesheets(id){
    "use strict";
    $.post(admin_url+'hr_profile/get_data_additional_timesheets/'+id).done(function(response){
      response = JSON.parse(response);
  ...
  function change_request_approval_status(id, status, rel_type){
    ...
    data.note = '';
    $.post(admin_url + 'hr_profile/approve_request/' + id, data).done(function(response){
      response = JSON.parse(response); 
  ...
  function get_date(el){
    ...
    $.post(admin_url+'hr_profile/get_date_leave',data).done(function(response){
      response = JSON.parse(response);
  ...
  function send_request_approve(id, addedfrom){
    ...
    $.post(admin_url + 'hr_profile/send_request_approve', data).done(function(response){
      response = JSON.parse(response);
  ...
  function choose_approver(id, addedfrom){
    ...
    if(data.staffid != ''){
      $("body").append('<div class="dt-loader"></div>');
      $.post(admin_url + 'hr_profile/choose_approver', data).done(function(response){
        response = JSON.parse(response);
        $("body").find('.dt-loader').remove();
        if (response.success === true || response.success == 'true') {
          alert_float('success', response.message);
