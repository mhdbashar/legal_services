...
foreach ($rResult as $aRow) {
	...
	$subjectOutput = '<a href="' . admin_url('hr/hr_profile/contracts/' . $aRow['id'] ).'" onclick="init_hr_profile_contract(' . $aRow['id'] . ');return false;">' . $aRow['contract_code'] . '</a>';

	$subjectOutput .= '<div class="row-options">';

	if (has_permission('hr_profile_contract', '', 'view') || is_admin()) {
		$subjectOutput .= '<a href="' . admin_url('hr/hr_profile/contracts/' . $aRow['id'] ).'" onclick="init_hr_profile_contract(' . $aRow['id'] . ');return false;">' . _l('hr_view') .' </a>';
	}
	if (has_permission('hr_profile_contract', '', 'edit') || is_admin()) {
		$subjectOutput .= ' | <a href="' . admin_url('hr/hr_profile/contract/' . $aRow['id']) . '">' . _l('hr_edit') . '</a>';
	}

	if (has_permission('hr_profile_contract', '', 'delete') || is_admin()) {
		$subjectOutput .= ' | <a href="' . admin_url('hr/hr_profile/delete_contract/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
	}
	...
	$row[] = ' <a href="' . admin_url('hr/hr_profile/member/' . $aRow['staff']) . '">' . $aRow['firstname'] . '</a>';
