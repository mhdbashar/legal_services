...
function get_client_kyc_details($client_id)
{
	...
	if (is_numeric($client_id)) {
		...
		$result = $CI->db->get(db_prefix() . 'babil_export_customer_kyc_files');
...
function get_client_services_list($client_id)
{
	...
	if (is_numeric($client_id)) {
		$CI->db->where('client_id',$client_id);
		$result = $CI->db->get(db_prefix() . 'babil_export_customer_services');
		if($result->num_rows()>0)
		{
...
function get_client_services_details($client_id)
{
	...
	if (is_numeric($client_id)) {
		...
		$CI->db->where('client_id',$client_id);
		$CI->db->join(db_prefix() . 'babil_export_customer_services',db_prefix() . 'babil_export_customer_services.item_id='.db_prefix() . 'items.id','left');
