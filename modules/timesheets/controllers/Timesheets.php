        ...
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                foreach ($data_timesheet as $key => $value) {
                    $shift_hour = $this->timesheets_model->get_hour_shift_staff($aRow['staffid'], $value['date_work']);
                    if(strtolower($value['type'])== 'w'){
                        if(is_numeric($value['value']) && $shift_hour > 0){
                            $cal = $value['value']/$shift_hour;
                            $total+=$cal;
                        }
                    }
