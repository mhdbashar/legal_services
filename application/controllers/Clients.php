...
class Clients extends ClientsController
{
                ...
                case 'edit_session':
                    ...
                    if ($data['rel_type'] == 'kd-y' || $data['rel_type'] == 'kdaya_altnfith') {
                        ...
                        if ($case) {
                            ...
                            $data['dept'] = $case->jud_num;
                            $data['cat_id'] = $case->cat_id;
                            $data['subcat_id'] = $case->subcat_id;
                            $data['childsubcat_id'] = $case->childsubcat_id;
                            $data['file_number_court'] = $case->file_number_court;
                        }
                    } elseif ($data['rel_type'] == 'customer') {
                        $customer = get_customer_by_id($data['rel_id']);
                        if ($customer) {
                            $data['court_id'] = $customer->court_id;
                            $data['dept'] = 0;
                        }
                    } elseif ($data['rel_type'] != '') {
                        $serv = get_service_by_id($data['rel_id']);
                        if ($serv) {
                            $data['court_id'] = $serv->court_id;
                            $data['dept'] = 0;
                        }
                    }
                    $assignees = isset($data['assignees']) ? $data['assignees'] : [];
    ...
    public
    function GetJudicialByCourtID($id)
    {
    ...
    public
    function download_all_project_files($id)
    {
        if (!has_contact_permission('projects')) {
            set_alert('warning', _l('access_denied'));
            redirect(site_url());
    ...
    public
    function download_all_session_files($id, $comment_id = null)
    {
    ...
    public
    function download_all_case_files($id)
    {
        if (!has_contact_permission('projects')) {
            set_alert('warning', _l('access_denied'));
            redirect(site_url());
    ...
    public
    function download_all_oservice_files($id)
    {
        if (!has_contact_permission('projects')) {
            set_alert('warning', _l('access_denied'));
            redirect(site_url());
    ...
    public
    function delete_file($id, $type = '', $ServID = '')
    {
    ...
    public
    function remove_task_comment($id)
    {
        echo json_encode([
    ...
    public
    function invoices($status = false)
    {
        if (!has_contact_permission('invoices')) {
            set_alert('warning', _l('access_denied'));
            redirect(site_url());
    ...
    public
    function resume_subscription($id)
    {
        if (!is_primary_contact(get_contact_user_id())
            || get_option('show_subscriptions_in_customers_area') != '1') {
            redirect(site_url());
