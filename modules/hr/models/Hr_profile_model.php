...
class Hr_profile_model extends App_Model
{
	...
	*/
	...
	{
		$department =  $this->db->query('select s.staffid as id, s.team_manage as pid, s.firstname as name,  r.name as rname, j.designation_name as job_designation_name
			from tblstaff as s left join tblroles as r on s.role = r.roleid left join '.db_prefix().'hr_designations as j on j.id = s.job_position where s.status_work != "inactivity"       
		...
		foreach ($department as $dep) {
			if($dep['pid'] == 0){
				...
				$node = array();
				$node['name'] = $dep['name'];
				$node['job_designation_name'] = '';

				if($dep['job_designation_name'] != null && $dep['job_designation_name'] != 'undefined'){
					$node['dp_user_icon'] = '"fa fa-map-pin menu-icon"';

					$node['job_designation_name'] = $dep['job_designation_name'];
				}
	...
	public function get_job_position($id = false)
	{
		if (is_numeric($id)) {
			$this->db->where('id', $id);
			return $this->db->get(db_prefix() . 'hr_designations')->row();
		}
		if ($id == false) {
			return $this->db->query('select * from '.db_prefix().'hr_designations')->result_array();
		}
	}
 ...
 public function get_job_position_interview_process($id = false){
	...
	public function get_jobposition_by_department($department_id = '', $status)
	{
		...
		if(is_array($department_id)){
			...
			$sql_where = 'SELECT '.db_prefix().'hr_designations.id, designation_name FROM '.db_prefix().'staff left join '.db_prefix().'hr_designations on '.db_prefix().'staff.job_position = '.db_prefix().'hr_designations.id WHERE '.db_prefix().'staff.job_position != "0" AND '.db_prefix().'staff.staffid IN ('.$arr_staff_id.')';
			
			
			$arr_job_position = $this->db->query($sql_where)->result_array();
			$arr_check_exist=[];
			foreach ($arr_job_position as $k => $note) {
				if(!in_array($note['id'], $arr_check_exist)){
					$select = ' selected';
					$options .= '<option value="' . $note['id'] . '" '.$select.'>' . $note['designation_name'] . '</option>';
					$arr_check_exist[$k] = $note['id'];
				}
			}
			/*get position end*/
			return $options;
			...
			foreach ($arr_job_position as $note) {
				$options .= '<option value="' . $note['id'] . '">' . $note['designation_name'] . '</option>';
			}
		  return $options;
		}
	}
   ...
   */
	...
	public function add_job_p($data)
	{
		...
		if($insert_id){
			if($option == 'on'){
				$data_position['designation_name'] = $data['job_name'];
	...
	public function add_job_position($data)
	{
		...
		if ($insert_id) {
			if(isset($tags)){
				handle_tags_save($tags, $insert_id, 'hr_designations');
			}
 ...
 */
	public function delete_job_position($id){
			...
			foreach ($job_position_file as $file_key => $file_value) {
				$this->delete_hr_designations_attachment_file($file_value['id']);
			}
	 ...
	 * @return [type]                  
	 */
	public function get_list_job_position_tags_file($job_id)
	{
		$data=[];
		$arr_file = $this->get_hrm_attachments_file($job_id, 'job_position');
		...
		$this->db->where(db_prefix() . 'taggables.rel_id', $job_id);
	...
	{   
		...
		$job_position =  $this->db->get(db_prefix().'hr_designations')->result_array();
	...
	public function check_child_in_job_p($id)
	{
		...
		$arr_job_chil = $this->db->get(db_prefix() . 'hr_designations')->result_array();

		foreach ($arr_job_chil as $key => $value) {
			if (is_reference_in_table('job_position', db_prefix() . 'staff', $value['id'])) {
			   return true;;
			}
		}
		return false;
...
public function get_job_position_tag($id=''){
	...
	*/
	public function get_interview_process_byposition($id = false){
		if (is_numeric($id)) {
			$sql_where ='find_in_set("'.$id.'", job_id)';
	...
	*/
	 ...
	 * @return array                  
	 */
	public function get_job_position_salary_scale($job_id){
		$data=[];
		$salary_insurance = 0;
		$salary_form = [];        
		$salary_allowance = [];   
	...
	public function get_department_from_position_department($arr_value, $position)
	{
		...
		if($position == false){
		 ...
		 foreach ($arr_value as $key => $value) {
			...
			$this->db->where($sql_where);
			$arr_job_position = $this->db->get(db_prefix().'hr_designations')->result_array();
...
public function get_position_by_department($department_id = '', $status)
{
	...
	if(is_array($department_id))
	{
		...
		foreach ($department_id as $key => $value) {
			...
			if(count($arr_job_position) > 0){
				foreach ($arr_job_position as $value) {
					if(!in_array($value['id'], $job_position)){
						$options .= '<option value="' . $value['id'] . '">' . $value['designation_name'] . '</option>';

						$job_position[$index_dep] = $value['id'];
						$index_dep++;
					}
				}
			}
...
public function add_manage_info_reception($data){
   ...
   foreach ($data['title_name'] as $key => $menu) {
	 ...
	 * @return object                
	 */
	public function get_jp_interview_training($id, $training_type = ''){
		if($training_type==''){
			$type_training = $this->getTraining_Setting();    
			if($type_training){
				return $this->db->query('select * from '.db_prefix().'hr_jp_interview_training where find_in_set('.$id.',job_id) and training_type = \''.$type_training->training_type.'\' ORDER BY date_add desc limit 1')->row();
			}
			else{
				return $this->db->query('select * from '.db_prefix().'hr_jp_interview_training where find_in_set('.$id.',job_id) ORDER BY date_add desc limit 1')->row();
			}
		}
		else{
			return $this->db->query('select * from '.db_prefix().'hr_jp_interview_training where find_in_set('.$id.',job_id) and training_type = \''.$training_type.'\' ORDER BY date_add desc limit 1')->row();
		}
	}
...
{
	$position = $this->db->query('select * from '.db_prefix().'hr_designations')->result_array();
	$position_arrray = [];
   ...
   public function get_staff_in_deparment($department_id)
   {
	...
	public function list_job_department($department){
		...
		if(count($arr_staff_id) > 0){
			...
			$arr_staff_id = implode(",", $arr_staff_id);
			$sql_where = 'SELECT '.db_prefix().'hr_designations.id, hr_designations.designation_name FROM '.db_prefix().'hr_extra_info left join '.db_prefix().'hr_designations on '.db_prefix().'hr_extra_info.designation = '.db_prefix().'hr_designations.id WHERE '.db_prefix().'hr_extra_info.designation != "0" AND '.db_prefix().'staff.staff_id IN ('.$arr_staff_id.')';
	 ...
	 * @return [type]                
	 */
	public function delete_hr_designations_attachment_file($attachment_id)
	{
		$deleted    = false;
		$attachment = $this->get_hr_profile_attachments_delete($attachment_id);
		if ($attachment) {
