...
$aColumns = [
   ...
   'ticketid',
    'subject',
    '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM ' . db_prefix() . 'taggables JOIN ' . db_prefix() . 'tags ON ' . db_prefix() . 'taggables.tag_id = ' . db_prefix() . 'tags.id WHERE rel_id = ' . db_prefix() . 'government_tickets.government_ticketid as ticketid and rel_type="ticket" ORDER by tag_order ASC) as tags',
    ...
    'priority',
    'lastreply',
    db_prefix() . 'government_tickets.date',
    ];
...
$additionalSelect = [
    'adminread',
    'ticketkey',
    db_prefix() . 'government_tickets.userid',
    'statuscolor',
    db_prefix() . 'government_tickets.name as ticket_opened_by_name',
    db_prefix() . 'government_tickets.email',
    db_prefix() . 'government_tickets.userid',
    'assigned',
...
$join = [
    'LEFT JOIN ' . db_prefix() . 'contacts ON ' . db_prefix() . 'contacts.id = ' . db_prefix() . 'government_tickets.contactid',
    'LEFT JOIN ' . db_prefix() . 'services ON ' . db_prefix() . 'services.serviceid = ' . db_prefix() . 'government_tickets.service',
    'LEFT JOIN ' . db_prefix() . 'departments ON ' . db_prefix() . 'departments.departmentid = ' . db_prefix() . 'government_tickets.department',
    'LEFT JOIN ' . db_prefix() . 'government_tickets_status ON ' . db_prefix() . 'government_tickets_status.ticketstatusid = ' . db_prefix() . 'government_tickets.status',
    'LEFT JOIN ' . db_prefix() . 'clients ON ' . db_prefix() . 'clients.userid = ' . db_prefix() . 'government_tickets.userid',
    'LEFT JOIN ' . db_prefix() . 'government_tickets_priorities ON ' . db_prefix() . 'government_tickets_priorities.priorityid = ' . db_prefix() . 'government_tickets.priority',
    ];
...
foreach ($custom_fields as $key => $field) {
    ...
    array_push($join, 'LEFT JOIN ' . db_prefix() . 'customfieldsvalues as ctable_' . $key . ' ON ' . db_prefix() . 'government_tickets.government_ticketid = ctable_' . $key . '.relid AND ctable_' . $key . '.fieldto="' . $field['fieldto'] . '" AND ctable_' . $key . '.fieldid=' . $field['id']);
}
...
if (isset($userid) && $userid != '') {
    array_push($where, 'AND ' . db_prefix() . 'government_tickets.userid = ' . $this->ci->db->escape_str($userid));
} elseif (isset($by_email)) {
    array_push($where, 'AND ' . db_prefix() . 'government_tickets.email = "' . $this->ci->db->escape_str($by_email) . '"');
}

if (isset($where_not_ticket_id)) {
    array_push($where, 'AND ' . db_prefix() . 'government_tickets.government_ticketid != ' . $this->ci->db->escape_str($where_not_ticket_id));
}
...
$statuses  = $this->ci->government_tickets_model->get_ticket_status();
$_statuses = [];
...
$assignees  = $this->ci->government_tickets_model->get_tickets_assignes_disctinct();
$_assignees = [];
...
$sIndexColumn = 'government_ticketid';
$sTable       = db_prefix() . 'government_tickets';
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == '1') {
            $_data = '<div class="checkbox"><input type="checkbox" value="' . $aRow['government_ticketid'] . '"><label></label></div>';
        ...
        } elseif ($aColumns[$i] == 'subject' || $aColumns[$i] == 'government_ticketid') {
            // Ticket is assigned
            if ($aRow['assigned'] != 0) {
                if ($aColumns[$i] != 'government_ticketid') {
            ...
            $url   = admin_url('government_tickets/ticket/' . $aRow['government_ticketid']);
            ...
            if ($aColumns[$i] == 'subject') {
                ...
                $_data .= ' <span class="text-dark"> | </span><a href="' . admin_url('government_tickets/delete/' . $aRow['government_ticketid']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
                $_data .= '</div>';
            }
        ...
        } elseif ($aColumns[$i] == db_prefix() . 'government_tickets.date') {
            $_data = _dt($_data);
