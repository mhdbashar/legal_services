...
<div id="pusherChat">
            ...
            <div class="chat-footer" style="background:<?php echo $currentChatColor; ?>">
                ...
                <div class="dropup">
                    ...
                    <ul class="dropdown-menu" id="colorChangerMenu">
                        <li>
                            <a href="#" id="colorChanger"><i class="fa fa-paint-brush" aria-hidden="true"></i><?= _l('chat_solid_color_text'); ?>
                            </a></li>
                        <li><a href="#" id="colorGradient"></i>
                        ...
                        <li>
                            <a href="#" id="resetColors" onClick="resetChatColors()"><i class="fa fa-refresh" aria-hidden="true"></i><?= _l('chat_reset_color_text'); ?>
                            </a></li>
                    </ul>
                </div>
                <div class="form-inline colorHolder">
                    <form method="POST" name="solidForm" style="display: none" action="<?php echo site_url('babilchat/Babilchat_Controller/colorchange/'); ?>" onsubmit="changeColor(this); return false;">
                    ...
                    <form method="POST" name="gradientForm" style="display: none" action="<?php echo site_url('babilchat/Babilchat_Controller/colorchange/'); ?>" onsubmit="changeColor(this); return false;">
    ...
    <div id="templateChatBox">
      ...
      <span class="state">
        <span class="userIsTyping"><img src="<?php echo module_dir_url('babilchat', 'assets/chat_implements/userIsTyping.gif'); ?>" /></span>
        <span class="quote">
...
<?php require('modules/babilchat/assets/module_includes/chat_settings.php'); ?>
<!-- Include chat statuses file -->
<?php require('modules/babilchat/assets/module_includes/chat_statuses.php'); ?>
...
<script>
    ...
    emojify.setConfig({
        ...
        "img_dir": site_url + "/modules/babilchat/assets/chat_implements/emojis"
    });
    emojify.run();
    ...
    function uploadFileForm(file)
    {
        ...
        $.ajax({
            type: "POST",
            url: '<?php echo site_url('babilchat/Babilchat_Controller/uploadMethod'); ?>',
            data: formData,
            dataType: "json",
            processData: false,
            contentType: false,
            ...
            success: function (r) {
                ...
                var basePath = "<?php echo module_dir_url('babilchat', 'uploads/'); ?>";
    ...
    function loadMessages(el)
    {
        ...
        if (endOfScroll[to] == true) {
            babilchat_setNoMoreMessages(to);
            return false;
        }
        ...
        if (pos == 0) {
            ...
            initiatePrepending = function () {
                $.get(babilchatSettings.getMessages, {
                    from: from,
                    to: to,
    ...
    var pusher = new Pusher(pusherKey, {
        authEndpoint: "<?php echo site_url('babilchat/Babilchat_Controller/pusher_auth'); ?>",
        authTransport: "jsonp",
        "cluster": appCluster,
        disableStats: true,
        auth: {
    ...
    presenceChannel.bind("pusher:subscription_succeeded", function (members) {
        ...
        if (!$(".liveUsers").length) {
            $("#menu .menu-item-babilchat span.menu-text").append("<span class=\"liveUsers badge menu-badge bg-info\" data-toggle=\"tooltip\" title=\"" + babilchatSettings.onlineUsersMenu + "\">" + (presenceChannel.members.count - 1) + "</span>");
        }
    });
    ...
    presenceChannel.bind("pusher:member_added", function (member) {
        ...
        if (member.info.justLoggedIn == true) {
            $(".liveUsers").remove();
            $("#menu .menu-item-babilchat span.menu-text").append("<span class=\"liveUsers badge menu-badge bg-info\" data-toggle=\"tooltip\" title=\"" + babilchatSettings.onlineUsersMenu + "\">" + (presenceChannel.member.count - 1) + "</span>");

            $.notify("", {
                "title": app.lang.new_notification,
                "body": member.info.name + " " + babilchatSettings.hasComeOnlineText,
        ...
        if (presenceChannel.members.me.id == data.to && data.from != presenceChannel.members.me.id) {
            if (app.options.desktop_notifications && chat_desktop_notifications_enabled) {
                if (user_chat_status != "busy" && user_chat_status != "offline") {
                    ...
                    if (data.message.includes("class=\"babilchat_convertedImage\"")) {
            ...
            pusherFrom.find(".userName").addClass("newMessage");
    ...
    $("#pusherChat").on("focusout", ".chatbox", function () {
        ...
        if ($(this).next().next().next().val() == "true") {
            $.post(babilchatSettings.serverPath, from.serialize());
    ...
    $("#pusherChat").on("click", ".pusherChatBox chathead", function (event) {
        ...
        var uName = $("#pusherChat .pusherChatBox#" + id + " .userName");
        var isSliderActive = $.Deferred();
    ...
    $("#pusherChat").on("click", ".closeBox", function (event) {
        ...
        $(this).parents(".pusherChatBox#" + id + " .userName").removeClass("newMessage");
    ...
    $("#pusherChat").on("keypress", ".pusherChatBox textarea", function (e) {
        ...
        if (e.which == 13 && !e.shiftKey) {
            ...
            // Send event
            $.post(babilchatSettings.serverPath, form.serialize());
            e.preventDefault();
            $(this).val("");
            $(this).focus();
    ...
    $("#pusherChat").on("click", ".logMsg, .chatbox", function (e) {
        ...
        pusherChatBox.find(".userName").removeClass("newMessage");
        ...
        if (!$(".pusherChatBox#" + id).html() || onlyLoadMessages) {
            ...
            promise.then(function (message) {
                ...
                if (obj.hasClass("on")) {
                    ...
                    // $("#pusherChat #" + id + " .userName").addClass("newMessage");
    ...
    $(document).keyup(function (e) {
        if (e.keyCode == 27) {
            var $babilChatChatboxes = $("body").find(".closeBox");
            $.each($babilChatChatboxes, function () {
    ...
    function delete_chat_message(msg_id)
    {
        ...
        $.post(babilchatSettings.deleteMessage, {
            id: msg_id
    ...
    $("#pusherChat svg.main_chat").on("click", function () {
        var redirect_url = $(".menu-item-babilchat a").attr("href");
    ...
    $("body").on("click", ".client_message_link", function () {
        ...
        window.location.href = admin_url + "babilchat/Babilchat_Controller/chat_full_view";
    });
    ...
    $("body").on("click", ".babilchat_convertedImage", function () {
    ...
    clientsChannel.bind("send-event", function (data) {
        ...
        if (data.to == "staff_" + userSessionId) {
            ...
            if ($(".chatNewMessages").length > 0) {
                ...
                $("#menu .menu-item-babilchat").append("<a href=\"#\" class=\"client_message_link\" data-toggle=\"tooltip\" title=\"" + babilchatSettings.newMessages + "\" ><i class=\"fa fa-envelope icon_new_messages\"><span class=\"chatNewMessages\">1</span></i></a>");
            }
        }
    });
    ...
    function appendClientIfOnline()
    {
        ...
        $("#menu .menu-item-babilchat span.menu-text").append('<span class="liveClients badge menu-badge bg-info" data-toggle="tooltip" title="<?= _l('chat_online_clients_label'); ?>">' + addMemberCount + "</span>");
    }
    ...
    function removeClientMember(member)
    {
        ...
        clientPendingRemoves[member.id] = setTimeout(function () {
            ...
            $("#menu .menu-item-babilchat span.menu-text").append('<span class="liveClients badge menu-badge bg-info" data-toggle="tooltip" title="<?= _l('chat_online_clients_label'); ?>">' + rMemberCount + "</span>");
        }, 5000);
    }
    <?php endif; ?>
    ...
    $("body").on("click", ".msgTxt ._forwardMessage", function () {
        ...
        $(".modal_container").load(babilchatSettings.showForwardModal, function () {
