...
class Service_model extends App_Model
{
    ...
    public function get($ServID, $id = '', $where = [])
    {
        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->where(array('my_imported_services.id' => $id,'my_imported_services.service_id' => $ServID, 'my_imported_services.deleted' => 0));
        ...
        $this->db->where(array('service_id' => $ServID, 'my_imported_services.deleted' => 0));
        $this->db->select('*,' . get_sql_select_client_company());
        $this->db->join(db_prefix() . 'clients', db_prefix() . 'clients.userid=' . db_prefix() . 'my_imported_services.clientid');
        $this->db->order_by('my_imported_services.id', 'desc');
        return $this->db->get(db_prefix() . 'my_imported_services')->result_array();
    ...
    public function GetTopNumbering()
    {
        ...
        $this->db->from('my_imported_services');
    ...
    public function GetClientsServices($id)
    {
        ...
        $this->db->join('my_imported_services', 'clients.userid = my_imported_services.clientid' ,'right');
        $this->db->where('my_imported_services.id', $id);
        return $this->db->get()->row();
    }
    ...
    public function GetMembersServices($id)
    {
        ...
        $this->db->join('my_imported_services', 'my_imported_services.id = my_members_services.oservice_id');
    ...
    public function update($ServID,$id, $data)
    {
        ...
        $old_status = $this->db->get(db_prefix() . 'my_imported_services')->row()->status;
        ...
        if ($affectedRows > 0) {
            ...
            if ($original_project->status != $data['status']) {
                ...
                if ($data['status'] == 4) {
                    ...
                    $this->db->where(array('id' => $id, 'service_id' => $ServID));
                    $this->db->update(db_prefix() . 'my_imported_services', ['date_finished' => date('Y-m-d H:i:s')]);
                } else {
    ...
    public function get_most_used_billing_type()
    {
        return $this->db->query('SELECT billing_type, COUNT(*) AS total_usage
                FROM ' . db_prefix() . 'my_imported_services
    ...
    public function get_last_project_settings()
    {
        ...
        $this->db->order_by('id', 'DESC');
        $this->db->limit(1);
        $last_project = $this->db->get(db_prefix() . 'my_imported_services')->row();
        if ($last_project) {
    ...
    public function get_all_projects_gantt_data($ServID, $filters = [])
    {
        ...
        foreach ($statuses as $status) {
            ...
            if (!has_permission('projects', '', 'view')) {
                $this->db->where(db_prefix() . 'my_imported_services.id IN (SELECT oservice_id FROM ' . db_prefix() . 'oservice_members WHERE staff_id=' . get_staff_user_id() . ')');
            }

            if ($filters['member']) {
                $this->db->where(db_prefix() . 'my_imported_services.id IN (SELECT oservice_id FROM ' . db_prefix() . 'oservice_members WHERE staff_id=' . $filters['member'] . ')');
            }

            $this->db->where('status', $status['id']);
            $this->db->order_by('deadline IS NULL ASC, deadline', '', false);
            $projects = $this->db->get(db_prefix() . 'my_imported_services')->result_array();
    ...
    public function send_project_customer_email($id, $template, $ServID = '')
    {
        ...
        $this->db->where('id', $id);
        $clientid = $this->db->get(db_prefix() . 'my_imported_services')->row()->clientid;
        $sent = false;
    ...
    public function mark_as($ServID, $data, $slug)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($data['status_id'] == 4) {
                ...
                if ($old_status == 4) {
                    $this->db->update(db_prefix() . 'my_imported_services', ['date_finished' => null]);
                }
            }
    ...
    public function get_discussion($id, $project_id = '')
    {
        ...
        if (is_client_logged_in()) {
            ...
            $this->db->where('oservice_id IN (SELECT id FROM ' . db_prefix() . 'my_imported_services WHERE clientid=' . get_client_user_id() . ')');
        }
    ...
    public function copy($ServID,$project_id, $data)
    {
        ...
        $_new_data = [];
        $fields = $this->db->list_fields(db_prefix() . 'my_imported_services');
        ...
        $this->db->insert(db_prefix() . 'my_imported_services', $_new_data);
    ...
    private function _get_project_billing_data($id)
    {
        ...
        return $this->db->get(db_prefix() . 'my_imported_services')->row();
    }
