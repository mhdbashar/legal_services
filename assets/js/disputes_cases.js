...
$(function() {
    ...
    $('#project_top').on('change', function() {
        ...
        window.location.href = admin_url + 'Disputes_cases/view/' + servid + '/' + val + __project_group;
    });
    ...
    _table_api_case = initDataTable('.table-case-expenses', admin_url + 'legalservices/disputes_cases/expenses/' + project_id + '/' + slug_expenses_case, 'undefined', 'undefined', Expenses_ServerParams, [4, 'desc']);
    ...
    initDataTable('.table-timesheets_case', admin_url + 'legalservices/disputes_cases/timesheets/' + project_id + '/' + slug_timesheets_case , [8], [8], Timesheets_ServerParams, [3, 'desc']);
    ...
    initDataTable('.table-case-discussions', admin_url + 'legalservices/disputes_cases/discussions/' + project_id + '/' + slug_case_discussions, undefined, undefined, 'undefined', [1, 'desc']);

    initDataTable('.table-case-procuration', admin_url + 'legalservices/disputes_cases/procurations/' + project_id, undefined, undefined, 'undefined', [1, 'desc']);
...
function saveProjectExternalFile(files, externalType) {
    $.post(admin_url + 'legalservices/disputes_cases/add_external_file', {
        files: files,
...
function mass_stop_timers(only_billable,servid) {
    requestGetJSON('legalservices/disputes_cases/mass_stop_timers/' + project_id + '/' + only_billable).done(function(response) {
...
function pre_invoice_case(servid) {
    requestGet('legalservices/disputes_cases/get_pre_invoice_project_info/' + servid + '/' + project_id).done(function(response) {
...
function delete_case_discussion(id) {
    if (confirm_delete()) {
        requestGetJSON('legalservices/disputes_cases/delete_discussion/' + id).done(function(response) {
            alert_float(response.alert_type, response.message);
            $('.table-case-discussions').DataTable().ajax.reload(null, false);
        });
    }
...
function view_case_file(id, $project_id) {
    $('#project_file_data').empty();
    $("#project_file_data").load(admin_url + 'legalservices/disputes_cases/file/' + id + '/' + project_id, function(response, status, xhr) {
        if (status == "error") {
            alert_float('danger', xhr.statusText);
        }
    });
...
function update_file_data(id) {
    ...
    $.post(admin_url + 'legalservices/disputes_cases/update_file_data/', data);
}
...
function case_files_bulk_action(e) {
    if (confirm_delete()) {
        ...
        setTimeout(function() {
            $.post(admin_url + 'legalservices/disputes_cases/bulk_action_files', data).done(function() {
                window.location.reload();
            });
        }, 200);
    }
...
function confirm_case_status_change(e, slug) {
    ...
    $.post(admin_url + 'legalservices/disputes_cases/mark_as/' + slug, data).done(function(response) {
        response = JSON.parse(response);
...
function milestones_case_kanban_update(ui, object) {
    if (object === ui.item.parent()[0]) {
        ...
        setTimeout(function() {
            $.post(admin_url + 'legalservices/disputes_cases/update_task_milestone', data)
        }, 50);
    }
}
...
function milestones_case_kanban() {
    ...
    init_kanban('legalservices/disputes_cases/milestones_kanban/' + slug_case_kanban, milestones_case_kanban_update, '.case-milestone', 445, 360, after_milestones_case_kanban);
}
...
function after_milestones_case_kanban() {
    $("#kan-ban").sortable({
        ...
        update: function(event, ui) {
            ...
            $.post(admin_url + 'legalservices/disputes_cases/update_milestones_order', data);
        }
    });
