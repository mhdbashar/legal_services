...
class Awards_model extends App_Model{

	private $table_name = 'hr_awards';
    ...
    public function uploadImage($field, $id)
    {
        ...
        mkdir('uploads/hr/awards/'.$id, 0777, true);
        $config['upload_path'] = 'uploads/hr/awards/'.$id.'/';
    ...
    public function add($data){
        ...
        if($insert_id){
            ...
            if (!empty($_FILES['award_photo']['name'])) {
                $data['id'] = $insert_id;
                $val = $this->uploadImage('award_photo', $insert_id);
                $val == TRUE || redirect($_SERVER['HTTP_REFERER']);
                if(!empty($val['path'])){
                    $data['award_photo'] = $val['path'];
                }else{
    ...
    public function update($data, $id){
        ...
        // Upload file
        if (!empty($_FILES['award_photo']['name'])) {
            $val = $this->uploadImage('award_photo', $id);
            $val == TRUE || redirect($_SERVER['HTTP_REFERER']);
            if(!empty($val['path'])){
                $data['award_photo'] = $val['path'];
            }else{
