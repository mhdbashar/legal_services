      ...
      <div class="panel_s mtop5">
         <div class="panel-body">
            <div class="_buttons">
               ...
               <a href="<?php echo admin_url('hr/knowledge_base_q_a/article'); ?>" class="btn btn-info mright5"><?php echo _l('kb_article_new_article'); ?></a>
               <?php } ?>
               <?php if($has_permission_edit || $has_permission_create){ ?>
               <a href="<?php echo admin_url('hr/knowledge_base_q_a/manage_groups'); ?>" class="btn btn-info mright5"><?php echo _l('als_kb_groups'); ?></a>
               <?php } ?>
                        ...
                        <ul class="kan-ban-col<?php if(!$has_permission_edit){echo ' sortable-disabled'; } ?>" data-col-group-id="<?php echo html_entity_decode($group['groupid']); ?>">
                           <li class="kan-ban-col-wrapper">
                              <div class="border-right panel_s">
                                 ...
                                 <?php
                                    $this->db->select('*, (SELECT COUNT(*) FROM '.db_prefix().'hr_views_tracking WHERE rel_type="hr_kb_article" AND rel_id='.db_prefix().'hr_knowledge_base.articleid) as total_views')->from(db_prefix().'hr_knowledge_base')->where('articlegroup',$group['groupid'])->order_by('article_order','asc');
                                       ...
                                       <ul class="sortable article-group groups<?php if(!$has_permission_edit){echo 'sortable-disabled'; } ?>" data-group-id="<?php echo html_entity_decode($group['groupid']); ?>">
                                          ...
                                          <li class="<?php if($article['active'] == 0){echo 'line-throught';} ?>" data-article-id="<?php echo html_entity_decode($article['articleid']); ?>">
                                             <div class="panel-body">
                                                ...
                                                <a href="<?php echo admin_url('hr/knowledge_base_q_a/view/'.$article['slug']); ?>">
                                                <?php } else { ?>
                                                ...
                                                <?php if($has_permission_edit){ ?>
                                                <a href="<?php echo admin_url('hr/knowledge_base_q_a/article/'.$article['articleid']); ?>" target="_blank" class="pull-right"><span><i class="fa fa-pencil-square-o" aria-hidden="true"></i></span></a>
                                                <?php } ?>
...
<?php init_tail(); ?>
<?php  require('modules/hr/assets/js/knowledge_base_q_a/articles_js.php'); ?>
