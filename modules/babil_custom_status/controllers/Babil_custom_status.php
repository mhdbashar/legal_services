...
class Babil_custom_status extends AdminController
{
	public function __construct()
	{
		...
		if (!is_admin() && !has_permission('babil_custom_status', '', 'view')) {
			access_denied(_l('babil_custom_status'));
		}
	}
	...
	public function statuses($relto='')
	{
		...
		if($relto=='')
			redirect(admin_url('babil_custom_status/statuses/projects'));
		...
		$data['title']		= _l('babil_custom_status_setup_menu');
		$data['relto']		= $relto;
	...
	public function status()
	{
		...
		if ($this->input->post()) {
			...
			if (!$this->input->post('id')) {
				...
				if (!$inline) {
					if ($id) {
						set_alert('success', _l('added_successfully', _l('babil_custom_statuses')));
					}
				} else {
				...
				if ($success) {
					set_alert('success', _l('updated_successfully', _l('babil_custom_statuses')));
				}
			}
		}
	}
	...
	public function delete_status($id)
	{
		...
		if (is_array($response) && isset($response['referenced'])) {
			set_alert('warning', _l('is_referenced', _l('babil_custom_statuses')));
		} elseif ($response == true) {
			set_alert('success', _l('deleted', _l('babil_custom_statuses')));
		} else {
			set_alert('warning', _l('problem_deleting', _l('babil_custom_statuses')));
		}
		redirect($_SERVER['HTTP_REFERER']);
	}
}