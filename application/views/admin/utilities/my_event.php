          ...
          <div class="col-md-12">
            ...
            <?php if($event->userid == get_staff_user_id() || is_admin()){ ?>
              ...
              <?php echo render_textarea('description','event_description',$event->description,array('rows'=>5)); ?>
                <div class="row">
                    <?php
                    $start = strtotime( _dt($event->start) );
                    $start = date( 'Y-m-d', $start );


                    $start_time = strtotime( _dt($event->start) );
                    $start_time = date( 'H:i', $start_time );
                    ?>
                    <div class="col-md-6">
                        <?php echo render_date_input('start','utility_calendar_new_event_start_date', $start); ?>
                    </div>

                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="time" class="col-form-label"><?php echo _l('time_start'); ?></label>
                            <?php $value = (isset($task) ? $task->time : ''); ?>
                            <input type="<?php echo (get_option('time_format') == 24) ? 'text' : 'time' ?>"
                                   class="form-control time" value="<?php echo $start_time; ?>" id="time" name="time"
                                   autocomplete="off">
                        </div>
                    </div>
                </div>
              <div class="clearfix mtop15"></div>
                <div class="row">
                    <?php
                    $end = strtotime( _dt($event->end) );
                    $end = date( 'Y-m-d', $end );


                    $end_time = strtotime( _dt($event->end) );
                    $end_time = date( 'H:i', $end_time );
                    ?>
                    <div class="col-md-6">
                        <?php echo render_date_input('end','utility_calendar_new_event_end_date', $end); ?>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="end_time" class="col-form-label"><?php echo _l('time_end'); ?></label>
                            <?php $value = (isset($task) ? $task->time : ''); ?>
                            <input type="<?php echo (get_option('time_format') == 24) ? 'text' : 'time' ?>"
                                   class="form-control time" value="<?php echo $end_time; ?>" id="end_time" name="end_time"
                                   autocomplete="off">
                        </div>
                    </div>
                </div>
