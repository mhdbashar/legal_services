...
<?php if(isset($client)){ 
$main_heading = get_option(BABIL_EXPORT_CUSTOMER_MODULE_NAME.'_print_heading_text');
$custom_fields_heading = get_option(BABIL_EXPORT_CUSTOMER_MODULE_NAME.'_print_custom_fields_text');
?>
<div role="tabpanel" class="tab-pane" id="customer_preview">
	<div class="col-md-12">
		<a href="<?php echo base_url(BABIL_EXPORT_CUSTOMER_MODULE_NAME.'/export_customer_profile/'.$client->userid);?>" target="_blank" class="btn btn-default btn-with-tooltip mright5 pull-right" data-toggle="tooltip" title="<?php echo _l('print'); ?>" data-placement="bottom">
			<i class="fa fa-print"></i>
		</a>
		<a href="<?php echo base_url(BABIL_EXPORT_CUSTOMER_MODULE_NAME.'/export_customer_profile/'.$client->userid.'?download=1');?>"  class="btn btn-default btn-with-tooltip mright5 pull-right" data-toggle="tooltip" title="<?php echo _l('view_pdf'); ?>" data-placement="bottom">
	...
	<div class="col-md-12">
		...
		<div class="col-md-6 text-right">
			...
			{?>
				<p class="control-label"><b><?php echo ucwords(_l('babil_customer_primary_contact'))?> :</b></p>
	...
	<div class="col-md-12">
		<h4 class="hr-panel-heading"><?php echo _l('babil_customer_file_attachment_info')." <a href='".admin_url('clients/client/'.$client->userid.'?group=attachments')."' target='_blank'>". _l('customer_attachments')."</a>";?>
		<br/><small><?php echo _l('babil_customer_file_attachment_note');?></small></h4>
		<div class="col-md-6">
			<?php
			...
			echo render_select('babil_export_customer_profile_file', $files, array('id','file_name'),'babil_customer_profile_logo', $selected_profile_file); ?>
		</div>
		<div class="col-md-6">
			<?php
			$selected_files = (!empty($client_kyc_details)?unserialize($client_kyc_details['files_id']):array());
			echo render_select('babil_export_customer_files[]',$files,array('id','file_name'),'babil_customer_file_attachment',$selected_files,array('multiple'=>true,'data-actions-box'=>true),array(),'','',false); ?>
		</div>
	</div>
	<div class="clearfix"></div>
	<hr />
	<div class="col-md-12">
		<div class="col-md-12">
			...
			echo render_select('babil_export_customer_items[]',$items,array('itemid','description'),'babil_customer_items',$selected_items,array('multiple'=>true,'data-actions-box'=>true),array('group_id'),'','',false); ?>
		</div>
	</div>
	<div class="clearfix"></div>
	<hr />
