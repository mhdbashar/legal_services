...
function disputes_get_invoice_total_left_to_pay($id, $invoice_total = null)
{
    ...
    if ($invoice_total === null) {
        $CI->db->select('total')
        ->where('id', $id);
        $invoice_total = $CI->db->get(db_prefix() . 'my_project_invoices')->row()->total;
    }
...
function disputes_update_invoice_status($id, $force_update = false, $prevent_logging = false)
{
    ...
    $CI->db->select('amount')
    ->where('invoiceid', $id)
    ->order_by(db_prefix() . 'my_project_invoicepaymentrecords.id', 'asc');
    $payments = $CI->db->get(db_prefix() . 'my_project_invoicepaymentrecords')->result_array();
    ...
    $CI->db->where('id', $id);
    $CI->db->update(db_prefix() . 'my_project_invoices', [
        'status' => $status,
    ]);
...
{
    $CI = & get_instance();
    $CI->db->select('id')->from(db_prefix() . 'my_project_invoices')->order_by('id', 'desc')->limit(1);
    $query           = $CI->db->get();
...
{
    $CI = & get_instance();
    $CI->db->select('date,number,prefix,number_format')->from(db_prefix() . 'my_project_invoices')->where('id', $id);
...
{
    $CI = & get_instance();
    $CI->db->select('' . db_prefix() . 'currencies.name as currency_name,allowed_payment_modes')->from(db_prefix() . 'my_project_invoices')->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'my_project_invoices.currency', 'left')->where(db_prefix() . 'my_project_invoices.id', $invoiceid);
    $invoice       = $CI->db->get()->row();
...
function disputes_get_invoices_percent_by_status($status)
{
    ...
    $total_invoices      = total_rows(db_prefix() . 'my_project_invoices', 'status NOT IN(5)' . (!$has_permission_view ? ' AND (' . disputes_get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));

    $data            = [];
    $total_by_status = 0;
    if (!is_numeric($status)) {
        if ($status == 'not_sent') {
            $total_by_status = total_rows(db_prefix() . 'my_project_invoices', 'sent=0 AND status NOT IN(2,5)' . (!$has_permission_view ? ' AND (' . disputes_get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));
        }
    } else {
        $total_by_status = total_rows(db_prefix() . 'my_project_invoices', 'status = ' . $status . ' AND status NOT IN(5)' . (!$has_permission_view ? ' AND (' . disputes_get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));
    }
...
function disputes_staff_has_assigned_invoices($staff_id = '')
{
    ...
    if (is_numeric($cache)) {
        ...
        $result = total_rows(db_prefix() . 'my_project_invoices', ['sale_agent' => $staff_id]);
...
function disputes_load_invoices_total_template()
{
    ...
    $CI->load->view('disputes/invoices/invoices_total_template', $data);
}
...
function disputes_get_invoices_where_sql_for_staff($staff_id)
{
    ...
    if ($has_permission_view_own) {
        $whereUser = '((' . db_prefix() . 'my_project_invoices.addedfrom=' . $staff_id . ' AND ' . db_prefix() . 'my_project_invoices.addedfrom IN (SELECT staff_id FROM ' . db_prefix() . 'staff_permissions WHERE feature = "invoices" AND capability="view_own"))';
