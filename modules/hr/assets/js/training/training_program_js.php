...
var table_training_program = $('.table-table_training_program');
    initDataTable(table_training_program, admin_url+'hr/hr_profile/table_training_program', [0], [0], TrainingProgramServerParams, [0, 'desc']);
...
function edit_training_process(invoker,id, rec_evaluation_form_id){
  ...
  $('#job_position_training select[name="training_type"]').change();
  $.post(admin_url + 'hr/hr_profile/get_list_job_position_training/'+id).done(function(response) {
...
function training_type_change(invoker){
  ...
  if(invoker.value){
    $.post(admin_url + 'hr/hr_profile/get_training_type_child/'+invoker.value).done(function(response) {
      response = JSON.parse(response);
...
function department_change(invoker){
  ...
  $.post(admin_url + 'hr/get_jobposition_fill_data',data_select).done(function(response){
   response = JSON.parse(response);
