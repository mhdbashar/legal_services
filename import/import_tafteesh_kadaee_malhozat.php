...
//        $HighestColumn = $worksheet->getHighestColumn();
        ...
        $sql = "INSERT INTO tblknowledge_base_groups(name, group_slug, active, color, parent_id, is_main)
                        VALUES
                        ('الملحوظات في القضايا الجنائية','الملحوظات-في-القضايا-الجنائية','1','','14','0')";
        if (mysqli_query($connect, $sql)) {
            $num1 = mysqli_insert_id($connect);
        }
        $sql = "INSERT INTO tblknowledge_base_groups(name, group_slug, active, color, parent_id, is_main)
                        VALUES
                        ('الملحوظات في القضايا الحقوقية','الملحوظات-في-القضايا-الحقوقية','1','','14','0')";
        if (mysqli_query($connect, $sql)) {
            $num2 = mysqli_insert_id($connect);
        }
        $sql = "INSERT INTO tblknowledge_base_groups(name, group_slug, active, color, parent_id, is_main)
                        VALUES
                        ('الملحوظات في الإنهاءات','الملحوظات-في-الإنهاءات','1','','14','0')";
        if (mysqli_query($connect, $sql)) {
            $num3 = mysqli_insert_id($connect);
        }
        $sql = "INSERT INTO tblknowledge_base_groups(name, group_slug, active, color, parent_id, is_main)
                        VALUES
                        ('الملحوظات العامة','الملحوظات-العامة','1','','14','0')";
        if (mysqli_query($connect, $sql)) {
            $num4 = mysqli_insert_id($connect);
        }
        ...
        for ($row = 1; $row <= $lastRow; $row++) {
            ...
            if($row >= 1 && $row < 17){
                $num = $num1;//الملحوظات في القضايا الجنائية
                $a = str_replace(substr($a,-14),'',$a);
            }
            if($row >= 17 && $row < 53){
                $num = $num2;//الملحوظات في القضايا الحقوقية
                if($row== 17){
                    $a = str_replace(substr($a,-20),'',$a);

                }else
                    $a = str_replace(substr($a,-16),'',$a);
            }
            if($row >= 53 && $row < 83){
                $num = $num3;//الملحوظات في الإنهاءات
                $a = str_replace(substr($a,-18),'',$a);
            }
            if($row >= 83 && $row < 115){
                $num = $num4;//الملحوظات العامة
                $a = str_replace(substr($a,-16),'',$a);
            }
            ...
            if($a != ''){
                ...
                $sql = "INSERT INTO tblknowledge_base(articlegroup, subject, description, slug, active, datecreated, article_order, staff_article, type)
                        VALUES
                        ('$num','$a','','$slug_id','1','2022-04-13 04:02:07','0','0','14')";
                if (mysqli_query($connect, $sql))
                {
                    $article_id = mysqli_insert_id($connect);
                    foreach ($BB as $A) {
                        $x = "$A" . $row;
                        $custom_field_title = $worksheet->getCell("$x")->getValue();
                        if ($custom_field_title != '') {
                            $row1 = $row + 1;
                            $x1 = "$A".$row1;
                            $fld_value = $worksheet->getCell("$x1")->getValue();
                            $sql = "INSERT INTO tblknowledge_custom_fields(knowledge_id, title, description)
                                    VALUES
                                    ('$article_id','$custom_field_title','$fld_value')";
                    ...
                    foreach ($AA as $a){
                        foreach ($AA as $b){
                            $i = "$a" . "$b" . $row;
                            $custom_field_title = $worksheet->getCell("$i")->getValue();
                            if($custom_field_title != '') {
                                $x1 = "$a" . "$b" . $row1;
                                $fld_value = $worksheet->getCell("$x1")->getValue();
                                $sql = "INSERT INTO tblknowledge_custom_fields(knowledge_id, title, description)
