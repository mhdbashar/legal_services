...
if (staff_can('view', BABIL_CHAT_MODULE_NAME) && get_option('pusher_chat_enabled') == '1') {
    hooks()->add_action('before_staff_login', 'babilchat_set_session_variable_before_login_for_notification');
    hooks()->add_action('app_admin_head', 'babil_chat_add_head_components');
    hooks()->add_action('app_admin_footer', 'babil_chat_init_checkView');
    hooks()->add_action('app_admin_footer', 'babil_chat_load_js');
    hooks()->add_action('app_admin_head', 'babil_chat_add_js_before_admin_render');
    hooks()->add_filter('migration_tables_to_replace_old_links', 'babil_chat_migration_tables_to_replace_old_links');
    hooks()->add_action('staff_member_deleted', 'babil_chat_staff_member_data_transfer');
...
if (isClientsEnabled() && get_option('pusher_chat_enabled') == '1') {
    hooks()->add_action('before_client_login', 'babilchat_set_session_variable_before_login_for_notification_client');
    hooks()->add_action('app_customers_head', 'handle_clients_css_styles');
    hooks()->add_action('app_customers_footer', 'babil_chat_init_checkViewClients');
}
...
{
    echo '<link href="' . base_url('modules/babilchat/assets/clients/styles.css' . '?v=' . VERSIONING . '') . '"  rel="stylesheet" type="text/css" >';
...
if (staff_can('view', 'settings')) {
    hooks()->add_action('admin_init', 'babilchat_add_settings_tab');
}

/**
 * [babilchat_add_settings_tab net menu item in setup->settings].
 *
 * @return void
 */
function babilchat_add_settings_tab()
{
    $CI = &get_instance();
    $CI->app_tabs->add_settings_tab('babilchat-settings', [
        'name' => '' . _l('chat_settings_name') . '',
        'view' => 'babilchat/babilchat_settings',
        'position' => 36,
    ]);
}
 ...
 * @return void
 */
function babilchat_set_session_variable_before_login_for_notification()
{
    get_instance()->session->set_userdata('babilchat_user_before_login', true);
}
 ...
 * @return void
 */
function babilchat_set_session_variable_before_login_for_notification_client()
{
    get_instance()->session->set_userdata('babilchat_client_before_login', true);
}

/**
 * [babil_chat_load_js inject javascript files].
 *
 * @return void
 */
function babil_chat_load_js()
{
     ...
     * Mentions js
     */
    echo '<script src="' . base_url('modules/babilchat/assets/js/mentions/underscore.js' . '?v=' . VERSIONING . '') . '"></script>';
    echo '<script src="' . base_url('modules/babilchat/assets/js/mentions/jquery-elastic.js' . '?v=' . VERSIONING . '') . '"></script>';
    echo '<script src="' . base_url('modules/babilchat/assets/js/mentions/mentions.js' . '?v=' . VERSIONING . '') . '"></script>';

    if (strpos(CHAT_CURRENT_URI, '/babilchat/babilchat_controller/chat_full_view')) {
        /**
         * Audio record js
         */
        if (isset($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) == 'on') {
            echo '<script src="' . module_dir_url('babilchat', 'assets/js/audio/WebAudioRecorder.min.js' . '?v=' . VERSIONING . '') . '"></script>';
            echo '<script src="' . module_dir_url('babilchat', 'assets/js/audio/WebAudioRecorderOgg.min.js' . '?v=' . VERSIONING . '') . '"></script>';
            echo '<script src="' . module_dir_url('babilchat', 'assets/js/audio/sound_app.js' . '?v=' . VERSIONING . '') . '"></script>';
        }
    }
}
 ...
 * @return array
 */
function babil_chat_migration_tables_to_replace_old_links($tables)
{
    $tables[] = [
 ...
 * @return null
 */
function babil_chat_add_head_components()
{
    if (!strpos($_SERVER['REQUEST_URI'], 'chat_full_view') !== false) {
        echo '<link href="' . base_url('modules/babilchat/assets/css/chat_styles.css' . '?v=' . VERSIONING . '') . '"  rel="stylesheet" type="text/css" >';
    } else {
 ...
 */
function babil_chat_add_js_before_admin_render()
{
    if (!strpos($_SERVER['REQUEST_URI'], 'chat_full_view') !== false) {
        echo '<script src="' . base_url('modules/babilchat/assets/js/jscolor.js' . '?v=' . VERSIONING . '') . '"></script>';
    }
    echo '<script src="' . base_url('modules/babilchat/assets/js/emoparser.js' . '?v=' . VERSIONING . '') . '"></script>';
}
...
function chat_check_theme_options()
{
    if (get_chat_theme_option() == 'light' || get_chat_theme_option() == null) {
        echo '<link href="' . base_url('modules/babilchat/assets/css/chat_full_view.css' . '?v=' . VERSIONING . '') . '"  rel="stylesheet" type="text/css" />';
    } else {
        echo '<link href="' . base_url('modules/babilchat/assets/css/chat_full_dark_view.css' . '?v=' . VERSIONING . '') . '"  rel="stylesheet" type="text/css" />';
    }
    echo '<link href="' . base_url('modules/babilchat/assets/css/lity.css' . '?v=' . VERSIONING . '') . '"  rel="stylesheet" type="text/css" />';
}
 ...
 * @return null
 */
function babil_chat_init_checkView()
{
    $CI = &get_instance();
    $CI->load->model('babilchat/babilchat_model', 'chat_model');
    $unreadMessages = $CI->chat_model->getUnread();

    echo '<script src="' . base_url('modules/babilchat/assets/js/lity.min.js' . '?v=' . VERSIONING . '') . '"></script>';
    echo $CI->load->view('babilchat/initViewCheck', ['unreadMessages' => $unreadMessages], true);
}
 ...
 * @return null
 */
function babil_chat_init_checkViewClients()
{
    $CI = &get_instance();
    echo $CI->load->view('babilchat/initViewCheckClients');
}

/*
    * Function that will convert links to iamges if meets the regex
 */
function babil_chat_convertLinkImageToString($string)
{
    $regexImg = '~(http.*\.)(jpe?g|png|gif|[tg]iff?|svg)~i';

    if (preg_match_all($regexImg, $string)) {
        $string = preg_replace($regexImg, '<a href="' . htmlspecialchars('$0') . '" data-lity><img class="babilchat_convertedImage" src="' . htmlspecialchars('$0') . '"/></a>', $string);
    }
 ...
 * @return string
 */
function babilchat_checkMessageIfFileExists($message)
{
 ...
 * @return boolean
 */
function babil_chat_staff_member_data_transfer($data)
{
    $deleted = $data['id'];
    ...
    foreach ($group_files as $group_file) {
        if (is_dir(BABIL_CHAT_MODULE_GROUPS_UPLOAD_FOLDER)) {
            unlink(BABIL_CHAT_MODULE_GROUPS_UPLOAD_FOLDER . '/' . $group_file['file_name']);
        }
    }

    foreach ($files as $file) {
        if (is_dir(BABIL_CHAT_MODULE_UPLOAD_FOLDER)) {
            unlink(BABIL_CHAT_MODULE_UPLOAD_FOLDER . '/' . $file['file_name']);
        }
    }
...
function get_customer_admins()
{
    ...
    foreach ($customer_admins as $key => &$admin) {
        if (get_option('chat_show_only_users_with_chat_permissions') == 1) {
            if (!staff_can('view', BABIL_CHAT_MODULE_NAME, $admin['staffid'])) {
...
?>
        <div id="babilchat-header-wrapper">
...
{    // Used in chat_full_view as ['prop' => 'class or something else']
    ...
    require('modules/babilchat/assets/module_includes/Components/' . $name . '.php');
}
...
// {    // Used in chat_full_view as ['prop' => 'class or something else']
//     // Then reuse this in the $name.php $params['prop']
//     require('modules/babilchat/assets/module_includes/Components/Javascript/' . $name . '.php');
// }
?>