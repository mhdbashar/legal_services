<script>
    ...
    var customerSettings = {
        ...
        "getStaffUnreadMessages": '<?php echo site_url('babilchat/Babilchat_ClientsController/getStaffUnreadMessages'); ?>',
        "updateStaffUnread": '<?php echo site_url('babilchat/Babilchat_ClientsController/updateClientUnreadMessages'); ?>',
        ...
        "uploadMethod": '<?php echo site_url('babilchat/Babilchat_ClientsController/uploadMethod'); ?>',
    ...
    function createTextLinks_(text)
    {
        ...
        return (text || "").replace(/([^\S]|^)(((https?\:\/\/)|(www\.))(\S+))/gi, function (match, string, url) {
            ...
            if (hyperlink.match(regex)) {
                return string + "<a href=\"" + hyperlink + "\" target=\"blank\" data-lity><img class=\"babilchat_convertedImage\" src=\"" + hyperlink + "\"/></a>";
            } else {
    ...
    function babilchat_setNoMoreMessages()
    {
        if ($("#no_messages").length == 0) {

            $(".client_messages").prepend("<div class=\"text-center mtop5\" id=\"no_messages\">" + babilchatSettings.noMoreMessagesText + "</div>");

            $(".messages").prepend("<div class=\"text-center mtop5\" id=\"no_messages\">" + babilchatSettings.noMoreMessagesText + "</div>");
        }
    }

    // Used in chat_full_view.php
    function babilchat_setNoMoreGroupMessages()
    {
        if ($("#no_messages").length == 0) {
            $(".group_messages .chat_group_messages").prepend("<div class=\"text-center mtop5\" id=\"no_messages\">" + babilchatSettings.noMoreMessagesText + "</div>");
        }
    }

    // Used in chat_clients_view.php
    function babilchat_setNoMoreStaffMessages()
    {
        if ($("#no_messages").length == 0) {
    ...
    $("body").on("click", "._forwardMessage", function () {
        ...
        let messageEscaped = $(this).parents("li").find("p").html();
