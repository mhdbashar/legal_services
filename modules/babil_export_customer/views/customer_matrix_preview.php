...
<?php if(isset($client)){ 
$main_heading = get_option(BABIL_EXPORT_CUSTOMER_MODULE_NAME.'_matrix_print_heading_text');
...
<div class="row">
	...
	<div class="col-md-3">
		<a id="btn_print" href="<?php echo base_url(BABIL_EXPORT_CUSTOMER_MODULE_NAME.'/export_customer_profile/'.$client->userid);?>" target="_blank" class="btn btn-default btn-with-tooltip mright5 pull-right" data-toggle="tooltip" title="<?php echo _l('print'); ?>" data-placement="bottom">
			<i class="fa fa-print"></i>
		</a>
		<a id="btn_pdf" href="<?php echo base_url(BABIL_EXPORT_CUSTOMER_MODULE_NAME.'/export_customer_profile/'.$client->userid.'?download=1');?>"  class="btn btn-default btn-with-tooltip mright5 pull-right" data-toggle="tooltip" title="<?php echo _l('view_pdf'); ?>" data-placement="bottom">
...
"use strict";
	...
	function render_customer_matrix(){
		...
		var statementUrl = '<?php echo base_url(BABIL_EXPORT_CUSTOMER_MODULE_NAME.'/get_customer_matrix');?>/'+customer_id;
	...
	$.get(statementUrl,function(response){
		...
		$('#btn_print').attr('href',buildUrl('<?php echo base_url(BABIL_EXPORT_CUSTOMER_MODULE_NAME.'/export_customer_matrix/');?>'+customer_id,statementUrlParams));
		statementUrlParams['download'] = true;
		$('#btn_pdf').attr('href',buildUrl('<?php echo base_url(BABIL_EXPORT_CUSTOMER_MODULE_NAME.'/export_customer_matrix/');?>'+customer_id,statementUrlParams));
