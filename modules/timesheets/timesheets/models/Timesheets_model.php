	...
	public function delete_requisition($id){
		...
		if($rel_type){
			...
			if($rel_type->rel_type == '1'){
				if($rel_type->type_of_leave == '8'){
					// Get total leave in year of staff
					$day_off = $this->get_day_off($rel_type->staff_id);
					// Number of leaving day
					$dd = $rel_type->number_of_leaving_day;

					$update_days_off = $day_off->days_off - $dd;
					$update_remain = $day_off->total - $update_days_off;

					$this->db->where('staffid', $rel_type->staff_id);
					$this->db->where('year', date('Y'));
					$this->db->update(db_prefix().'timesheets_day_off',[
						'remain' =>  $update_remain,
						'days_off' => $update_days_off
...
public function format_date_time($date){
	...
	public function get_day_off_staff_by_date($staff,$date){     
		if($staff!='' && $date != ''){
			...
			$query = 'select * from '.db_prefix().'day_off where '.$add_query.' break_date = \''.$date.'\'';
...
public function get_timesheet($staffid='', $from_date, $to_date){
	...
	public function get_taskstimers($staff_id, $where){
		...
		$this->db->select('*, CASE
			...
			END as total_logged_time, from_unixtime(start_time, \'%Y-%m-%d\') as start_time, from_unixtime(end_time, \'%Y-%m-%d\') as end_time');
		$this->db->order_by('id', 'desc');
...
public function get_attendance_task($staffs_list, $month = '', $year = '', $from_date = '', $to_date = ''){
	...
	foreach($staffs_list as $s){
		...
		foreach ($list_date as $key => $value) {
			...
			if($working_hour > $max_hour){
				$working_hour = $max_hour;
			}
				...
				if(!$check_holiday){
					...
					if ($ts_type){
						foreach ($ts_type as $kts => $ts_row) {
							...
							$ts_lack .= $ts_row['type'].':'.round($ts_row['value']).'; ';
						}
					}
...
public function get_attendance_manual($staffs_list, $month = '', $year = '', $from_date = '', $to_date = ''){
  ...
  function check_check_out($staff, $date){
  	$obj = new stdClass();
  	$type_check_in_out = '';
  	$data_check_in_out = $this->db->query('select id, type_check, date from '.db_prefix().'check_in_out where staff_id = '.$staff.' and date(date) = "'.$date.'" order by id desc limit 1')->row();
...
function has_check_in($staff, $date){
	$data_check_in_out = $this->db->query('select id '.db_prefix().'check_in_out where staff_id = '.$staff.' and date(date) = "'.$date.'" and type_check = 1 order by id desc limit 1')->row();
	if($data_check_in_out){
		return true;  		
	}
	return false;
