...
//         }
    ...
    private function regular_durations_reminders()
    {
        ...
        $this->db->where('reg_id IS NOT NULL');
        $this->db->where('deadline_notified', 0);
        $cases = $this->db->get(db_prefix() . 'cases_regular_durations')->result_array();
        $now = new DateTime(date('Y-m-d'));
        $notifiedUsers = [];
        foreach ($cases as $case) {

            if ($case['end_date'] < $now) {
            ...
            if ($case['end_date'] > date('Y-m-d')) {
                $end_date = new DateTime($case['end_date']);
                $diff = $end_date->diff($now)->format('%a');
                // Check if difference between start date and end_date is the same like the reminder before
    ...
    public function fix_and_separate_names()
    {
...
<td style="width: 25%; height: 17px; border-style: none;"><strong>رابط القضية</strong></td></tr>';
            ...
            $sql1 = "select * from tblmy_cases where id  in (select project_id from tblcase_activity  )";
            $info = $this->db->query($sql1)->result_array();
            foreach ($info as $data) {
                $sql2 = 'select project_id, MAX(dateadded) AS maxdate from tblcase_activity where project_id = "' . $data['id'] . '" ';
                $result = $this->db->query($sql2)->result_array();
                foreach ($result as $data) {
                    $subb = '';

                    $date_max = new DateTime($data['maxdate']);
                    $subb = $date_max->diff($now)->format("%a");

                    if (($subb >= $daily_agenda_day) && ($now->format("Y-m-d") > $date_max->format("Y-m-d"))) {
                        $sql3 = 'select * from tblmy_cases where id=' . $data['project_id'] . '';
...
<td style="width: 25%; height: 17px; border-style: none;"><strong>رابط القضية</strong></td></tr>';
            ...
            $sql1 = "select * from tblmy_disputes_cases where id  in (select project_id from tblcase_activity  )";
            $info = $this->db->query($sql1)->result_array();
            foreach ($info as $data) {
                $sql2 = 'select project_id, MAX(dateadded) AS maxdate from tblcase_activity where project_id = "' . $data['id'] . '" ';
                $result = $this->db->query($sql2)->result_array();
                foreach ($result as $data) {
                    ...
                    $max_date = new DateTime($data['maxdate']);
                    $subb = $max_date->diff($now)->format("%a");

                    if (($subb >= $daily_agenda_day) && ($now->format("Y-m-d") > $max_date->format("Y-m-d"))) {
                        $sql3 = 'select * from tblmy_disputes_cases where id=' . $data['project_id'] . '';
                        $cases_info = $this->db->query($sql3)->row();
