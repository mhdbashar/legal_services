...
<?php } ?>
<a href="#" class="btn btn-default" onclick="milestones_oservice_switch_view(<?php echo $ServID; ?>, '<?php echo $service->slug; ?>'); return false;"><i class="fa fa-th-list"></i></a>
<?php if($milestones_found) { ?>
    <div id="kanban-params" class="pull-right">
        <div class="checkbox">
            <input type="checkbox" value="yes" id="exclude_completed_tasks" name="exclude_completed_tasks"<?php if($milestones_exclude_completed_tasks){echo ' checked';} ?> onclick="window.location.href = '<?php echo admin_url('SOther/view/'.$ServID.'/'.$project->id.'?group=project_milestones&exclude_completed='); ?>'+(this.checked ? 'yes' : 'no')">
...
<div id="milestones-table" class="hide mtop25">
    ...
    ),'milestones_oservice', [], $table_attributes); ?>
</div>
