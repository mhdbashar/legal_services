...
class Babil_export_customer extends AdminController
{
	public function __construct()
	{
		parent::__construct(); 
		$this->load->model('babil_export_customer_model');
		if (!is_admin() && !has_permission('babil_export_customer', '', 'view') && !has_permission('babil_export_customer_matrix', '', 'view') && !has_permission('babil_export_customer_services', '', 'view')) {
			access_denied(_l('babil_export_customer'));
		}
	}
	
	public function export_customer_profile($id)
	{
		if (has_permission('babil_export_customer', '', 'view')) {
			$pdf = app_pdf('babil-customer-profile', APP_MODULES_PATH.BABIL_EXPORT_CUSTOMER_MODULE_NAME.'/libraries/pdf/Babil_customer_profile_pdf', $id);
	...
	{
		if (has_permission('babil_export_customer_matrix', '', 'view')) {
			$pdf = app_pdf('babil-customer-matrix', APP_MODULES_PATH.BABIL_EXPORT_CUSTOMER_MODULE_NAME.'/libraries/pdf/Babil_customer_matrix_pdf', $id);
			$client = $this->clients_model->get($id);
			if (!$client) {
				show_404();
			}
	...
	public function save_customer_files($id)
	{
		...
		$this->babil_export_customer_model->update_client_kyc($data,$id);
	...
	public function get_customer_matrix($customer_id)
	{
		...
		$data['invoices'] = $this->babil_export_customer_model->get_invoices_total($inputs);
		...
		$data['from'] = $from;
		$data['to']   = $to;
		$viewData['html'] = $this->load->view('babil_export_customer/includes/_get_client_matrix_data', $data, true);
	...
	{
		if (!has_permission('babil_export_customer_services', '', 'view')) {
			header('HTTP/1.0 400 Bad error');
			echo _l('access_denied');
			die;
		}
		...
		if($filter_by=='customer')
		{
			...
			{
				$result = $this->babil_export_customer_model->get_customer_services_list($filter_id,$group_id,$staff_id);
		...
		if($filter_by=='service')
		{
			...
			{
				$result = $this->babil_export_customer_model->get_service_customers_list($filter_id,$group_id,$staff_id);
				foreach($result as $row)
				{
					$by='';
					if($group_by =='customer')
		...
		$data['title']    = _l('babil_customer_services');
