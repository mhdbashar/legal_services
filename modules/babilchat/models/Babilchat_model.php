...
class Babilchat_model extends App_Model
{
    /**
    ...
    public function getUsers()
    {
        ...
        foreach ($users as $key => $user) {
            ...
            if (get_option('chat_show_only_users_with_chat_permissions') == 1) {
                if (!staff_can('view', BABIL_CHAT_MODULE_NAME, $user['staffid'])) {
                    unset($users[$key]);
                    continue;
                }
            }
    ...
    public function getMessages($from, $to, $limit, $offset)
    {
        ...
        foreach ($query as &$chat) {
            $chat->message = babil_chat_convertLinkImageToString($chat->message);
        ...
        if (!empty($files) && is_array($files)) {
            foreach ($files as $file) {
                if (is_dir(BABIL_CHAT_MODULE_GROUPS_UPLOAD_FOLDER)) {
                    unlink(BABIL_CHAT_MODULE_GROUPS_UPLOAD_FOLDER . '/' . $file['file_name']);
                }
            }
            $this->db->where('group_id', $group_id);
    ...
    public function deleteClientMessage($message_id)
    {
        ...
        if (babilchat_checkMessageIfFileExists($possible_file->message)) {
            ...
            if ($file_name == 'audio&amp;gt;') {
                ...
                if (strlen($file_name)) {
                    if (is_dir(BABIL_CHAT_MODULE_UPLOAD_FOLDER . '/audio')) {
                        @unlink(BABIL_CHAT_MODULE_UPLOAD_FOLDER . '/audio/' . $file_name);
                    }
                }
            } else {
                if (is_dir(BABIL_CHAT_MODULE_UPLOAD_FOLDER)) {
                    unlink(BABIL_CHAT_MODULE_UPLOAD_FOLDER . '/' . $file_name);
                }
            }
        }
    ...
    public function deleteMessage($id, $mixed_id)
    {
        ...
        if (strpos($mixed_id, 'group_id') !== false) {
            ...
            if (babilchat_checkMessageIfFileExists($possible_file->message)) {
                ...
                if (is_dir(BABIL_CHAT_MODULE_GROUPS_UPLOAD_FOLDER)) {
                    unlink(BABIL_CHAT_MODULE_GROUPS_UPLOAD_FOLDER . '/' . $file_name);
                }
            ...
            if (babilchat_checkMessageIfFileExists($possible_file)) {
    ...
    private function handleAudioDeleteFile($possible_file)
    {
        ...
        if (preg_match('~\b(src|audio|controls|.ogg)\b~i', $possible_file)) {
            ...
            if (isset($matches[0]) && !empty($matches[0])) {
                ...
                if (is_dir(BABIL_CHAT_MODULE_UPLOAD_FOLDER . '/audio')) {
                    @unlink(BABIL_CHAT_MODULE_UPLOAD_FOLDER . '/audio/' . $finallyFileToDelete);
                }
            }
        }
    ...
    public function deleteMutualConversation($id, $table)
    {
        ...
        if ($possible_files) {
            foreach ($possible_files as $file) {
                if (babilchat_checkMessageIfFileExists($file['message'])) {
    ...
    public function purgeConversations($type)
    {
        ...
        if ($type == 'all') {
            try {
                ...
                $folder_path = BABIL_CHAT_MODULE_UPLOAD_FOLDER;
    ...
    public function get_shared_files_and_create_template($own_id, $contact_id)
    {
        ...
        $dir = list_files(BABIL_CHAT_MODULE_UPLOAD_FOLDER);
        ...
        foreach ($files as $file) {
            if (preg_match("/^[^\?]+\.('" . $photoExtensions . "')$/", $file)) {
                $html .= "<a data-lity href='" . base_url('modules/babilchat/uploads/' . $file) . "'>
               <div class='col-xs-3 shared_files_ahref' style='background-image:url(" . base_url('modules/babilchat/uploads/' . $file) . ");'></div></a>";
            }
        }
        $html .= '</div>';
        ...
        foreach ($files as $file) {
            ...
            if (preg_match("/^[^\?]+\.('" . $docFiles . "')$/", $file)) {
                $html .= "<div class='col-md-12'><a target='_blank' " . $data_lity . " href ='" . base_url('modules/babilchat/uploads/' . $file) . "'><i class='fa fa-file-o icon_shared_files' aria-hidden='true'></i>" . $file . '</a></div>';
            }
        }
        $html .= '</div></div>';
    ...
    public function get_group_shared_files_and_create_template($group_id)
    {
        ...
        $dir = list_files(BABIL_CHAT_MODULE_GROUPS_UPLOAD_FOLDER);
        ...
        foreach ($files as $file) {
            if (preg_match("/^[^\?]+\.('" . $photoExtensions . "')$/", $file)) {
                $html .= "<a data-lity href='" . base_url('modules/babilchat/uploads/groups/' . $file) . "'>
               <div class='col-md-3 shared_files_ahref' style='background-image:url(" . base_url('modules/babilchat/uploads/groups/' . $file) . ");'></div></a>";
            }
        }
        $html .= '</div>';
        ...
        foreach ($files as $file) {
            ...
            if (preg_match("/^[^\?]+\.('" . $docFiles . "')$/", $file)) {
                $html .= "<div class='col-md-12'><a target='_blank' " . $data_lity . " href ='" . base_url('modules/babilchat/uploads/groups/' . $file) . "'><i class='fa fa-file-o icon_shared_files' aria-hidden='true'></i>" . $file . '</a></div>';
            }
        }
        $html .= '</div></div>';
    ...
    public function handleMentionEvent($data, $pusher)
    {
        ...
        foreach ($data['users'] as $user) {
            ...
            add_notification([
                ...
                'fromcompany' => true,
                'link'        => 'babilchat/Babilchat_Controller/chat_full_view',
            ]);
        }
    ...
    public function handleAudioData($audioB64Data)
    {
        ...
        $fileSaveLocation = BABIL_CHAT_MODULE_UPLOAD_FOLDER . "/audio/{$hashFilename}.ogg";
