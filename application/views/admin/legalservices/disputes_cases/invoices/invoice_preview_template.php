...
<div class="panel_s no-padding mbot5">
   <div class="panel-body">
      ...
      <p>
         <a href="<?php echo admin_url('legalservices/disputes_invoices/list_invoices/'.$_inv->id); ?>" target="_blank"><?php echo disputes_format_invoice_number($_inv->id); ?></a> - <?php echo app_format_money($_inv->total,$_inv->currency_name); ?>
         <span class="pull-right text-<?php echo disputes_get_invoice_status_label($_inv->status); ?>">
         <?php echo disputes_format_invoice_status($_inv->status,'',false); ?>
         </span>
      </p>
      <?php } ?>
   </div>
...
<div class="col-md-12 no-padding">
   <div class="panel_s">
      <div class="panel-body">
         ...
         <div class="row">
            <div class="col-md-3">
               <?php echo disputes_format_invoice_status($invoice->status,'mtop5'); ?>
            ...
            <div class="col-md-9 _buttons">
               ...
               <div class="pull-right">
                  ...
                  <a href="<?php echo admin_url('legalservices/disputes_invoices/invoice/'.$invoice->id); ?>" data-toggle="tooltip" title="<?php echo _l('edit_invoice_tooltip'); ?>" class="btn btn-default btn-with-tooltip" data-placement="bottom"><i class="fa fa-pencil-square-o"></i></a>
                  <?php } ?>
                  <div class="btn-group">
                     <a href="#" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-file-pdf-o"></i><?php if(is_mobile()){echo ' PDF';} ?> <span class="caret"></span></a>
                     <ul class="dropdown-menu dropdown-menu-right">
                        <li class="hidden-xs"><a href="<?php echo admin_url('legalservices/disputes_invoices/pdf/'.$invoice->id.'?output_type=I'); ?>"><?php echo _l('view_pdf'); ?></a></li>
                        <li class="hidden-xs"><a href="<?php echo admin_url('legalservices/disputes_invoices/pdf/'.$invoice->id.'?output_type=I'); ?>" target="_blank"><?php echo _l('view_pdf_in_new_window'); ?></a></li>
                        <li><a href="<?php echo admin_url('legalservices/disputes_invoices/pdf/'.$invoice->id); ?>"><?php echo _l('download'); ?></a></li>
                        <li>
                           <a href="<?php echo admin_url('legalservices/disputes_invoices/pdf/'.$invoice->id.'?print=true'); ?>" target="_blank">
                           <?php echo _l('print'); ?>
                           </a>
                        </li>
                     </ul>
                  ...
                  <div class="btn-group">
                     ...
                     <ul class="dropdown-menu dropdown-menu-right">
                        <li><a href="<?php echo site_url('disputes_invoice/index/' . $invoice->id . '/' .  $invoice->hash) ?>" target="_blank"><?php echo _l('view_invoice_as_customer_tooltip'); ?></a></li>
                        <li>
                           <?php hooks()->do_action('after_invoice_view_as_client_link', $invoice); ?>
                           <?php if(($invoice->status == 4
                              || ($invoice->status == 3 && !empty($invoice->duedate) && $invoice->duedate && date('Y-m-d') > date('Y-m-d',strtotime(to_sql_date($invoice->duedate)))))
                              && disputes_is_invoices_overdue_reminders_enabled()){ ?>
                           <a href="<?php echo admin_url('legalservices/disputes_invoices/send_overdue_notice/'.$invoice->id); ?>"><?php echo _l('send_overdue_notice_tooltip'); ?></a>
                           <?php } ?>
                        </li>
                        ...
                        <li>
                           <a href="<?php echo admin_url('legalservices/disputes_invoices/copy/'.$invoice->id); ?>"><?php echo _l('invoice_copy'); ?></a>
                        </li>
                        <?php } ?>
                        <?php if($invoice->sent == 0){ ?>
                        <li>
                           <a href="<?php echo admin_url('legalservices/disputes_invoices/mark_as_sent/'.$invoice->id); ?>"><?php echo _l('invoice_mark_as_sent'); ?></a>
                        </li>
                        <?php } ?>
                        ...
                        <li>
                           <?php if($invoice->status != 5
                              && $invoice->status != 2
                              && $invoice->status != 3){ ?>
                           <a href="<?php echo admin_url('legalservices/disputes_invoices/mark_as_cancelled/'.$invoice->id); ?>"><?php echo _l('invoice_mark_as',_l('invoice_status_cancelled')); ?></a>
                           <?php } else if($invoice->status == 5) { ?>
                           <a href="<?php echo admin_url('legalservices/disputes_invoices/unmark_as_cancelled/'.$invoice->id); ?>"><?php echo _l('invoice_unmark_as',_l('invoice_status_cancelled')); ?></a>
                           <?php } ?>
                        </li>
                        <?php } ?>
                        <?php if(!in_array($invoice->status, array(2, 5, 6))
                           && has_permission('invoices','','edit')
                           && $invoice->duedate
                           && disputes_is_invoices_overdue_reminders_enabled()){ ?>
                        <li>
                           <?php if($invoice->cancel_overdue_reminders == 1) { ?>
                           <a href="<?php echo admin_url('legalservices/disputes_invoices/resume_overdue_reminders/'.$invoice->id); ?>"><?php echo _l('resume_overdue_reminders'); ?></a>
                           <?php } else { ?>
                           <a href="<?php echo admin_url('legalservices/disputes_invoices/pause_overdue_reminders/'.$invoice->id); ?>"><?php echo _l('pause_overdue_reminders'); ?></a>
                           <?php } ?>
                        </li>
                        <?php } ?>
                        <?php
                           if((get_option('delete_only_on_last_invoice') == 1 && disputes_is_last_invoice($invoice->id)) || (get_option('delete_only_on_last_invoice') == 0)){ ?>
                        <?php if(has_permission('invoices','','delete')){ ?>
                        <li data-toggle="tooltip" data-title="<?php echo _l('delete_invoice_tooltip'); ?>">
                           <a href="<?php echo admin_url('legalservices/disputes_invoices/delete/'.$invoice->id); ?>" class="text-danger delete-text _delete"><?php echo _l('delete_invoice'); ?></a>
                        </li>
                        <?php } ?>
                        <?php } ?>
                     </ul>
                  ...
                  <a href="#" onclick="record_payment_disputes(<?php echo $invoice->id; ?>); return false;"  class="mleft10 pull-right btn btn-success<?php if($invoice->status == 2 || $invoice->status == 5){echo ' disabled';} ?>">
         ...
         <div class="tab-content">
            ...
            <div role="tabpanel" class="tab-pane" id="tab_child_invoices">
               ...
               <ul class="list-group">
                  ...
                  <li class="list-group-item">
                     <a href="<?php echo admin_url('legalservices/disputes_invoices/list_invoices/'.$recurring->id); ?>" onclick="init_invoice_disputes(<?php echo $recurring->id; ?>); return false;" target="_blank"><?php echo disputes_format_invoice_number($recurring->id); ?>
                     ...
                     <?php echo disputes_format_invoice_status($recurring->status,'',false); ?>
                     </span>
                  </li>
                  <?php } ?>
               </ul>
            ...
            <div role="tabpanel" class="tab-pane" id="tab_notes">
               <?php echo form_open(admin_url('legalservices/disputes_invoices/add_note/'.$invoice->id),array('id'=>'sales-notes','class'=>'invoice-notes-form')); ?>
                                 ...
                                 if(!empty($activity['additional_data'])){
                                  ...
                                  foreach($additional_data as $data){
                                    if(strpos($data,'<original_status>') !== false){
                                      ...
                                      $additional_data[$i] = disputes_format_invoice_status($original_status,'',false);
                                    } else if(strpos($data,'<new_status>') !== false){
                                      $new_status = get_string_between($data, '<new_status>', '</new_status>');
                                      $additional_data[$i] = disputes_format_invoice_status($new_status,'',false);
...
<?php $this->load->view('admin/legalservices/disputes_cases/invoices/invoice_send_to_client'); ?>
...
<script>
   ...
   <?php if($record_payment) { ?>
      record_payment_disputes(<?php echo $invoice->id; ?>);
   <?php } ?>
</script>
