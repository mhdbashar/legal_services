...
$aColumns = [
    '1',
    db_prefix() .'my_procurationstate.id as id',
    'procurationstate',
];
...
foreach ($custom_fields as $key => $field) {
    ...
    array_push($join, 'LEFT JOIN ' . db_prefix() . 'customfieldsvalues as ctable_' . $key . ' ON ' . db_prefix() . 'my_procurationstate.id = ctable_' . $key . '.relid AND ctable_' . $key . '.fieldto="' . $field['fieldto'] . '" AND ctable_' . $key . '.fieldid=' . $field['id']);
}

$where  = [];
$filter = [];
$sIndexColumn = 'id';
$sTable       = db_prefix() . 'my_procurationstate';
