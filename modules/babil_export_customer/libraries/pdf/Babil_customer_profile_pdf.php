...
class Babil_customer_profile_pdf extends App_pdf
{
	protected $client_id;
	...
	public function prepare()
	{
		...
		{
			$profile_logo = $this->ci->babil_export_customer_model->get_customer_files($client->userid,array($client_kyc_detail['profile_file_id']));
			$data['profile_logo']=(!empty($profile_logo)?$path.$profile_logo[0]['file_name']:'');
			$files = unserialize($client_kyc_detail['files_id']);
			if(!empty($files))
			{
				$data['files']=array();
				$files_list = $this->ci->babil_export_customer_model->get_customer_files($client->userid,$files);
	...
	{
		return 'babil-customer-profile';
	}
	
	protected function file_path()
	{
		$customPath = APP_MODULES_PATH.BABIL_EXPORT_CUSTOMER_MODULE_NAME . '/views/my_customer_export_profile_pdf.php';
		$actualPath = APP_MODULES_PATH.BABIL_EXPORT_CUSTOMER_MODULE_NAME . '/views/customer_export_profile_pdf.php';
	...
	{
		return  [
			'orientation' => (get_option(BABIL_EXPORT_CUSTOMER_MODULE_NAME.'_print_orientation') == 'L'?'L':'P'),
			'format'      => (get_option(BABIL_EXPORT_CUSTOMER_MODULE_NAME.'_print_orientation') == 'L'?'Landscape':'Portrait'),
		];
	}
}
