...
$aColumns = [
    'subject',
    'last_activity',
    '(SELECT COUNT(*) FROM ' . db_prefix() . 'oservicediscussioncomments WHERE discussion_id = ' . db_prefix() . 'oservicediscussions.id AND discussion_type="regular") as totalComments',
    'show_to_customer',
];
$sIndexColumn = 'id';
...
foreach ($rResult as $aRow) {
    ...
    $subject = '<a href="' . admin_url('SOther/view/' . $ServID . '/' . $project_id . '?group=project_discussions&discussion_id=' . $aRow['id']) . '">' . $aRow['subject'] . '</a>';
    ...
    if (has_permission('projects', '', 'edit') || has_permission('projects', '', 'delete')) {
        ...
        if (has_permission('projects', '', 'delete')) {
            $subject .= (has_permission('projects', '', 'edit') ? ' | ' : '') . '<a href="#" onclick="delete_oservice_discussion(' . $aRow['id'] . ') return false;" class="text-danger">' . _l('delete') . '</a>';
        }
        $subject .= '</div>';
    }
