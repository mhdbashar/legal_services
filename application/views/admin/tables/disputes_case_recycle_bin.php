...
$where  = [];
$filter = [];
array_push($where, 'AND ' . db_prefix() . 'my_disputes_cases.deleted = 1');
$sIndexColumn = 'id';
...
foreach ($rResult as $aRow) {
    ...
    if (has_permission('legal_recycle_bin', '', 'delete')) {
        $_data .= '<a href=' . admin_url("Disputes_cases/delete/" . $ServID . '/' . $aRow['id'] . "") . ' class="btn btn-danger btn-icon _delete" data-toggle="tooltip" data-placement="top" title="' . _l('delete') . '"><i class="fa fa-trash"></i></a>';
    }
    $row[] = $_data;
    $output['aaData'][] = $row;
    $i++;
