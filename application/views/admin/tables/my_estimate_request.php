...
foreach ($rResult as $aRow) {
    ...
    $row[] = '<span data-toggle="tooltip" data-title="' . _dt($CI->app_modules->is_active('hijri') ? _d($aRow['date_added']) . '<br>' . to_hijri_date(_d($aRow['date_added'])) : _d($aRow['date_added'])) . '" class="text-has-action is-date">' . time_ago($CI->app_modules->is_active('hijri') ? _d($aRow['date_added']) . '<br>' . to_hijri_date(_d($aRow['date_added'])) : _d($aRow['date_added'])) . '</span>';
