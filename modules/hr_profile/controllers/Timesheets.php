...
//abdo
    ...
    public function check_code($code){
        $validate_code=$this->timesheets_model->validate_code($code);
             if(is_object($validate_code)==true){
    ...
    public function leave()
    {
        ...
        $this->load->view('hr_profile/manage_leave', $data);
    }
    ...
    public function add_requisition_ajax1()
    {
        ...
        if ($this->input->post()) {
            ...
            redirect(admin_url('hr/core_hr/vacations'));
...
//        echo '<pre>';print_r($data);exit();
    ...
    {
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data(module_views_path('hr_profile', 'approval_process/table_approval_process'));
        }
    }
    ...
    public function get_data_type_of_leave($id)
    {
        ...
        $type_of_leave_allocation = $this->timesheets_model->get_type_of_leave_allocation($id);
        $manager_sorts = $this->timesheets_model->get_manager_sorts($id);
        $array = explode(',', $manager_sorts->manager_sorts);
	...
	<div class="modal-body">';
        $html .= '<table class="table border table-striped margin-top-0"><tbody>';
        foreach ($type_of_leave_allocation as $type) {
            $html .= '
                <tr class="project-overview" >
                <td class="bold" width="30%">' . _l('number of days') . '</td>
                <td>' . ($type['days']) . '</td>
                <td class="bold" width="30%">' . _l('percent') . '</td>
                <td>' . ($type['percent']) . '</td>
                </tr>
		...
		<tr class="project-overview">
		<td class="bold">' . _l('manager_sorts') . '</td>
    ...
    {
        $this->app->get_table_data(module_views_path('hr_profile', 'route_management/table_route'));
    }
    ...
    public function get_next_shift_date($staff_id, $date)
    {
        echo json_encode([
            'next_shift_date' => _d($this->timesheets_model->get_next_shift_date($staff_id, $date))
        ]);
    }
