                        ...
                        if (is_array($groups) && !empty($groups)) {
                            foreach ($groups as $group) : ?>
                                <li id="<?= $group['group_id'] ?>">
                                    <img class="_group_image" src="<?= module_dir_url('babilchat', 'assets/chat_implements/icons/groups.png'); ?>" data-toggle="tooltip" data-title="<?= $group['group_name'] ?>">
...
<script>
    ...
    function _forwardTo(receiver_id, el, target)
    {
        ...
        if (target === null) {
            // Send to staff
            $.post(babilchatSettings.serverPath, {
                from: userSessionId,
                to: receiver_id,
                msg: message,
                typing: false
