            ...
            <div class="col-md-5 left-column">
                <div class="panel_s">
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            ...
                            <select required="required" class="form-control" id="clientid" name="client" placeholder="<?php echo _l('staff') ?>" aria-invalid="false">
                                ...
                                foreach ($staffs as $value) {
                                    $is_selected = '';
                                    if($value['staffid'] == $selected){
                                        $is_selected = 'selected';
                                    }
                                    ?>
                                    <option <?php echo $is_selected ?> value="<?php echo $value['staffid'] ?>">
                        ...
                        // Show selector only if contract is already added and there is no client linked to the contract or isset customer id
                        ...
                        <div class="panel-body">
                                    ...
                                    <ul class="nav nav-tabs tabs-in-body-no-margin contract-tab nav-tabs-horizontal mbot15" role="tablist">
                                        ...
                                        <li role="presentation">
                                            <a href="#tab_comments" aria-controls="tab_comments" role="tab" data-toggle="tab" onclick="get_contract_comments(); return false;">
                                                ...
                                                <?php
                                                $totalComments = total_rows(db_prefix().'hr_contract_comments','contract_id='.$contract->id)
                                                ?>
                                        ...
                                        <li role="presentation" class="tab-separator">
                                            <a href="#tab_tasks" aria-controls="tab_tasks" role="tab" data-toggle="tab" onclick="init_rel_tasks_table(<?php echo $contract->id; ?>,'hr_contract'); return false;">
                                                <?php echo _l('tasks'); ?>
                                            </a>
                                        </li>
                                        <li role="presentation" class="tab-separator">
                                            <a href="#tab_notes" onclick="get_sales_notes(<?php echo $contract->id; ?>,'hr/contracts'); return false" aria-controls="tab_notes" role="tab" data-toggle="tab">
                                        ...
                                        <div class="col-md-12 text-right _buttons">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-right">
                                                    <li class="hidden-xs"><a href="<?php echo admin_url('hr/contracts/pdf/'.$contract->id.'?output_type=I'); ?>"><?php echo _l('view_pdf'); ?></a></li>
                                                    <li class="hidden-xs"><a href="<?php echo admin_url('hr/contracts/pdf/'.$contract->id.'?output_type=I'); ?>" target="_blank"><?php echo _l('view_pdf_in_new_window'); ?></a></li>
                                                    <li><a href="<?php echo admin_url('hr/contracts/pdf/'.$contract->id); ?>"><?php echo _l('download'); ?></a></li>
                                                    <li>
                                                        <a href="<?php echo admin_url('hr/contracts/pdf/'.$contract->id.'?print=true'); ?>" target="_blank">
                                                            <?php echo _l('print'); ?>
                                                        </a>
                                                    </li>
                                                </ul>
                           ...
                           <i class="fa fa-envelope"></i></span>
                                ...
                                <div role="tabpanel" class="tab-pane" id="tab_notes">
                                    <?php echo form_open(admin_url('hr/contracts/add_note/'.$contract->id),array('id'=>'contracts-notes','class'=>'hr-contract-notes-form')); ?>
                                ...
                                <div role="tabpanel" class="tab-pane<?php if($this->input->get('tab') == 'renewals'){echo ' active';} ?>" id="renewals">
                                    <?php if(has_permission('hr', '', 'create') || has_permission('hr', '', 'edit')){ ?>
                                        <div class="_buttons">
                                            ...
                                            <div class="media-body">
                                                <div class="display-block">
                                                    ...
                                                    <?php if($renewal['renewed_by_staff_id'] == get_staff_user_id() || is_admin()){ ?>
                                                        <a href="<?php echo admin_url('hr/contracts/delete_renewal/'.$renewal['id'] . '/'.$renewal['contractid']); ?>" class="pull-right _delete text-danger"><i class="fa fa-remove"></i></a>
                                                        <br />
                                                    <?php } ?>
                              ...
                              <?php echo _l('contract_renewal_new_value', app_format_money($renewal['new_value'], $base_currency)); ?>
                                ...
                                <div role="tabpanel" class="tab-pane" id="tab_tasks">
                                    <?php hr_init_relation_tasks_table(array('data-new-rel-id'=>$contract->id,'data-new-rel-type'=>'hr_contract')); ?>
                                </div>
...
<script>
    ...
    $(function () {
        ...
        $("body").on('submit', '#contracts-notes', function () {
            ...
            $.post(form.attr('action'), $(form).serialize()).done(function (rel_id) {
                ...
                // Reload the notes
                if (form.hasClass('hr-contract-notes-form')) {
                    get_sales_notes(rel_id, 'hr/contracts');
                }
            });
            return false;
        });
        ...
        if (typeof (Dropbox) != 'undefined' && $('#dropbox-chooser').length > 0) {
            document.getElementById("dropbox-chooser").appendChild(Dropbox.createChooseButton({
                success: function (files) {
                    $.post(admin_url + 'hr/contracts/add_external_attachment', {
                        files: files,
    ...
    function save_contract_content(manual) {
        ...
        $.post(admin_url + 'hr/contracts/save_contract_data', data).done(function (response) {
    ...
    function delete_contract_attachment(wrapper, id) {
        if (confirm_delete()) {
            $.get(admin_url + 'hr/contracts/delete_contract_attachment/' + id, function (response) {
    ...
    function add_contract_comment() {
        ...
        $.post(admin_url + 'hr/contracts/add_comment', data).done(function (response) {
            response = JSON.parse(response);
    ...
    function get_contract_comments() {
        ...
        requestGet('hr/contracts/get_comments/' + contract_id).done(function (response) {
    ...
    function edit_contract_comment(id) {
        ...
        if (content != '') {
            $.post(admin_url + 'hr/contracts/edit_comment/' + id, {
                content: content
    ...
    function contractGoogleDriveSave(pickData) {
        ...
        data.files = pickData;
        $.post(admin_url + 'hr/contracts/add_external_attachment', data).done(function () {
            var location = window.location.href;
            window.location.href = location.split('?')[0] + '?tab=attachments';
        });
    }
