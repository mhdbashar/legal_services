...
$aColumns = [
    'subject',
    'last_activity',
    '(SELECT COUNT(*) FROM ' . db_prefix() . 'my_disputes_casediscussioncomments WHERE discussion_id = ' . db_prefix() . 'my_disputes_casediscussions.id AND discussion_type="regular") as totalComments',
    'show_to_customer',
];
$sIndexColumn = 'id';
$sTable = db_prefix() . 'my_disputes_casediscussions';
...
foreach ($rResult as $aRow) {
    ...
    $subject = '<a href="' . admin_url('Disputes_cases/view/' . $ServID . '/' . $project_id . '?group=project_discussions&discussion_id=' . $aRow['id']) . '">' . $aRow['subject'] . '</a>';
