 ...
 */
  ...
  public function send_request_approve($data, $staff_id = '') {
    ...
    $staff_addedfrom = isset($data['addedfrom']) ? $data['addedfrom'] : '';
    $date_send = date('Y-m-d H:i:s');
    $rel_type = isset($data['rel_type']) ? $data['rel_type'] : '';
    $rel_id = isset($data['rel_id']) ? $data['rel_id'] : '';

    $data_new = $this->get_approve_setting($rel_type, true, $staff_addedfrom);
    $data_setting = $this->get_approve_setting($rel_type, false, $staff_addedfrom);

    if (!$data_new) {
        $this->update_approve_request($rel_id, $rel_type, 1);
        return false;
    }
