...
<?php if (isset($client)) {
    $CI = &get_instance();
    $services = $CI->db->get_where('my_basic_services', ['is_module' => 0])->result();
    foreach ($services as $key => $service) {
        if ($service->id == 1) unset($services[$key]);
        if ($service->id == 22) unset($services[$key]);
    }
    ?>
    <?php
    $count_of_services = 0;
    foreach ($services as $service):
        if (has_permission('projects', '', 'create')) { ?>
            <a href="<?php echo admin_url('SOther/add/' . $service->id . '?customer_id=' . $client->userid); ?>"
               class="btn btn-info mbot25<?php if ($client->active == 0) {
                   echo ' disabled';
               } ?>"><?php echo _l('permission_create') . ' ' . $service->name; ?></a>
        <?php } ?>
        <div class="row">
            <?php
            $_where = '';
