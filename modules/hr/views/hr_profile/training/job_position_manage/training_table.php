	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		if($aColumns[$i] == '1') {
			...
			if (is_admin() || has_permission('staffmanage_training', '', 'edit')) {
				$_data .= ' <a href="' . admin_url('hr/hr_profile/position_training/' . $aRow['training_id']) . '">' . _l('hr_edit') . '</a>';
			}    

			if (is_admin() || has_permission('staffmanage_training', '', 'delete')) {
				$_data .= ' | <a href="' . admin_url('hr/hr_profile/delete_position_training/' . $aRow['training_id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
			}
