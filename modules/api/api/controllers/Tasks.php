...
class Tasks extends REST_Controller {
...

    {
        ...
        'startdate' => $this->get('startdate')  ? date('Y-m-d 00:00:00',strtotime($this->get('startdate'))) : NULL,
        'datefinished' => $this->get('datefinished') ? date('Y-m-d 00:00:00',strtotime($this->get('datefinished'))) : NULL,
        'dateadded' => $this->get('dateadded') ? date('Y-m-d 00:00:00',strtotime($this->get('dateadded'))) : NULL,
        'order_by' => $this->get('order_by') ? explode(',', $this->get('order_by')) : ['id', 'decs'],
        'rel_type' => $this->get('rel_type') ? $this->get('rel_type') : NULL,
        'rel_id' => $this->get('rel_id') ? $this->get('rel_id') : NULL,
        'status' => $this->get('status') ? $this->get('status') : NULL,
        'priority' => $this->get('priority') ? $this->get('priority') : NULL,
        ];
        // Check if the data store contains
     
        if ($tasks = $this->Task_model->get_tasks_all($filters))
        {
            $sl_data = [];
              foreach ($tasks as $task) {
                    $task->status = $this->Api_model->get_task_status($task->status);
                    $task->addedfrom_name = $this->Api_model->get_addedfrom_name($task->addedfrom);
                    $task->priority_name = $this->Api_model->get_priority_name($task->priority);
