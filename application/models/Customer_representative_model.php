...
class Customer_representative_model extends App_Model
{
    ...
    public function get($id = '')
    {
        if (is_numeric($id)) {
            ...
            return $this->db->get(db_prefix() . 'my_customer_representative')->row();
        }
        $this->db->where('is_default', 0);
        $this->db->order_by('id', 'desc');
        return $this->db->get(db_prefix() . 'my_customer_representative')->result_array();
    }
    ...
    public function add($data)
    {
        ...
        $this->db->insert(db_prefix() . 'my_customer_representative', $data);
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'my_customer_representative', $data);
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'my_customer_representative');
        if ($this->db->affected_rows() > 0) {
