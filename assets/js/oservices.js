  Dropzone.options.oserviceFilesUpload = false;
  Dropzone.options.oserviceExpenseForm = false;

  var expenseDropzone;
  $(function() {

      init_ajax_search('customer', '#clientid_copy_oservice.ajax-search');

      // remove the divider for oservice actions in case there is no other li except for pin oservice
      $('ul.oservice-actions li:first-child').next('li.divider').remove();

      var file_id = get_url_param('file_id');
      if (file_id) {
          view_oservice_file(file_id, oservice_id);
      }

      // Fix for shortcuts in discussions textarea/contenteditable - jquery-comments plugin
      var $discussionsContentEditable = $('#oservice_file_data, #discussion-comments');
      ...
      $('body').on('show.bs.modal', '._oservice_file', function() {
          discussion_comments('#oservice-file-discussion', discussion_id, 'file');
      });
      ...
      initDataTable('.table-credit-notes', admin_url + 'credit_notes/table?oservice_id=' + oservice_id, ['undefined'], ['undefined'], undefined, [0, 'desc']);

      if ($('#timesheetsChart').length > 0 && typeof(oservice_overview_chart) != 'undefined') {
          var chartOptions = {
              type: 'bar',
              data: {},
              options: {
          ...
          chartOptions.data = oservice_overview_chart.data;
          var ctx = document.getElementById("timesheetsChart");
          timesheetsChart = new Chart(ctx, chartOptions);
      }
      milestones_kanban();
      $('#oservice_top').on('change', function() {
          var val = $(this).val();
          var __oservice_group = get_url_param('group');
          if (__oservice_group) {
              __oservice_group = '?group=' + __oservice_group;
          } else {
              __oservice_group = '';
          }
          window.location.href = admin_url + 'SOther/view/'+service_id+"/" + val + __oservice_group;
      });

      if (typeof(Dropbox) != 'undefined' && $('#dropbox-chooser').length > 0) {
          document.getElementById("dropbox-chooser").appendChild(Dropbox.createChooseButton({
              success: function(files) {
                  saveoserviceExternalFile(files, 'dropbox');
              },
              linkType: "preview",
      ...
      $('body').on('click', '.milestone-column .cpicker,.milestone-column .reset_milestone_color', function(e) {
          ...
          $.post(admin_url + 'LegalServices/Other_services_controller/change_milestone_color', {
              color: color,
      ...
      if ($('#oservice-files-upload').length > 0) {
          new Dropzone('#oservice-files-upload', appCreateDropzoneOptions({
              paramName: "file",
              uploadMultiple: true,
              parallelUploads: 20,
              maxFiles: 20,
              ...
              success: function(file, response) {
                  if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
                      window.location.href = admin_url + 'SOther/view/'+service_id+"/"  + oservice_id + '?group=oservice_files';
                  }
              },
      ...
      if ($('#oservice-expense-form').length > 0) {
          expenseDropzone = new Dropzone("#oservice-expense-form", appCreateDropzoneOptions({
      ...
      appValidateForm($('#oservice-expense-form'), {
          category: 'required',
          date: 'required',
          amount: 'required',
          currency: 'required'
      }, oserviceExpenseSubmitHandler);
      ...
      gantt = $("#gantt").gantt({
          ...
          onRender: function() {
              ...
              $('#gantt .leftPanel .spacer').html('<span class="gantt_oservice_name"><i class="fa fa-cubes"></i> ' + $('.oservice-name').text() + '</span>');
              var _percent = $('input[name="oservice_percent"]').val();
          ...
          onAddClick: function(dt, rowId) {
              ...
              new_task(admin_url + 'tasks/task?rel_type='+slug+'&rel_id=' + oservice_id + '&start_date=' + d1);
          }
      });
      ...
      _table_api = initDataTable('.table-oservice-expenses', admin_url + 'LegalServices/Other_services_Controller/expenses/' + oservice_id, 'undefined', 'undefined', Expenses_ServerParams, [4, 'desc']);

      if (_table_api) {
          _table_api.column(0).visible(false, false).columns.adjust();
      }

      init_rel_tasks_table(oservice_id, 'oservice');
      initDataTable('.table-notes', admin_url + 'LegalServices/Other_services_Controller/notes/' + oservice_id, [4], [4], 'undefined', [1, 'desc']);
      ...
      initDataTable('.table-timesheets', admin_url + 'LegalServices/Other_services_Controller/timesheets/' +service_id+"/"+service_slug+"/"+ oservice_id, [8], [8], Timesheets_ServerParams, [3, 'desc']);
      initDataTable('.table-oservice-discussions', admin_url + 'LegalServices/Other_services_Controller/discussions/'+service_id+"/"+service_slug+"/" + oservice_id, undefined, undefined, 'undefined', [1, 'desc']);
      ...
      $('#timesheet select[name="timesheet_task_id"]').on('change', function() {
          ...
          requestGet('LegalServices/Other_services_controller/timesheet_task_assignees/' + _task_id + '/' + oservice_id + '/' + staff_id).done(function(response) {
              select_staff.html(response);
              select_staff.selectpicker('refresh');
          });
      });

      $('body').on('change', '#oservice_invoice_select_all_tasks,#oservice_invoice_select_all_expenses', function() {
  ...
  function oserviceFileGoogleDriveSave(pickData) {
      saveOserviceExternalFile(pickData, 'gdrive');
  }
  function saveOserviceExternalFile(files, externalType) {
      $.post(admin_url + 'LegalServices/Other_services_Controller/add_external_file', {
          files: files,
          ...
          window.location.href = location.split('?')[0] + '?group=oservice_files';
      });
  }
  ...
  function mass_stop_timers(only_billable) {
      requestGetJSON('LegalServices/Other_services_controller/mass_stop_timers/' + oservice_id + '/' + only_billable).done(function(response) {
          alert_float(response.type, response.message);
          setTimeout(function() {
              $('body').find('.modal-backdrop').eq(0).remove();
              init_timers();
              reload_tasks_tables();
              pre_invoice_oservice();
          }, 500);
      });
  }
  ...
  function oserviceExpenseSubmitHandler(form) {
  ...
  function view_oservice_file(id, $oservice_id) {
      $('#oservice_file_data').empty();
      $("#oservice_file_data").load(admin_url + 'LegalServices/Other_services_Controller/file/' + id + '/' + oservice_id, function(response, status, xhr) {
          if (status == "error") {
  ...
  function oservice_mark_as_modal(status_id, $oservice_id, target) {
      $('#mark_tasks_finished_modal').modal('show');
      $('#oservice_mark_status_confirm').attr('data-status-id', status_id);
      $('#oservice_mark_status_confirm').attr('data-oservice-id', oservice_id);
      var $oserviceMarkedAsFinishedInput = $('#oservice_marked_as_finished_email_to_contacts');
      if (status_id == 4) {
          if ($oserviceMarkedAsFinishedInput.length > 0) {
              $oserviceMarkedAsFinishedInput.parents('.oservice_marked_as_finished').removeClass('hide');
          }
      } else {
          if ($oserviceMarkedAsFinishedInput.length > 0) {
              $oserviceMarkedAsFinishedInput.prop('checked', false);
              $oserviceMarkedAsFinishedInput.parents('.oservice_marked_as_finished').addClass('hide');
          }
      }
  ...
  function gantt_filter() {
      ...
      params['group'] = 'oservice_gantt';
      if (status) {
          params['gantt_task_status'] = status;
      }
      window.location.href = buildUrl(admin_url + 'SOther/view/'+service_id+"/" + oservice_id, params);
  }

  function confirm_oservice_status_change(e) {
      var data = {};

      $(e).attr('disabled', true);

      data.oservice_id = $(e).data('oservice-id');
      data.status_id = $(e).data('status-id');

      if (data.status_id == 4) {
          var $oserviceMarkedAsFinishedInput = $('#oservice_marked_as_finished_email_to_contacts');
          if ($oserviceMarkedAsFinishedInput.length > 0) {
              data.send_oservice_marked_as_finished_email_to_contacts = $oserviceMarkedAsFinishedInput.prop('checked') === true ? 1 : 0;
          }
      }
      ...
      data.notify_oservice_members_status_change = $('#notify_oservice_members_status_change').prop('checked') === true ? 1 : 0;

      $.post(admin_url + 'LegalServices/Other_services_controller/mark_as', data).done(function(response) {
          response = JSON.parse(response);
  ...
  function milestones_kanban() {
      init_kanban('LegalServices/Other_services_controller/milestones_kanban', milestones_kanban_update, '.oservice-milestone', 445, 360, after_milestones_kanban);
  }
  ...
  // When marking task as complete if the staff in on oservice milestones area, remove this task from milestone in case exists
  function _maybe_remove_task_from_oservice_milestone(task_id) {
      var $milestonesTasksWrappers = $('.milestone-column');
      if ($("body").hasClass('oservice') && $milestonesTasksWrappers.length > 0) {
