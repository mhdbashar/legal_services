...
class Disputes_invoices_model extends App_Model
{
    ...
    {
        return $this->db->query('SELECT DISTINCT(sale_agent) as sale_agent, CONCAT(firstname, \' \', lastname) as full_name FROM ' . db_prefix() . 'my_disputes_cases_invoices JOIN ' . db_prefix() . 'staff ON ' . db_prefix() . 'staff.staffid=' . db_prefix() . 'my_disputes_cases_invoices.sale_agent WHERE sale_agent != 0')->result_array();
    }
    ...
    {
        $this->db->select('*, ' . db_prefix() . 'currencies.id as currencyid, ' . db_prefix() . 'my_disputes_cases_invoices.id as id, ' . db_prefix() . 'currencies.name as currency_name');
        $this->db->from(db_prefix() . 'my_disputes_cases_invoices');
        $this->db->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'my_disputes_cases_invoices.currency', 'left');
        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->where(db_prefix() . 'my_disputes_cases_invoices' . '.id', $id);
    ...
    public function get_invoice_recurring_invoices($id)
    {
        ...
        $invoices           = $this->db->get(db_prefix() . 'my_disputes_cases_invoices')->result_array();
        $recurring_invoices = [];
    ...
    public function get_invoices_total($data)
    {
        ...
        $noPermissionsQuery                 = disputes_get_invoices_where_sql_for_staff(get_staff_user_id());

        for ($i = 1; $i <= 3; $i++) {
            $select = 'id,total';
            if ($i == 1) {
                $select .= ', (SELECT total - (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'my_disputes_cases_invoicepaymentrecords WHERE invoiceid = ' . db_prefix() . 'my_disputes_cases_invoices.id) - (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'credits WHERE ' . db_prefix() . 'credits.invoice_id=' . db_prefix() . 'my_disputes_cases_invoices.id)) as outstanding';
            } elseif ($i == 2) {
                $select .= ',(SELECT SUM(amount) FROM ' . db_prefix() . 'my_disputes_cases_invoicepaymentrecords WHERE invoiceid=' . db_prefix() . 'my_disputes_cases_invoices.id) as total_paid';
            }
    ...
    public function add($data, $expense = false, $opponents)
    {
        ...
        if ($insert_id) {
            ...
            $this->db->where('name', 'next_disputes_invoice_number');
            ...
            foreach ($invoices_to_merge as $m) {
                ...
                if ($cancel_merged_invoices == false) {
                    ...
                    if ($this->mark_as_cancelled($m)) {
                        ...
                        $note       = 'Merged into invoice ' . disputes_format_invoice_number($insert_id);
                        if ($admin_note != '') {
                            $admin_note .= "\n\r" . $note;
                        } else {
                            $admin_note = $note;
            ...
            update_sales_total_tax_column($insert_id, 'invoice', db_prefix() . 'my_disputes_cases_invoices');
    ...
    public function check_for_merge_invoice($client_id, $current_invoice = '')
    {
        if ($current_invoice != '') {
            ...
            $row = $this->db->get(db_prefix() . 'my_disputes_cases_invoices')->row();
    ...
    public function copy($id)
    {
        ...
        $new_invoice_data['number']   = get_option('next_disputes_invoice_number');
        ...
        foreach ($_invoice->items as $item) {
            ...
            $taxes                                                  = disputes_get_invoice_item_taxes($item['id']);
    ...
    public function update($data, $id, $opponents)
    {
        ...
        $original_number_formatted = disputes_format_invoice_number($id);
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'my_disputes_cases_invoices', $data);
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($original_number != $data['number']) {
                $this->log_invoice_activity($original_invoice->id, 'invoice_activity_number_changed', false, serialize([
                    ...
                    disputes_format_invoice_number($original_invoice->id),
                ]));
            }
        }
        if (count($items) > 0) {
            foreach ($items as $key => $item) {
                ...
                if (!isset($item['taxname']) || (isset($item['taxname']) && count($item['taxname']) == 0)) {
                    ...
                    $item_taxes        = disputes_get_invoice_item_taxes($item['itemid']);
                    $_item_taxes_names = [];
        ...
        foreach ($invoices_to_merge as $m) {
            ...
            if ($cancel_merged_invoices == false) {
                ...
                if ($this->mark_as_cancelled($m)) {
                    $merged     = true;
                    $admin_note = $or_merge->adminnote;
                    $note       = 'Merged into invoice ' . disputes_format_invoice_number($id);
                    if ($admin_note != '') {
                        $admin_note .= "\n\r" . $note;
                    } else {
                        $admin_note = $note;
        ...
        if ($affectedRows > 0) {
            update_sales_total_tax_column($id, 'invoice', db_prefix() . 'my_disputes_cases_invoices');
            disputes_update_invoice_status($id);
        }
    ...
    public function delete($id, $simpleDelete = false)
    {
        if (get_option('delete_only_on_last_invoice') == 1 && $simpleDelete == false) {
            if (!disputes_is_last_invoice($id)) {
                return false;
            }
        }
        $number = disputes_format_invoice_number($id);

        hooks()->do_action('before_invoice_deleted', $id);
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'my_disputes_cases_invoices');
        if ($this->db->affected_rows() > 0) {
    ...
    public function set_invoice_sent($id, $manually = false, $emails_sent = [], $is_status_updated = false)
    {
        ...
        if ($is_status_updated == false) {
            disputes_update_invoice_status($id, true);
        }
...
foreach ($client_ids as $clientid) {
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' . db_prefix() . 'my_disputes_cases_invoices ORDER BY year DESC')->result_array();
    }
