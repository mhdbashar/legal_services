...
<script>
    (function() {
        ...
        $("#frame").on('keypress', 'textarea.client_chatbox', function(e) {
            ...
            if (e.which == 13) {
                ...
                $('.client_messages .chat_client_messages ul').append('<li class="sent" id="' + userSessionId + '"><img data-toggle="tooltip" title="' + moment().format('hh:mm A') + '" class="myProfilePic" src="' + imgPath + '"/><p class="you" id="' + userSessionId + '">' + message + '</p></li>');
            ...
            promise.then(function(messages) {
                ...
                $(messages).each(function(i, value) {
                    ...
                    if (value.sender_id == 'staff_' + staff_id) {
                        clientUl.prepend('<li class="sent" id="' + value.id + '"><img data-toggle="tooltip" data-container="body" data-placement="left" title="' + value.time_sent_formatted + '" class="myProfilePic" src="' + own_image_url + '"/><p class="you" id="msg_' + value.id + '">' + value.message + '</p></li>');
                        ...
                        clientUl.prepend('<li class="replies" data-sqldate="' + value.time_sent + '"><img class="clientProfilePic" data-toggle="tooltip" data-container="body" data-placement="right" data-original-title="' + value.time_sent_formatted + '" src="' + value.client_image_path + '"/><p  class="client">' + value.message + '</p><a href="#" class="btn btn-primary btn-xs btn_convert_conversation_to_ticket">Convert to ticket</a></li>');
                    }
                });
                $('.group_members_inline').remove();
            });
                    ...
                    .done(function(messages) {
                        ...
                        $(messages).each(function(i, value) {
                            ...
                            if (value.sender_id == 'staff_' + userSessionId) {
                                clientUl.prepend('<li class="sent" id="' + value.id + '"><img data-toggle="tooltip" data-container="body" data-placement="left" data-original-title="' + value.time_sent_formatted + '" class="myProfilePic" src="' + own_image_url + '"/><p class="you" id="msg_' + value.id + '">' + value.message + '</p></li>');

                                if (previous_time !== current_time) {
                                    $('<span class="middleDateTime">' + moment(value.time_sent).format('llll') + '</span>').prependTo($('.client_messages ul li#' + value.id).parents('ul'));
                                }
                            } else {
                                clientUl.prepend('<li class="replies" data-sqldate="' + value.time_sent + '"><img class="clientProfilePic" data-toggle="tooltip" data-container="body" data-placement="right" data-original-title="' + value.time_sent_formatted + '" src="' + value.client_image_path + '"/><p  class="client">' + value.message + '</p></li>');
                            }
        ...
        clientsChannel.bind('send-event', function(data) {
            ...
            if (selectedContact == data.from && data.to == 'staff_' + userSessionId) {
                ...
                $('.client_messages#' + data.from + ' .chat_client_messages ul')
                    .append('<li class="replies" data-sqldate="' + moment().format("YYYY-MM-DD HH:mm:ss") + '"><img class="clientProfilePic" data-toggle="tooltip" data-original-title="' + moment().format('hh:mm A') + '" src="' + data.client_image_path + '"/><p class="client">' + data.message + '</p><a href="#" class="btn btn-primary btn-xs btn_convert_conversation_to_ticket">Convert to ticket</a></li></li>');
    ...
    $('body').on('click', '.chat_client_messages .btn_convert_conversation_to_ticket', function() {
        var id = $('.tab-pane.fade.active.in .company_selector.active .contact_name.selected').attr('id');
