...
<div class="row">
    <div class="col-md-12">
        ...
        <?php /* <div class="form-group select-placeholder projects-wrapper<?php if((!isset($subscription)) || (isset($subscription) && !customer_has_projects($subscription->clientid))){ echo ' hide';} ?>">
        ...
        <?php
        if((!isset($subscription)) || (isset($subscription) && !customer_has_cases($subscription->clientid)) || (isset($subscription) && !customer_has_oservices($subscription->clientid))){
        ...
        ?>
        <?php
        $selected = (isset($subscription) ? $subscription->rel_stype : '');
        ...
        <div class="form-group <?php echo $hide_project_selector; ?>" id="div_rel_sid">
            ...
            <select class="form-control custom_select_arrow" id="rel_sid" name="rel_sid" placeholder="<?php echo _l('dropdown_non_selected_tex'); ?>">
                ...
                <?php
                if(isset($subscription) && $subscription->rel_sid != 0){
                    $this->load->model('legalservices/LegalServicesModel', 'legal');
                    $slug = (isset($subscription) ? $subscription->rel_stype : '');
                    $ServID = $this->legal->get_service_id_by_slug($slug);
                    if($ServID == 1){
                        $service_name = get_case_name_by_id($subscription->rel_sid);
                    }elseif ($ServID == 22){
                        $service_name = get_disputes_case_name_by_id($subscription->rel_sid);
                    }else{
                        $service_name = get_oservice_name_by_id($subscription->rel_sid);
                    }
                    echo '<option value="'.$subscription->rel_sid.'" selected>'.$service_name.'</option>';
                }
                ?>
            </select>
        </div>
