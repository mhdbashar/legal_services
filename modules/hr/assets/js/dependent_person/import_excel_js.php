...
<script>
	...
	$(function(){
		...
		{
			$( "#dowload_file_sample" ).append( '<a href="'+ site_url+'modules/hr/uploads/sample_file/Sample_import_hrm_dependent_person_file_vi.xlsx" class="btn btn-primary mright5" ><?php echo _l('hr_download_sample') ?></a>' );
			$( "#dowload_file_sample" ).append( '<a href="<?php echo admin_url('hr/hr_profile/dependent_persons'); ?>" class=" mright5 btn btn-default" ><?php echo _l('hr__back') ?></a><hr>' );

		}else{
			$( "#dowload_file_sample" ).append( '<a href="'+ site_url+'modules/hr/uploads/sample_file/Sample_import_hrm_dependent_person_file_en.xlsx" class="btn btn-primary mright5" ><?php echo _l('hr_download_sample') ?></a>' );
			$( "#dowload_file_sample" ).append( '<a href="<?php echo admin_url('hr/hr_profile/dependent_persons'); ?>" class=" mright5 btn btn-default" ><?php echo _l('hr__back') ?></a><hr>' );
		}
	...
	function uploadfilecsv(){
		...
		if(($("#file_csv").val() != '') && ($("#file_csv").val().split('.').pop() == 'xlsx')){
			...
			$.ajax({ 
				url: admin_url + 'hr/hr_profile/import_dependent_person_excel',
				method: 'post', 
				data: formData, 
				contentType: false, 
				processData: false
