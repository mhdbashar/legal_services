...
if ($project->billing_type == 2 || $project->billing_type == 3) {
    ...
    $logged_time_data = $this->ci->Dcase->total_logged_time_by_billing_type($slug, $project->id);
    ...
    $logged_time_data = $this->ci->Dcase->data_billable_time($slug, $project->id);
    ...
    $logged_time_data = $this->ci->Dcase->data_billed_time($slug, $project->id);
    ...
    $logged_time_data = $this->ci->Dcase->data_unbilled_time($slug, $project->id);
...
foreach ($members as $member) {
    ...
    $html .= '<td>' . total_rows(db_prefix().'my_disputes_casediscussions', ['staff_id' => $member['staff_id'], 'project_id' => $project->id]) . '</td>';
    $html .= '<td>' . total_rows(db_prefix().'my_disputes_casediscussioncomments', 'staff_id=' . $member['staff_id'] . ' AND discussion_id IN (SELECT id FROM '.db_prefix().'my_disputes_casediscussions WHERE project_id=' . $project->id . ')') . '</td>';
    $html .= '<td>' . total_rows(db_prefix().'my_disputes_case_files', ['staffid' => $member['staff_id'], 'project_id' => $project->id]) . '</td>';
