...
$additionalSelect = ['id','service_id'];
$join             = [
    'JOIN ' . db_prefix() . 'clients ON ' . db_prefix() . 'clients.userid = ' . db_prefix() . 'my_other_services.clientid',
    ];
...
array_push($where, ' AND ' . db_prefix() . 'my_other_services.id IN (SELECT oservice_id FROM ' . db_prefix() . 'my_members_services WHERE staff_id=' . $this->ci->db->escape_str($staff_id) . ')');
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0 ; $i < count($aColumns) ; $i++) {
        ...
        if ($aColumns[$i] == 'start_date' || $aColumns[$i] == 'deadline') {
            ...
            $status = get_oservice_status_by_id($_data);
