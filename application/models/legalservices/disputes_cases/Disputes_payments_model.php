...
class Disputes_payments_model extends App_Model
{
    ...
    {
        $this->db->select('*,' . db_prefix() . 'my_disputes_cases_invoicepaymentrecords.id as paymentid');
        $this->db->join(db_prefix() . 'payment_modes', db_prefix() . 'payment_modes.id = ' . db_prefix() . 'my_disputes_cases_invoicepaymentrecords.paymentmode', 'left');
        $this->db->order_by(db_prefix() . 'my_disputes_cases_invoicepaymentrecords.id', 'asc');
        $this->db->where(db_prefix() . 'my_disputes_cases_invoicepaymentrecords.id', $id);
        $payment = $this->db->get(db_prefix() . 'my_disputes_cases_invoicepaymentrecords')->row();
        if (!$payment) {
            return false;
        }
        // Since version 1.0.1
    ...
    public function get_invoice_payments($invoiceid)
    {
        ...
        $payments = $this->db->get(db_prefix() . 'my_disputes_cases_invoicepaymentrecords')->result_array();
        // Since version 1.0.1
        $this->load->model('payment_modes_model');
        $payment_gateways = $this->payment_modes_model->get_payment_gateways(true);
        $i                = 0;
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'my_disputes_cases_invoicepaymentrecords', $data);
