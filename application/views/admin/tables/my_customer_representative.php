...
$aColumns = [
    '1',
    db_prefix() .'customer_representative.id as id',
    'representative',
];
$join = [];
foreach ($custom_fields as $key => $field) {
    ...
    array_push($join, 'LEFT JOIN ' . db_prefix() . 'customfieldsvalues as ctable_' . $key . ' ON ' . db_prefix() . 'customer_representative.id = ctable_' . $key . '.relid AND ctable_' . $key . '.fieldto="' . $field['fieldto'] . '" AND ctable_' . $key . '.fieldid=' . $field['id']);
}
$where  = [];
$filter = [];
$sIndexColumn = 'id';
$sTable       = db_prefix() . 'customer_representative';
