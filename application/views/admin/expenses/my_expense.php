            ...
            <div class="col-md-6">
                <div class="panel_s">
                    <div class="panel-body">
                      ...
                      <?php /* <div class="form-group projects-wrapper<?php echo $hide_project_selector; ?>">
                        ...
                        <?php
                        $selected = (isset($expense) ? $expense->rel_stype : '');
                        ...
                        <div class="form-group <?php echo $hide_project_selector; ?>" id="div_rel_sid">
                            ...
                            <select class="form-control custom_select_arrow" id="rel_sid" name="rel_sid" placeholder="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                ...
                                <?php
                                if(isset($expense) && $expense->rel_sid != 0){
                                    $this->load->model('legalservices/LegalServicesModel', 'legal');
                                    $slug = (isset($expense) ? $expense->rel_stype : '');
                                    $ServID = $this->legal->get_service_id_by_slug($slug);
                                    if($ServID == 1){
                                        $service_name = get_case_name_by_id($expense->rel_sid);
                                    }elseif ($ServID == 22){
                                        $service_name = get_disputes_case_name_by_id($expense->rel_sid);
                                    }else{
                                        $service_name = get_oservice_name_by_id($expense->rel_sid);
                                    }
                                    echo '<option value="'.$expense->rel_sid.'" selected>'.$service_name.'</option>';
                                }
                                ?>
                            </select>
                        </div>
...
<script>
    ...
    function customer_init(){
        $('#div_rel_stype').removeClass('hide');
