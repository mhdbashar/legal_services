...
<div class="modal-body">
    <div class="row">
        ...
        <div class="col-md-4 task-single-col-right">
                                ...
                                <div>
                                    <p class="bold">
                                        ...
                                        <?php if ($reminder['creator'] == get_staff_user_id() || is_admin()) { ?>
                                            ...
                                            <?php } ?>
                                            <a href="<?php echo admin_url('tasks/delete_reminder_session/' . $task->id . '/' . $reminder['id']); ?>" class="text-danger delete-reminder"><i class="fa fa-remove"></i></a>
                                        <?php } ?>
                                    </p>
                                    <?php
            ...
            <hr class="task-info-separator" />
            <?php echo form_open_multipart('admin/tasks/upload_file_session',array('id'=>'task-attachment','class'=>'dropzone')); ?>
            <?php echo form_close(); ?>
...
<script>
    ...
    function make_session_public(task_id) {
        requestGetJSON('tasks/make_session_public/' + task_id).done(function(response) {
    ...
    function unmark_complete(task_id) {
        task_mark_as(4, task_id, 'tasks/unmark_session_complete/' + task_id);
    }

    // Mark task status
    function task_mark_as(status, task_id, url) {
        url = typeof(url) == 'undefined' ? 'tasks/mark_as_session/' + status + '/' + task_id : url;
    ...
    function add_task_comment(task_id) {
        ...
        data.taskid = task_id;
        $.post(admin_url + 'tasks/add_session_comment', data).done(function(response) {
            response = JSON.parse(response);
            _task_append_html(response.taskHtml);
    ...
    function save_edited_comment(id, task_id) {
        ...
        $.post(admin_url + 'tasks/edit_session_comment', data).done(function(response) {
            response = JSON.parse(response);
            if (response.success === true || response.success == 'true') {
    ...
    function task_change_priority(priority_id, task_id) {
        url = 'tasks/change_session_priority/' + priority_id + '/' + task_id;
        var taskModalVisible = $('#task-modal').is(':visible');
        url += '?single_task=' + taskModalVisible;
        requestGetJSON(url).done(function(response) {
            if (response.success === true || response.success == 'true') {
    ...
    function new_task_reminder(id) {
        ...
        if (!$container.is(':visible') || $container.is(':visible') && $container.attr('data-edit') != undefined) {
            ...
            $container.find('form').attr('action', admin_url + 'tasks/add_reminder_session/' + id);
    ...
    function edit_reminder(id, e) {
        requestGetJSON('misc/get_reminder/' + id).done(function(response) {
            ...
            if ($container.length === 0 && $('body').hasClass('all-reminders')) {
                ...
                actionURL = admin_url + 'tasks/edit_reminder_session/' + id;
    ...
    function remove_assignee(id, task_id) {
        if (confirm_delete()) {
            requestGetJSON('tasks/remove_assignee_session/' + id + '/' + task_id).done(function(response) {
                if (response.success === true || response.success == 'true') {
                    alert_float('success', response.message);
                    _task_append_html(response.taskHtml);
                }
    ...
    function remove_follower(id, task_id) {
        if (confirm_delete()) {
            requestGetJSON('tasks/remove_follower_session/' + id + '/' + task_id).done(function(response) {
                if (response.success === true || response.success == 'true') {
                    alert_float('success', response.message);
                    _task_append_html(response.taskHtml);
                }
    ...
    function timer_action(e, task_id, timer_id, adminStop) {
        ...
        var taskModalVisible = $('#task-modal').is(':visible');
        var reqUrl = admin_url + 'tasks/timer_tracking_session?single_task=' + taskModalVisible;
        if (adminStop) {
    ...
    <script>
        ...
        async function pickerCallback(data) {
            if (data.action === google.picker.Action.PICKED) {
                ...
                taskExternalFileUpload(retVal, 'gdrive', <?php echo $task->id; ?>);
            }
        }
