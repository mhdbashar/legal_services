...
<div id="wrapper">
                ...
                <div class="panel_s">
                    <div class="panel-body">
                        <?php if (!$this->input->get('project_id')) { ?>
                            <a href="<?php echo admin_url('legalservices/sessions'); ?>"
                               class="btn btn-default pull-left"><?php echo _l('back_to_sessions_list'); ?></a>
                        <?php } else {
                        ...
                        <div class="row">
                            ...
                            <?php if (has_permission('sessions', '', 'view')) { ?>
                            ...
                            <div class="col-md-2 text-center border-right">
                                <div class="form-group no-margin select-placeholder">
                                    <select name="status" id="status" class="selectpicker no-margin" data-width="100%"
                                        ...
                                        <option value="previous" <?php if ($this->input->post('status') == 'previous') {
                                            echo 'selected';
                                        } ?>><?php echo _l('Previous_Sessions'); ?></option>
                                        <option value="waiting" <?php if ($this->input->post('status') == 'waiting') {
                                            echo 'selected';
                                        } ?>><?php echo _l('Waiting_sessions'); ?></option>
                    ...
                    <div class="panel-body">
                        <?php foreach ($overview as $month => $data) {
                            ...
                            <h4 class="bold text-success"><?php echo _l(date('F', mktime(0, 0, 0, $month, 1))); ?>
                                ...
                                <?php if (is_numeric($staff_id) && has_permission('sessions', '', 'view')) {
 ...
 -->                           <!-- <th><?php //echo _l('task_finished_on_time'); ?></th> -->
                                ...
                                foreach ($data as $task) { ?>
                                    <tr>
                                        <td data-order="<?php echo htmlentities($task['name']); ?>"><a
                                                    href="<?php echo admin_url('legalservices/sessions/view/' . $task['id']); ?>"
                                                    onclick="init_session_modal(<?php echo $task['id']; ?>); return false;"><?php echo $task['name']; ?></a>
                                            <?php
                              ...
                              <span class="label label-default-light" data-toggle="tooltip"
                              ...
                              <span class="label <?php //if($task['total_checklist_items'] == '0'){ echo 'label-default-light'; } else if(($task['total_finished_checklist_items'] != $task['total_checklist_items'])){ echo 'label-danger';
                                        //} else if($task['total_checklist_items'] == $task['total_finished_checklist_items']){echo 'label-success';}
                                        ?> pull-left mright5" data-toggle="tooltip" data-title="<?php //echo _l('tasks_total_checklists_finished');
                                        ?>">
