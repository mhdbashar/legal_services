...
class Templates extends AdminController
{
    ...
    public function template($id = null)
    {
        ...
        if ($data['type'] == 'contracts') {
            redirect(admin_url('contracts/contract/' . $rel_id));
        } elseif ($data['type'] == 'hr_contracts') {
            redirect(admin_url('hr_profile/contracts/#' . $rel_id));
        } else {
            redirect(admin_url('proposals/list_proposals/' . $rel_id));
        }
    }
