...
<?php init_head();?>
<link href="<?php echo module_dir_url('babil_export_customer','assets/css/babil_export_customer_style.css'); ?>" rel="stylesheet" />
<div id="wrapper">
	<div class="content">
		<div class="row">
			<div class="col-md-12">
				<div class="panel_s">
					<div class="panel-body">
						...
						<h4 class="pull-left"><?php echo _l('babil_customer_services'); ?></h4>
						<button type="submit" data-toggle="tooltip" data-title="<?php echo _l('babil_customer_apply_filter'); ?>" class=" pull-right btn btn-info mleft4"><?php echo _l('filter'); ?></button>
						<div class="clearfix"></div>
						<hr />
						<div class="row">
							<div class="col-md-3 border-right">
								<label><?php echo _l('babil_customer_filter_by');?></label><br/>
								<div class="radio radio-inline radio-primary">
								  <input type="radio" id="filter_by2" name="filter_by" value="customer" <?php if($filter_by == 'customer'){echo 'checked';} ?>>
								  <label for="filter_by2"><?php echo _l('babil_customer_filter_customer'); ?></label>
								</div>
								<div class="radio radio-inline radio-primary">
								  <input type="radio" id="filter_by1" name="filter_by" value="service" <?php if($filter_by == 'service'){echo 'checked';} ?>>
								  <label for="filter_by1"><?php echo _l('babil_customer_filter_service'); ?></label>
								</div>
							</div>
							...
							<?php } ?>
							<div class="col-md-2 border-right">
								<label id='label_filter_by' class="control-label"><?php echo ($filter_by=='customer'?_l('babil_customer_filter_customer'):_l('babil_customer_filter_service')); ?></label>		
								<div class="form-group no-margin select-placeholder">
									<?php echo render_select('search_list',$search_list,array('id','name'),'',$filter_id,array('data-none-selected-text'=>_l('babil_customer_select_all')),array(),'no-margin'); ?>
								</div>
							</div>
							<div class="col-md-2 border-right">
								<label id='label_filter_by' class="control-label"><?php echo _l('item_group_name'); ?></label>		
								<div class="form-group no-margin select-placeholder">
									<?php echo render_select('group_list',$group_list,array('id','name'),'',$group_id,array('data-none-selected-text'=>_l('babil_customer_select_all')),array(),'no-margin'); ?>
								</div>
							</div>
							<div class="col-md-2 border-right form-group">
								<label for="group_by" class="control-label"><span class="control-label"><?php echo _l('babil_customer_group_by'); ?></span></label>
								<select name="group_by" id="group_by" class="selectpicker no-margin" data-width="100%">
									...
									<option value="item" <?php echo ($group_by!='' && $group_by=='item'?'selected':'')?>><?php echo _l('babil_customer_filter_service'); ?></option>
									<option value="item_group" <?php echo ($group_by!='' && $group_by=='item_group'?'selected':'')?>><?php echo _l('babil_customer_filter_service_group'); ?></option>
								</select>
							</div>
						</div><!--end row-->
						<?php echo form_close(); ?>
					...
					<div class="panel-body">
						...
						foreach($table_data as $key_group =>$data){ if(count($data) == 0){continue;} $i=1; ?>
							...
							<table class="table tasks-overview dt-table scroll-responsive">
								<caption class="babil_export_customer_caption"><?php echo htmlspecialchars($key_group);?></caption>
								<thead>
									<tr>
										<th>#</th>
										...
										<th><?php echo _l('babil_customer_filter_service'); ?></th>
										<?php } ?>
										<?php if($group_by!='item_group'){?>
										<th><?php echo _l('babil_customer_filter_service_group'); ?></th>
										<?php } ?>
										<?php if(has_permission('staff','','view')){ ?>
...
<script src="<?php echo module_dir_url('babil_export_customer','assets/js/babil_export_customer_customer_service_report.js'); ?>"></script>
<script>
	var txt_customer = '<?php echo _l('babil_customer_filter_customer');?>';
	var txt_service = '<?php echo _l('babil_customer_filter_service'); ?>';
</script>