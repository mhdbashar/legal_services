...
class Whatsapp extends AdminController
{
    ...
    {
        $messages = _get_whatsapp_messages();
    ...
    {
        $data = $this->Whatsapp_model->get($id);
    ...
    public function get_sms($id)
    {
        ...
        if (is_object($sms))
        {
            ...
            $success = $this->Whatsapp_model->add($data);
            if($success){
    ...
    public function save_sms()
    {
        ...
        $success = $this->Whatsapp_model->update($msg_id, $data);
        if($success){
    ...
    public function saved_messages()
    {
        ...
        {
            $this->babilsmsgateway->get_table_data('saved_whatsapp_messages_table');
        }
        $data['legal_services'] = $this->legal->get_all_services();
        $data['rel_type']    = $this->input->get('rel_type');
        $data['rel_id']    = $this->input->get('rel_id');
        ...
        $this->load->view('saved_whatsapp_messages/manage', $data);
    }

    public function delete($id)
    {
        $response = $this->Whatsapp_model->delete($id);
        if ($response == true) {
