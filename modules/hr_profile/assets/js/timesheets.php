<script>
  ...
  (function(){
    ...
    $('.timesheets_filter').on('click', function() {
      ...
      $.post(admin_url + 'hr_profile/reload_timesheets_byfilter', data).done(function(response) {
 ...
 function show_detail_timesheets(data){
   ...
   if($('input[name="is_edit"]').val() == 0){
    ...
    data.month = month;
    $.post(admin_url + 'hr_profile/show_detail_timesheets', data).done(function(response) {
      response = JSON.parse(response);
