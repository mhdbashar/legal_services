...
class Babil_export_customer_model extends App_Model
{
	...
	*/
	public function update_client_kyc($data,$client_id)
	{
		if (is_numeric($client_id)) {
			...
			$result = $this->db->get(db_prefix() . 'babil_export_customer_kyc_files');
			if($result->num_rows()>0)
			{
				$this->db->where('client_id',$client_id);
				$update = $this->db->update(db_prefix() . 'babil_export_customer_kyc_files', $data);
				if ($update) {
					return true;
				}
				return false;
			...
			else
			{
				...
				$insert = $this->db->insert(db_prefix() . 'babil_export_customer_kyc_files', $data);
				if ($insert) {
					return true;
				}
				return false;
	...
	*/
	public function update_client_services($data,$client_id)
	{
		if (is_numeric($client_id)) {
			$this->db->where('client_id',$client_id);
			$this->db->delete(db_prefix() . 'babil_export_customer_services');
			$insert_data = array();
			if(!empty($data))
			{
				foreach($data as $item_id)
					$insert_data[] = array('client_id'=>$client_id,'item_id'=>$item_id);
				$this->db->insert_batch(db_prefix() . 'babil_export_customer_services', $insert_data);
			}
		}
	}
	...
	*/
    ...
    function get_tasks_total($data)
    {
        ...
        foreach ($tasks as $task){
            ...
            if((time()-(60*60*24)) > strtotime($task['startdate'])){
                $previous++;
            }else{
                $waiting++;
            }
        ...
        $prev['name'] = _l('previous');
        $prev['total_sessions']    = $previous;
        $prev['color'] = '#a7a1a1';


        $wait = [];

        $wait['name'] = _l('waiting');
        $wait['total_sessions']    = $waiting;
        $wait['color'] = '#a7a1a1';
...
//	}
	...
	function get_customer_services_list($client_id,$group_id,$staff_id)	
	{
		if (is_numeric($client_id)) {
			...
			$this->db->join(db_prefix() . 'items',db_prefix() . 'items.id='.db_prefix() . 'babil_export_customer_services.item_id','left');
			$this->db->join(db_prefix() . 'items_groups',db_prefix() . 'items_groups.id='.db_prefix() . 'items.group_id','left');
			$this->db->join(db_prefix() . 'clients',db_prefix() . 'clients.userid='.db_prefix() . 'babil_export_customer_services.client_id','left');
			$this->db->join(db_prefix() . 'customer_groups',db_prefix() . 'customer_groups.customer_id='.db_prefix() . 'babil_export_customer_services.client_id','left');
			
			if($client_id > 0)
				$this->db->where(db_prefix() . 'babil_export_customer_services.client_id',$client_id);
			if($group_id > 0)
			{
			...
			if($staff_id > 0)
			{
				$this->db->join(db_prefix() . 'customer_admins',db_prefix() . 'customer_admins.customer_id='.db_prefix() . 'babil_export_customer_services.client_id','left');
				$this->db->where(db_prefix() . 'customer_admins.staff_id',$staff_id);	
			}
			$this->db->where(db_prefix() . 'clients.active',1);
			$this->db->group_by(db_prefix() . 'babil_export_customer_services.client_id,'.db_prefix() . 'items.id');
					
			$result = $this->db->get(db_prefix() . 'babil_export_customer_services');
			if($result->num_rows()>0)
			{
