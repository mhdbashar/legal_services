...
class Session_discussions_model extends App_Model
{
    ...
    {
        if ($project_id != '') {
            $this->db->where('session_id', $project_id);
        }
        $this->db->where('id', $id);
    ...
    {
        $this->db->where('id', $id);
        $comment = $this->db->get(db_prefix() . 'my_sessiondiscussioncomments')->row();
    ...
    public function get_discussion_comments($id, $type)
    {
        ...
        $comments             = $this->db->get(db_prefix() . 'my_sessiondiscussioncomments')->result_array();
        $i                    = 0;
        $allCommentsIDS       = [];
    ...
    public function get_discussions($project_id)
    {
        ...
        $discussions = $this->db->get(db_prefix() . 'my_sessiondiscussions')->result_array();
        $i           = 0;
        foreach ($discussions as $discussion) {
            $discussions[$i]['total_comments'] = total_rows(db_prefix() . 'my_sessiondiscussioncomments', [
    ...
    public function add_discussion_comment($data, $discussion_id, $type)
    {
        ...
        $this->db->insert(db_prefix() . 'my_sessiondiscussioncomments', $_data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            if ($type == 'regular') {
                $discussion = $this->get_discussion($discussion_id);
                $not_link   = 'projects/view/' . $discussion->session_id . '?group=project_discussions&discussion_id=' . $discussion_id;
            } else {
                $discussion                   = $this->get_file($discussion_id);
                $not_link                     = 'projects/view/' . $discussion->session_id . '?group=project_files&file_id=' . $discussion_id;
    ...
    public function update_discussion_comment($data)
    {
        ...
        $this->db->where('id', $data['id']);
        $this->db->update(db_prefix() . 'my_sessiondiscussioncomments', [
    ...
    public function add_discussion($data)
    {
        ...
        $data['description'] = nl2br($data['description']);
        $this->db->insert(db_prefix() . 'my_sessiondiscussions', $data);
        if ($this->db->affected_rows() > 0) 
            return true;
        else 
            return false;
    ...
    public function edit_discussion($data, $id)
    {
        ...
        $data['description'] = nl2br($data['description']);
        $this->db->update(db_prefix() . 'my_sessiondiscussions', $data);
        if ($this->db->affected_rows() > 0) {
            $this->log_activity($data['session_id'], 'project_activity_updated_discussion', $data['subject'], $data['show_to_customer']);
